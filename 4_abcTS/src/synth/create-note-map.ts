...
export default function createNoteMap(sequence): any[] {
    const map = [];
    for (let i: number = 0; i < sequence.tracks.length; i++)
        map.push([]);
    ...
    sequence.tracks.forEach((track, i) => {
        track.forEach((ev) => {
            switch (ev.cmd) {
                case "note":
                    const inst: string = ev.instrument !== undefined ? instrumentIndexToName[ev.instrument] : currentInstrument;
                    if (ev.duration > 0) {
