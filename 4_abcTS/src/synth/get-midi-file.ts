...
export default function getMidiFile(source, options): string | Uint8Array | string | Uint8Array {
    const params = Object.assign({}, options);
    params.generateInline = false;
    const callback = (div, tune, index): string | Uint8Array => {
        const downloadMidi: string = midiCreate(tune, params);
        switch (params.midiOutputType) {
            case "encoded":
                return downloadMidi;
            case "binary":
                let decoded: string = downloadMidi.replace("data:audio/midi,", "");
                decoded = decoded.replace(/MThd/g, "%4d%54%68%64");
                decoded = decoded.replace(/MTrk/g, "%4d%54%72%6b");
                const output: Uint8Array = new Uint8Array(decoded.length / 3);
                for (let i: number = 0; i < decoded.length / 3; i++) {
                    const p: number = i * 3 + 1;
                    const d: number = parseInt(decoded.substring(p, p + 2), 16);
                    output[i] = d;
                }
                return output;
            case "link":
...
function generateMidiDownloadLink(tune, midiParams, midi: string, index): string {
    const divClasses: string[] = ['abcjs-download-midi', `abcjs-midi-${index}`];
    if (midiParams.downloadClass)
        divClasses.push(midiParams.downloadClass);
    let html: string = `<div class="${divClasses.join(' ')}">`;
