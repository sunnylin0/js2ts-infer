...
const notSupportedMessage: string = "MIDI is not supported in this browser.";
const originalSoundFontUrl: string = "https://paulrosen.github.io/midi-js-soundfonts/abcjs/";
const defaultSoundFontUrl: string = "https://paulrosen.github.io/midi-js-soundfonts/FluidR3_GM/";
const alternateSoundFontUrl: string = "https://paulrosen.github.io/midi-js-soundfonts/MusyngKite/";
export default class CreateSynth {
    ...
    onEnded: any;
    callbackContext: any;
    sequenceCallback: any;
    flattened: any;
    pan: any;
    audioBufferPossible: any;
    debugCallback: any;
    directSource = [];
    audioBuffers = [];
    isRunning = false;
    ...
    init(options: SynthSequence): Promise<unknown> {
        if (!options)
            options = {};
        if (options.options)
        ...
        if (this.debugCallback)
            this.debugCallback("init called");
        ...
        if (this.soundFontUrl[this.soundFontUrl.length - 1] !== '/')
            this.soundFontUrl += '/';
        ...
        if (params.programOffsets)
            ...
            this.programOffsets = {};
        const fadeLen: number = params.fadeLength !== undefined ? parseInt(params.fadeLength, 10) : NaN;
        this.fadeLength = isNaN(fadeLen) ? 200 : fadeLen;
        const nEnd: number = params.noteEnd !== undefined ? parseInt(params.noteEnd, 10) : NaN;
        ...
        if (options.visualObj) {
            ...
            if (meter.den)
                this.meterSize = meter.num / meter.den;
        ...
        const cached = [];
        const errorNotes = [];
        let currentInstrument: string = instrumentIndexToName[0];
        this.flattened.tracks.forEach((track) => {
            track.forEach((event) => {
                ...
                if (event.pitch !== undefined) {
                    ...
                    const inst: string = event.instrument !== undefined ? instrumentIndexToName[event.instrument] : currentInstrument;
                    if (noteName) {
                        if (!allNotes[inst])
                            allNotes[inst] = {};
                        if (!soundsCache[inst] || !soundsCache[inst][noteName])
                            ...
                            const label2: string = inst + ":" + noteName;
                            if (cached.indexOf(label2) < 0)
                                cached.push(label2);
                        }
                    }
                    else {
                        const label: string = inst + ":" + noteName;
        ...
        if (this.debugCallback)
            this.debugCallback("notes " + JSON.stringify(notes));
        const batches = [];
        const CHUNK: number = 256;
        for (let i: number = 0; i < notes.length; i += CHUNK) {
            batches.push(notes.slice(i, i + CHUNK));
        }
        return new Promise((resolve, reject) => {
            const results = { cached, error: errorNotes, loaded: [] };
            let index: number = 0;
            const next = (): void => {
                if (this.debugCallback)
                    this.debugCallback("loadBatch idx=" + index + " len=" + batches.length);
                if (index < batches.length) {
                    this._loadBatch(batches[index], this.soundFontUrl, startTimeLoading).then((data) => {
                        if (this.debugCallback)
                            this.debugCallback("loadBatch then");
                    ...
                    if (this.debugCallback)
                        this.debugCallback("resolve init");
                    resolve(results);
                }
            };
            next();
    ...
    _loadBatch(batch, soundFontUrl: string, startTime, delay): Promise<unknown> {
        const promises = [];
        batch.forEach(item => {
            if (this.debugCallback)
                this.debugCallback("getNote " + item.instrument + ':' + item.note);
        ...
        return Promise.all(promises).then((response) => {
            ...
            for (let i: number = 0; i < response.length; i++) {
                ...
                const which: string = oneResponse.instrument + ":" + oneResponse.name;
            ...
            if (pending.length > 0) {
                if (this.debugCallback)
                    this.debugCallback("pending " + JSON.stringify(pending));
                if (!delay)
                    delay = 50;
                else
                    delay = delay * 2;
                if (delay < 90000) {
                    return new Promise((resolve, reject) => {
                        setTimeout(() => {
                            ...
                            if (this.debugCallback)
                                this.debugCallback("retry " + JSON.stringify(newBatch));
                    ...
                    if (this.debugCallback)
                        this.debugCallback("loadBatch timeout");
                    return Promise.reject(new Error("timeout attempting to load: " + list.join(", ")));
                }
            }
            else {
                if (this.debugCallback)
                    this.debugCallback("loadBatch resolve");
                return Promise.resolve({ loaded, cached, error });
            }
        }).catch(error => {
            if (this.debugCallback)
                this.debugCallback("loadBatch catch " + error.message);
        });
    }
    prime(): Promise<unknown> {
        const fadeTimeSec: number = this.fadeLength / 1000;
        this.isRunning = false;
        if (!this.audioBufferPossible)
            return Promise.reject(new Error(notSupportedMessage));
        if (this.debugCallback)
            this.debugCallback("prime called");
        return new Promise((resolve, reject) => {
            try {
                const startTime = activeAudioContext().currentTime;
                const tempoMultiplier: number = this.millisecondsPerMeasure / 1000 / this.meterSize;
                ...
                const totalSamples: number = Math.floor(activeAudioContext().sampleRate * this.duration);
                this.stop();
                const noteMapTracks: any[] = createNoteMap(this.flattened);
                ...
                const panDistances: any[] = this._setPan(noteMapTracks.length, this.pan);
                const uniqueSounds: {} = {};
                noteMapTracks.forEach((noteMap, trackNumber) => {
                    ...
                    noteMap.forEach(note => {
                        const key: string = note.instrument + ':' + note.pitch + ':' + note.volume + ':' + Math.round((note.end - note.start) * 1000) / 1000 + ':' + panDistance + ':' + tempoMultiplier + ':' + (note.cents ? note.cents : 0);
                        if (!uniqueSounds[key])
                            uniqueSounds[key] = [];
                ...
                Object.keys(uniqueSounds).forEach(k => {
                    const partsArr: string[] = k.split(":");
                    const cents: number = partsArr[6] !== undefined ? parseFloat(partsArr[6]) : 0;
                    const parts = {
    ...
    _setPan(numTracks: number, panParam): any[] {
        if (panParam === null || panParam === undefined)
            return null;
        const panDistances = [];
        if (Array.isArray(panParam)) {
            for (let pp: number = 0; pp < numTracks; pp++) {
                if (pp < panParam.length) {
                    let x: number = parseFloat(panParam[pp]);
                    if (x < -1)
                        x = -1;
                    else if (x > 1)
                        x = 1;
            ...
            const panNumber: number = parseFloat(panParam);
            if (panNumber * (numTracks - 1) > 2)
                return null;
            const even: boolean = numTracks % 2 === 0;
            let currLow: number = even ? 0 - panNumber / 2 : 0;
            let currHigh: number = currLow + panNumber;
            for (let p: number = 0; p < numTracks; p++) {
                const isEvenTrack: boolean = p % 2 === 0;
                if (isEvenTrack) {
    ...
    seek(position, units): void {
        ...
        switch (units) {
            ...
            case "beats":
                ...
                break;
            default:
                offset = (this.duration - this.fadeLength / 1000) * position;
                break;
        }
        if (!this.audioBufferPossible)
            throw new Error(notSupportedMessage);
    ...
    stop() {
        ...
        this.pausedTimeSec = undefined;
        this.directSource.forEach(source => { try {
            source.stop();
        }
        catch (e) {
        ...
        this.directSource = [];
        const elapsed: number = activeAudioContext().currentTime - (this.startTimeSec || activeAudioContext().currentTime);
    ...
    download(): string { return downloadBuffer(this); }
    getAudioBuffer() { return this.audioBuffers[0]; }
    getIsRunning(): boolean { return this.isRunning; }
    _deviceCapable(): boolean {
        if (!supportsAudio()) {
    ...
    _kickOffSound(seconds): void {
        ...
        if (this.onEnded && this.directSource.length > 0) {
            this.directSource[0].onended = () => { if (this.onEnded)
                this.onEnded(this.callbackContext); };
        }
    }
    _addSwing(noteMapTracks: any[], swing, meterFraction, pickupLength: number): void {
        if (meterFraction.den != 4 && meterFraction.den != 8)
            return;
        let swingVal: number = parseFloat(swing);
        if (isNaN(swingVal) || swingVal <= 50)
            return;
        if (swingVal > 75)
            swingVal = 75;
        swingVal = swingVal / 50 - 1;
        const volumeIncrease: number = 0.0;
        let beatLength: number = 0.25;
        if (meterFraction.den === 8)
            beatLength = beatLength / 2;
        const halfbeatLength: number = beatLength / 2;
        const swingDuration: number = halfbeatLength * swingVal;
        for (let t: number = 0; t < noteMapTracks.length; t++) {
