...
export default class Repeats {
    voice: any;
    sections = [{ type: 'startRepeat', index: -1 }];

    constructor(voice) {
        this.voice = voice;
    }
    addBar(elem): void {
        var thisIndex: number = this.voice.length - 1;
        var isStartRepeat: boolean = elem.type === "bar_left_repeat" || elem.type === "bar_dbl_repeat";
        var isEndRepeat: boolean = elem.type === "bar_right_repeat" || elem.type === "bar_dbl_repeat";
        var startEnding: any[] = elem.startEnding ? this.startEndingNumbers(elem.startEnding) : undefined;
        if (isEndRepeat) {
    ...
    resolveRepeats(): any[] {
        ...
        var lastElement: number = this.voice.length - 1;
        ...
        for (var r: number = 0; r < repeatInstructions.length; r++) {
    ...
    duplicateSpan(input, output: any[], start, end): void {
        if (start < 0)
            start = 0;
    ...
    startEndingNumbers(startEnding): any[] {
        ...
        if (startEnding.indexOf(',') > 0) {
            ...
            endings = startEnding.split('-');
            var se: number = parseInt(endings[0], 10);
            var ee: number = parseInt(endings[1], 10);
            for (i = se; i <= ee; i++) {
                nums.push(i);
            }
        }
