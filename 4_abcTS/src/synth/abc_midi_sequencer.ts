...
export class MidiSequencer {
    measureLength = 1;
    PERCUSSION_PROGRAM = 128;

    constructor() {
    }
    sequence(abctune: Tune, options): any[] {
        options = options || {};
        let qpm;
        ...
        let channelExplicitlySet: boolean = false;
        let drumPattern = options.drum || "";
        let drumBars = options.drumBars || 1;
        let drumIntro = options.drumIntro || 0;
        let drumOn: boolean = drumPattern !== "";
        let drumOffAfterIntro: boolean = !!options.drumOff;
        let style = [];
        let rhythmHeadThisBar: boolean = false;
        let crescendoSize: number = 50;
        ...
        if (abctune.formatting.midi) {
            const globals: Midi = abctune.formatting.midi;
        ...
        for (let ss: number = 0; ss < startingMidi.length; ss++)
        ...
        let currentVolume;
        const repeats = [];
        let startingDrumSet: boolean = false;
        const lines: Lines[] = abctune.lines;
        for (let i: number = 0; i < lines.length; i++) {
            const line: Lines = lines[i];
            if (line.staff) {
                const staves: Staff[] = line.staff;
                let voiceNumber: number = 0;
                for (let j: number = 0; j < staves.length; j++) {
                    const staff: Staff = staves[j];
                    if (staff.clef && staff.clef.type === "TAB")
                        continue;
                    for (let k: number = 0; k < staff.voices.length; k++) {
                        const voice: Voice[] = staff.voices[k];
                        ...
                        if (staff.clef && staff.clef.type === 'perc' && !channelExplicitlySet) {
                            for (let cl: number = 0; cl < voices[voiceNumber].length; cl++) {
                        ...
                        let noteEventsInBar: number = 0;
                        let tripletMultiplier: number = 0;
                        let tripletDurationTotal: number = 0;
                        let tripletDurationCount: number = 0;
                        currentVolume = [105, 95, 85, 1];
                        for (let v: number = 0; v < voice.length; v++) {
                            const elem: Voice = voice[v];
                            switch (elem.el_type) {
                                case "note":
                                    ...
                                    if (!elem.rest || elem.rest.type !== 'spacer') {
                                        const noteElem = { elem: elem, el_type: "note", timing: durationCounter[voiceNumber] };
                                        if (elem.style)
                                        ...
                                        if (elem.startTriplet) {
                                            ...
                                            if (elem.startTriplet !== elem.tripletR) {
                                                if (v + elem.tripletR <= voice.length) {
                                                    ...
                                                    for (let w: number = v; w < v + elem.tripletR; w++) {
        ...
        for (let r: number = 0; r < repeats.length; r++)
            voices[r] = repeats[r].resolveRepeats();
        this.insertTempoChanges(voices, tempoChanges);
        if (drumIntro) {
            const pickups: number = abctune.getPickupLength();
            for (let vv: number = 0; vv < voices.length; vv++) {
                ...
                if (voices[vv].length > insertPoint) {
                    for (let w: number = 0; w < drumIntro; w++) {
                        if (pickups === 0 || w < drumIntro - 1) {
                            voices[vv].splice(insertPoint, 0, { el_type: "note", rest: { type: "rest" }, duration: this.measureLength }, { el_type: "bar" });
                            insertPoint += 2;
                        }
                        else {
    ...
    setDynamics(elem: Voice, crescendoSize: number, inCrescendo: any[], inDiminuendo: any[], voiceNumber: number, voice: Voice[], v: number, currentVolume, voices: any[]): void {
        const volumes = {
            'pppp': [15, 10, 5, 1],
            'ppp': [30, 20, 10, 1],
            'pp': [45, 35, 20, 1],
        ...
        if (elem.decoration) {
            ...
            if (dynamicType) {
                ...
                for (let i: number = 0; i < currentVolume.length; i++)
                    currentVolume[i] = activeVol[i];
                let volumesPerNotePitch: any[] = [currentVolume.slice(0)];
            ...
            if (elem.decoration.indexOf("crescendo(") >= 0) {
                const n: number = this.numNotesToDecoration(voice, v, "crescendo)");
                let top: number = Math.min(127, currentVolume[0] + crescendoSize);
                ...
                const n2: number = this.numNotesToDecoration(voice, v, "diminuendo)");
                let bottom: number = Math.max(15, currentVolume[0] - crescendoSize);
    ...
    numNotesToDecoration(voice: Voice[], start: number, decoration: string): number {
        let counter: number = 0;
        for (let i: number = start + 1; i < voice.length; i++) {
    ...
    endingVolume(voice: Voice[], start: number, volumeDecorations: string[]) {
        const end: number = Math.min(voice.length, start + 3);
        for (let i: number = start; i < end; i++) {
            if (voice[i].el_type === "note") {
                if (voice[i].decoration) {
                    for (let j: number = 0; j < voice[i].decoration.length; j++) {
    ...
    insertTempoChanges(voices: any[], tempoChanges): void {
        const changePositions: string[] = Object.keys(tempoChanges);
    ...
    chordVoiceOffThisBar(voices: any[]): void {
        for (let i: number = 0; i < voices.length; i++) {
            const voice = voices[i];
            let j: number = voice.length - 1;
    ...
    getTrackTitle(staff: Staff[], voiceNumber: number): string {
        if (!staff || staff.length <= voiceNumber || !staff[voiceNumber].title)
            return undefined;
        return staff[voiceNumber].title.join(" ");
    }
    interpretTempo(element: Tempo | Voice, beatLength: number): number {
        let duration: number = 1 / 4;
        if (element.duration) {
    ...
    removeNaturals(accidentals): any[] {
        const acc = [];
        for (let i: number = 0; i < accidentals.length; i++) {
            if (accidentals[i].acc !== "natural")
                acc.push(accidentals[i]);
        }
        return acc;
    }
    addKey(arr, key: KeySignature | Voice): void {
        let newKey;
        if (key.root === 'HP')
...
export default function sequence(abctune: Tune, options): any[] {
    const sequencer: MidiSequencer = new MidiSequencer();
