...
const modeNames: Array<string> = ['maj', 'ion', 'min', 'aeo', 'm', 'mix', 'dor', 'phr', 'lyd', 'loc'];
export function isLegalMode(mode): boolean {
    return modeNames.indexOf(mode.toLowerCase()) >= 0;
}
let keyReverse = null;
function createKeyReverse(): void {
    keyReverse = {};
    const allKeys: Array<string> = Object.keys(keys);
    for (let i: number = 0; i < allKeys.length; i++) {
        var keyObj = keys[allKeys[i]];
        keyReverse[allKeys[i].toLowerCase()] = allKeys[i];
        for (var j: number = 0; j < keyObj.modes.length; j++) {
            var mode: string = keyObj.modes[j].toLowerCase();
...
export function relativeMajor(key: string): string {
    ...
    var mode: Array<string> | Array<string | undefined> = key.toLowerCase().match(/([a-g][b#]?)(maj|ion|min|aeo|mix|dor|phr|lyd|loc|m)?/);
    if (!mode || !mode[2])
        return key;
    var modeStr: string = mode[1] + mode[2];
    var maj: string = keyReverse[modeStr];
    if (maj)
        return maj;
    return key;
}
export function relativeMode(majorKey: string, mode): string {
    ...
    if (!match)
        return majorKey;
    var regMode = match[1];
    for (var i: number = 0; i < group.modes.length; i++) {
...
export function transposeKey(key: string, steps): string {
    ...
    while (steps < 0)
        steps += 12;
    var fromC: number = (match.stepsFromC + steps) % 12;
    for (var i: number = 0; i < Object.keys(keys).length; i++) {
        var k: string = Object.keys(keys)[i];
