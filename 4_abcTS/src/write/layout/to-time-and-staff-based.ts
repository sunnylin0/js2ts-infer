function toTimeAndStaffBased(abcLines): any[] {
	var results = []
	for (var lin: number = 0; lin < abcLines.length; lin++) {
		var line = abcLines[lin]
		var staffGroup = line.staffGroup

		var group = []
		if (staffGroup && staffGroup && staffGroup.staffs) {
			for (var s: number = 0; s < staffGroup.staffs.length; s++) {
				var staff = staffGroup.staffs[s]
				var timeSlot: {} = {}
				for (var i: number = 0; i < staff.voices.length; i++) {
					var voice = staffGroup.voices[staff.voices[i]]
					var time: number = 0
					for (var k: number = 0; k < voice.children.length; k++) {
						var index: string = 'T' + Math.round(time*1000) // There can be inexactness when calculating triplets, so we'll round, but we'll make sure that no make sure that we don't lose necessary precision by making it a shorter time than would ever happen
						if (!timeSlot[index])
							timeSlot[index] = []
