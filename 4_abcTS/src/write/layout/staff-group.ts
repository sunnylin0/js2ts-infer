...
function checkLastBarX(voices: Array<VoiceElement>): void {
    let maxX: number = 0;
    for (let i: number = 0; i < voices.length; i++) {
        const curVoice: VoiceElement = voices[i];
        if (curVoice.children.length > 0) {
            const lastChild: number = curVoice.children.length - 1;
            const maxChild: AbsoluteElement = curVoice.children[lastChild];
            if (maxChild.abcelem.el_type === 'bar') {
                const barX: number = maxChild.children[0].x;
                if (barX > maxX) {
                    maxX = barX;
                }
                else {
...
const layoutStaffGroup = function (spacing: number, minPadding: number, debug: boolean, staffGroup: StaffGroupElement, leftEdge: number) {
    const epsilon: number = 0.0000001; // Fudging for inexactness of floating point math.
    let spacingunits: number = 0; // number of times we will have ended up using the spacing distance (as opposed to fixed width distances)
    let minspace: number = 1000; // a big number to start off with - used to find out what the smallest space between two notes is
    let x: number = leftEdge;
    staffGroup.startx = x;
    let i;
    ...
    let spacingunit: number = 0; // number of spacingunits coming from the previously laid out element to this one
    while (!finished(staffGroup.voices)) {
        ...
        const currentvoices: Array<any> = [];
        const othervoices: Array<any> = [];
        for (i = 0; i < staffGroup.voices.length; i++) {
            const durationIndex: number = getDurationIndex(staffGroup.voices[i]);
        ...
        let lastTopVoice = undefined;
        for (i = 0; i < currentvoices.length; i++) {
            const v: VoiceElement = currentvoices[i];
            if (v.voicenumber === 0)
                lastTopVoice = i;
            ...
            if (!isSameStaff(v, topVoice))
                topVoice = undefined;
            const voicechildx: number = layoutVoiceElements.layoutOneItem(x, spacing, v, minPadding, topVoice);
            const dx: number = voicechildx - x;
            if (dx > 0) {
                x = voicechildx; //update x
                for (let j: number = 0; j < i; j++) { // shift over all previously laid out elements
        ...
        for (i = 0; i < currentvoices.length; i++) {
            const voice: VoiceElement = currentvoices[i];
...
function finished(voices: Array<VoiceElement>): boolean {
    for (let i: number = 0; i < voices.length; i++) {
...
function getDurationIndex(element: VoiceElement): number {
    return element.durationindex - (element.children[element.i] && (element.children[element.i].duration > 0) ? 0 : 0.0000005);
}
function isSameStaff(voice1: VoiceElement, voice2: any): boolean {
