...
const layoutTriplet = function (element: TripletElem): void {
    if (element.anchor1 && element.anchor2) {
        ...
        if (element.hasBeam) {
            ...
            let max: number = 0;
            for (let i: number = 0; i < element.middleElems.length; i++) {
...
function isAbove(beam: BeamElem): boolean {
    return beam.stemsUp;
}
function heightAtMidpoint(startX: number, endX: number, beam: BeamElem): number {
    if (beam.beams.length === 0)
        return 0;
    const b: BeamElem = beam.beams[0];
    const midPoint: number = startX + (endX - startX) / 2;
    return getBarYAt(b.startX, b.startY, b.endX, b.endY, midPoint);
}
function xAtMidpoint(startX: number, endX: number): number {
