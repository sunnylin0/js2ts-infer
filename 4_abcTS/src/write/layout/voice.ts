...
const layoutVoice = function (voice: VoiceElement): void {
    for (let i: number = 0; i < voice.beams.length; i++) {
        if (voice.beams[i].type === 'BeamElem') {
            ...
            for (let j: number = 0; j < voice.beams[i].elems.length; j++) {
    ...
    for (let i: number = 0; i < voice.otherchildren.length; i++) {
        const child: string | TieElem = voice.otherchildren[i];
...
function moveDecorations(beam: BeamElem): void {
    const padding: number = 1.5; // Vertical padding in pitches
    for (let ch: number = 0; ch < beam.elems.length; ch++) {
        const child: AbsoluteElement = beam.elems[ch];
        if (child.top) {
            let top: number = yAtNote(child, beam);
            for (let i: number = 0; i < child.children.length; i++) {
                const el: RelativeElement = child.children[i];
                if (el.klass === 'ornament' && el.position !== 'below') {
                    if (el.bottom - padding < top) {
                        const distance: number = top - el.bottom + padding;
                        el.bottom += distance;
                        el.top += distance;
                        el.pitch += distance;
...
function placeInLane(rightMost: Array<number>, relElem: RelativeElement): void {
    const xCoords = relElem.getChordDim();
    if (xCoords) {
        for (let i: number = 0; i < rightMost.length; i++) {
...
function setLaneForChord(absElems: Array<AbsoluteElement>) {
    const rightMostAbove: Array<number> = [0];
    const rightMostBelow: Array<number> = [0];
    for (let i: number = 0; i < absElems.length; i++) {
        for (let j: number = 0; j < absElems[i].children.length; j++) {
            const relElem: RelativeElement | TempoElement = absElems[i].children[j];
            if (relElem.chordHeightAbove) {
                placeInLane(rightMostAbove, relElem);
            }
        }
        for (let j: number = absElems[i].children.length - 1; j >= 0; j--) {
            const relElem: RelativeElement | TempoElement = absElems[i].children[j];
...
function setLane(absElems: AbsoluteElement[], numLanesAbove: number): void {
    for (let i: number = 0; i < absElems.length; i++) {
        for (let j: number = 0; j < absElems[i].children.length; j++) {
            const relElem: RelativeElement = absElems[i].children[j];
            if (relElem.chordHeightAbove) {
...
function yAtNote(element: AbsoluteElement, beam: BeamElem): number {
    const b: BeamElem = beam.beams[0];
