...
function constructHLine(x1, y1, x2): string {
  const len: number = x2 - x1;
  return "M " + x1 + " " + y1 + " l " + len + " " + 0 + " l " + 0 + " " + 1 + " " + " l " + -len + " " + 0 + " " + " z ";
}
function constructVLine(x1, y1, y2): string {
  const len: number = y2 - y1;
  return "M " + x1 + " " + y1 + " l " + 0 + " " + len + " l " + 1 + " " + 0 + " " + " l " + 0 + " " + -len + " " + " z ";
}
class Svg {
  currentGroup: any[] = [];
  dummySvg: SVGSVGElement = null;
  svg: SVGSVGElement = createSvg();
  constructor(wrapper: HTMLElement) {
    wrapper.appendChild(this.svg);
  }
  clear(): void {
    if (this.svg) {
      const wrapper: HTMLDivElement = this.svg.parentNode as HTMLDivElement;
      this.svg = createSvg();
      this.currentGroup = [];
      if (wrapper) {
  ...
  setScale(scale: number): void {
    if (scale !== 1) {
      const s: string = "scale(" + scale + "," + scale + ")";
  ...
  insertStyles(styles: string): void {
    const el: SVGStyleElement = document.createElementNS(svgNS, "style") as SVGStyleElement;
    el.textContent = styles;
    this.svg.insertBefore(el, this.svg.firstChild); // prepend is not available on older browsers.
    //	this.svg.prepend(el);
  }
  setParentStyles(attr: any): void {
  ...
  dottedLine(attr): void {
    const el: SVGLineElement = document.createElementNS(svgNS, "line");
  ...
  rectBeneath(attr): void {
    const el: SVGRectElement = document.createElementNS(svgNS, "rect") as SVGRectElement;
  ...
  text(text: string, attr: any, target: any = null, spanAttr: any = null): SVGTextElement {
    const el: SVGTextElement = document.createElementNS(svgNS, "text") as SVGTextElement;
    ...
    const isFreeText: boolean = attr["data-name"] === "free-text";
    ...
    for (let i: number = 0; i < lines.length; i++) {
      ...
      const line: SVGTSpanElement = document.createElementNS(svgNS, "tspan") as SVGTSpanElement;
      if (spanAttr) {
      ...
      if (lines[i].indexOf("\x03") !== -1) {
        const parts: string[] = lines[i].split("\x03");
        line.textContent = parts[0];
        if (parts[1]) {
          const ts2: SVGTSpanElement = document.createElementNS(svgNS, "tspan") as SVGTSpanElement;
        ...
        if (parts[2]) {
          const dist: string = parts[1] ? "0.4em" : "0.1em";
          const ts3: SVGTSpanElement = document.createElementNS(svgNS, "tspan") as SVGTSpanElement;
  ...
  richTextLine(phrases, x, y, klass, anchor, target): SVGTextElement {
    const el: SVGTextElement = document.createElementNS(svgNS, "text") as SVGTextElement;
    el.setAttribute("stroke", "none");
    ...
    for (let i: number = 0; i < phrases.length; i++) {
      ...
      const tspan: SVGTSpanElement = document.createElementNS(svgNS, "tspan") as SVGTSpanElement;
      const attrs: string[] = Object.keys(phrase.attrs);
      for (let j: number = 0; j < attrs.length; j++) {
  ...
  guessWidth(text: string, attr) {
    const svg = this.createDummySvg();
    const el: SVGTextElement = this.text(text, attr, svg);
    let size;
    try {
  ...
  getTextSize(text: string | number, attr: any, el: SVGTextElement) {
    ...
    const removeLater: boolean = !el;
    if (!el)
      el = this.text(text, attr);
    let size;
    try {
      const bbox: SVGRect = el.getBBox();
      if (isNaN(bbox.height) || !bbox.height)
  ...
  openGroup(options: any): SVGGElement {
    ...
    const el: SVGGElement = document.createElementNS(svgNS, "g") as SVGGElement;
    if (options.klass)
  ...
  closeGroup(): SVGGElement {
    const g: SVGGElement = this.currentGroup.shift();
  ...
  path(attr: any): SVGPathElement {
    const el: SVGPathElement = document.createElementNS(svgNS, "path") as SVGPathElement;
    for (const key in attr) {
      if (Object.prototype.hasOwnProperty.call(attr, key)) {
        if (key === "path")
  ...
  lineToBack(attr): SVGLineElement {
    const el: SVGLineElement = document.createElementNS(svgNS, "line") as SVGLineElement;
  ...
  append(el: SVGTextElement | SVGGElement | SVGPathElement): void {
    if (this.currentGroup.length > 0)
      this.currentGroup[0].appendChild(el);
    else
      this.svg.appendChild(el);
  }
  prepend(el: SVGGElement | SVGPathElement): void {
  ...
  setAttributeOnElement(el: SVGGElement | SVGPathElement, attr: any): void {
    for (const key in attr) {
      if (Object.prototype.hasOwnProperty.call(attr, key)) {
        el.setAttributeNS(null, key, attr[key]);
      }
  ...
  moveElementToChild(parent, child): void {
