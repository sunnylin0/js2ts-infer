...
function drawCrescendo(renderer: Renderer, params, selectables: Selectables): any[] {
    ...
    selectables.wrapSvgEl({ el_type: "dynamicDecoration", startChar: -1, endChar: -1 }as Abcelem, el);
    return [el];
}
const drawCurveLine = function (renderer, y1, y2, y3, y4, left, right): SVGPathElement {
    y1 = roundNumber(y1);
    y2 = roundNumber(y2);
    y3 = roundNumber(y3);
    y4 = roundNumber(y4);
    ...
    const pathString: string = sprintf("M %f %f L %f %f M %f %f L %f %f", left, y1, right, y2, left, y3, right, y4);
