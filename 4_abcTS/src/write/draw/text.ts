...
function renderText(renderer: Renderer, params: any, alreadyInGroup: boolean): SVGTextElement {
    ...
    if (params.phrases) {
        ...
        const elem: SVGTextElement = renderer.paper.richTextLine(params.phrases, params.x, params.y, params.klass, params.anchor);
        return elem;
    }
    if (params.lane) {
        const laneMargin: number = params.dim.font.size * 0.25;
    ...
    let elem: SVGTextElement = renderer.paper.text(text, hash.attr);
    if (hash.font.box) {
        const size: DOMRect = elem.getBBox();
        let delta: number = 0;
