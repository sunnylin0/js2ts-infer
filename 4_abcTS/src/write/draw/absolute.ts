...
function drawAbsolute(renderer: Renderer, params: AbsoluteElement, bartop: number, selectables: Selectables, staffPos: any): void {
    if (params.invisible)
        return;
    const isTempo: boolean = params.children.length > 0 && params.children[0].type === "TempoElement";
    params.elemset = [];
    elementGroup.beginGroup(renderer.paper, renderer.controller);
    for (let i: number = 0; i < params.children.length; i++) {
        const child: RelativeElement | TempoElement = params.children[i];
        switch (child.type) {
            case "TempoElement":
                drawTempo(renderer, child as TempoElement);
                break;
            default:
                const el: SVGPathElement | SVGTextElement = drawRelativeElement(renderer, child, bartop);
                if (el && child.type === "symbol" && child.c && typeof child.c === 'string' && child.c.indexOf('notehead') >= 0) {
                    el.setAttribute('class', 'abcjs-notehead');
                }
        }
    }
    let klass: string = params.type;
    if (params.type === 'note' || params.type === 'rest') {
        ...
        if (params.abcelem.pitches) {
            for (let j: number = 0; j < params.abcelem.pitches.length; j++) {
                klass += ' p' + params.abcelem.pitches[j].pitch;
            }
        }
    }
    const g: SVGGElement = elementGroup.endGroup(klass, params.type);
    if (g) {
        if (params.cloned) {
            params.cloned.overrideClasses = g.className.baseVal;
        }
        if (params.overrideClasses) {
            const type: string = g.classList && g.classList.length > 0 ? g.classList[0] + ' ' : '';
    ...
    if (params.heads && params.heads.length > 0) {
        ...
        for (let jj: number = 0; jj < params.heads.length; jj++) {
