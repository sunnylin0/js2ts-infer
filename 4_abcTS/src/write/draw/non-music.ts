...
function nonMusic(renderer: Renderer, obj: TopText | BottomText, selectables: Selectables): void {
    if (!obj || !obj.rows)
        return;
    for (let i: number = 0; i < obj.rows.length; i++) {
        const row: Rows = obj.rows[i];
        if (row.absmove) {
            ...
            const x: number = row.left ? row.left : 0;
            const el: SVGTextElement = renderText(renderer, {
                x: x,
                y: renderer.y,
                text: row.text,
                phrases: row.phrases,
