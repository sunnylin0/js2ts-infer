...
function drawVoice(renderer: Renderer, params: VoiceElement, bartop: number, selectables: Selectables, staffPos: any): void {
    const width: number = params.w - 1;
    renderer.staffbottom = params.staff.bottom;
    const saveColor: string = renderer.foregroundColor;
    if (params.color)
        renderer.foregroundColor = params.color;
    if (params.header) { // print voice name
        const textEl: SVGTextElement = renderText(renderer, { x: renderer.padding.left, y: renderer.calcY(params.headerPosition), text: params.header, type: 'voicefont', klass: 'staff-extra voice-name', anchor: 'start', centerVertically: true, name: "voice-name" }, true);
    ...
    for (i = 0; i < params.children.length; i++) {
        ...
        if (child.type === 'note' || child.type === 'rest')
            foundNote = true;
        let justInitializedMeasureNumber: boolean = false;
    ...
    for (i = 0; i < params.beams.length; i++) {
        const beam: string | BeamElem = params.beams[i];
        if (beam === 'bar') {
            renderer.controller.classes.incrMeasure();
        }
        else
...
function isNonSpacerRest(elem: AbsoluteElement): boolean {
    if (elem.type !== 'rest')
        return false;
