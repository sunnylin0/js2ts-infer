...
function drawStaffGroup(renderer: Renderer, params: StaffGroupElement, selectables: Selectables, lineNumber: number): void {
    ...
    const startY: number = renderer.y; // So that it can be restored after we're done.
    // Set the absolute Y position for each staff here, so the voice drawing below can just use if.
    for (let j: number = 0; j < params.staffs.length; j++) {
        const staff1: Staffs = params.staffs[j];
        ...
        if (renderer.showDebug) {
            ...
            if (renderer.showDebug.indexOf("grid") >= 0) {
                ...
                colorIndex = 0;
                const debugPrintGridItem = (staff, key): void => {
                    const colors: string[] = ["rgb(207,27,36)", "rgb(168,214,80)", "rgb(110,161,224)", "rgb(191,119,218)", "rgb(195,30,151)",
                        "rgb(31,170,177)", "rgb(220,166,142)"];
                    if (staff.positionY && staff.positionY[key]) {
                        let height: number = staff.specialY[key] * spacing.STEP;
    ...
    let topLine; // these are to connect multiple staves. We need to remember where they are.
    let bottomLine: number = 0;
    let linePitch: number = 2;
    let bartop: number = 0;
    for (let i: number = 0; i < params.voices.length; i++) {
        const staff: Staff = params.voices[i].staff;
        ...
        if (!params.voices[i].duplicate) {
            ...
            if (staff.lines !== 0) {
                ...
                const lines: Array<number> = printStaff(renderer, params.startx, params.w, staff.lines, staff.linePitch, 0.35);
                bottomLine = lines[1];
    ...
    const staffSize: number = params.staffs.length;
    if (staffSize > 1) {
...
function printBrace(renderer: Renderer, absoluteY: number, brace: Array<BraceElem>, index: number, selectables: Selectables): void {
    if (brace) {
...
function boxAllElements(renderer: Renderer, voices: VoiceElement[], which: Voice[][]): void {
    for (let i: number = 0; i < which.length; i++) {
        ...
        for (let j: number = 0; j < children.length; j++) {
            ...
            if (elem.invisible || coords.t === undefined || coords.b === undefined)
                continue;
            const height: number = (coords.t - coords.b) * spacing.STEP;
            printDebugBox(renderer, {
                x: coords.x,
            ...
            for (let k: number = 0; k < elem.children.length; k++) {
