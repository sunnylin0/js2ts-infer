...
function printStem(renderer: Renderer, x: number, dx: number, y1: number, y2: number, klass: string, name: string): SVGPathElement {
    if (dx < 0 || y1 < y2) { // correct path "handedness" for intersection with other elements
        const tmp: number = roundNumber(y2);
        y2 = roundNumber(y1);
        y1 = tmp;
    }
    else {
    ...
    x = roundNumber(x);
    const x2: number = roundNumber(x + dx);
    ...
    const pathArray: Array<Array<string | number> | Array<string>> = [["M", x, y1], ["L", x, y2], ["L", x2, y2], ["L", x2, y1], ["z"]];
    const attr = { path: "" };
