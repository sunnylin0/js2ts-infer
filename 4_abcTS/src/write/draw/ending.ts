...
function drawEnding(renderer: Renderer, params: EndingElem, linestartx: number, lineendx: number, selectables: Selectables): SVGGElement[] {
    if (params.pitch === undefined)
        window.console.error("Ending Element y-coordinate not set.");
    const y: number = roundNumber(renderer.calcY(params.pitch));
