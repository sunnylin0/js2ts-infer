...
function printHorizontalLine(renderer, width, vertical, comment): void {
    const dy: number = 0.35;
    const fill: string = "rgba(0,0,255,.4)";
    let y = renderer.y;
    if (vertical)
        y = vertical;
    y = Math.round(y);
    ...
    const x1: number = 50;
    const x2 = width;
    let pathString: string = sprintf("M %f %f L %f %f L %f %f L %f %f z", x1, y - dy, x1 + x2, y - dy, x2, y + dy, x1, y + dy);
    renderer.paper.pathToBack({ path: pathString, stroke: "none", fill: fill, 'class': renderer.controller.classes.generate('staff') });
    for (let i: number = 1; i < width / 100; i++) {
