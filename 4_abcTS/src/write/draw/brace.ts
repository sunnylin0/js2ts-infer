...
function drawBrace(renderer: Renderer, params: BraceElem, selectables: Selectables): SVGPathElement {
    ...
    const startY: number = params.startVoice.staff.absoluteY - spacing.STEP * 10;
...
function straightPath(renderer: Renderer, xLeft: number, yTop: number, yBottom: number, type: string): SVGPathElement {
    xLeft += spacing.STEP;
    const xLineWidth: number = spacing.STEP * 0.75;
    const yOverlap: number = spacing.STEP * 0.75;
    const height: number = yBottom - yTop;
    // Straight line
    let pathString: string = sprintf("M %f %f l %f %f l %f %f l %f %f z", xLeft, yTop - yOverlap, // top left line
    0, height + yOverlap * 2, // bottom left line
    xLineWidth, 0, // bottom right line
    0, -(height + yOverlap * 2) // top right line
    );
    // Top arm
    const wCurve: number = spacing.STEP * 2;
    const hCurve: number = spacing.STEP;
...
function curvyPath(renderer: Renderer, xLeft: number, yTop: number, yBottom: number, type: string) {
    const yHeight: number = yBottom - yTop;
    let pathString: string = curve(xLeft, yTop, [7.5, -8, 21, 0, 18.5, -10.5, 7.5], [0, yHeight / 5.5, yHeight / 3.14, yHeight / 2, yHeight / 2.93, yHeight / 4.88, 0]);
...
const draw = (renderer: Renderer, xLeft: number, yTop: number, yBottom: number, type: string, header: any, selectables: Selectables) => {
    let ret;
    if (header) {
        renderer.paper.openGroup({ klass: renderer.controller.classes.generate("staff-extra voice-name"), "data-name": type });
        let position: number = yTop + (yBottom - yTop) / 2;
