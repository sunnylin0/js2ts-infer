...
class TopText {
    ...
    constructor(metaText: any, metaTextInfo: any, formatting: any, lines: Array<{ [key: string]: any }> | Array<any>, width: number, isPrint: boolean, paddingLeft: number, spacing: any, shouldAddClasses: boolean, getTextSize: GetTextSize) {
        ...
        const tLeft: number = formatting.titleleft ? paddingLeft : paddingLeft + width / 2;
        if (metaText.title) {
            const klass: string = shouldAddClasses ? 'abcjs-title' : '';
        ...
        if (lines.length) {
            ...
            while (index < lines.length && lines[index].subtitle) {
                const klass: string = shouldAddClasses ? 'abcjs-text abcjs-subtitle' : '';
        ...
        if (metaText.rhythm || metaText.origin || metaText.composer) {
            ...
            if (metaText.rhythm && metaText.rhythm.length > 0) {
                const noMove: boolean = !!(metaText.composer || metaText.origin);
                const klass: string = shouldAddClasses ? 'abcjs-rhythm' : '';
            ...
            let composerLine: string = metaText.composer ? metaText.composer : '';
            if (metaText.origin) {
            ...
            if (composerLine) {
                const klass: string = shouldAddClasses ? 'abcjs-composer' : '';
        ...
        if (metaText.author && metaText.author.length > 0) {
            const klass: string = shouldAddClasses ? 'abcjs-author' : '';
        ...
        if (metaText.partOrder && metaText.partOrder.length > 0) {
            const klass: string = shouldAddClasses ? 'abcjs-part-order' : '';
