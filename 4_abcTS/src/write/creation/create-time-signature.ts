...
const createTimeSignature = function (elem: any, tuneNumber: number): AbsoluteElement {
    elem.el_type = "timeSignature";
    const abselem: AbsoluteElement = new AbsoluteElement(elem, 0, 10, 'staff-extra time-signature', tuneNumber);
    if (elem.type === "specified") {
        let x: number = 0;
        for (let i: number = 0; i < elem.value.length; i++) {
            if (i !== 0) {
            ...
            if (elem.value[i].den) {
                ...
                for (let i2: number = 0; i2 < elem.value[i].num.length; i2++)
                    numWidth += glyphs.getSymbolWidth(elem.value[i].num[i2]);
                let denWidth: number = 0;
                for (let i2: number = 0; i2 < elem.value[i].den.length; i2++)
                    denWidth += glyphs.getSymbolWidth(elem.value[i].den[i2]);
                const maxWidth: number = Math.max(numWidth, denWidth);
                ...
                for (let i3: number = 0; i3 < elem.value[i].num.length; i3++)
