...
const createKeySignature: AbsoluteElement = function (elem: any, tuneNumber: number): AbsoluteElement {
    elem.el_type = "keySignature";
    if (!elem.accidentals || elem.accidentals.length === 0)
        return null;
    const abselem: AbsoluteElement = new AbsoluteElement(elem, 0, 10, 'staff-extra key-signature', tuneNumber);
