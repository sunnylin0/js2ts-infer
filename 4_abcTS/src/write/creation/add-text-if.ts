function addTextIf(rows: Array<any> | Array<{ [key: string]: any }>, params: any, getTextSize: GetTextSize): void {
    if (!params.text)
        return;
    ...
    const attr = { left: params.marginLeft, text: params.text, font: params.font, anchor: params.anchor, startChar: params.info.startChar, endChar: params.info.endChar, 'dominant-baseline': params['dominant-baseline'] };
    ...
    const size = getTextSize.calc("A", params.font, params.klass);
    let numLines: number = params.text.split("\n").length;
    ...
    if (!params.noMove) {
        const h: number = (size.height * 1.1) * numLines;
