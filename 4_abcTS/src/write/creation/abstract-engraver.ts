...
class AbstractEngraver {
    ...
    tiesbyvoiceSave = {};
    triplet: any = null;
    tempoSet: boolean = false;
    decoration: Decoration = new Decoration();

    constructor(getTextSize: GetTextSize, tuneNumber: number, options: any) {
    ...
    setStemHeight(heightInPixels: number): void {
        this.stemHeight = Math.round(heightInPixels * 10 / spacing.STEP) / 10;
    }
    getCurrentVoiceId(s: number, v: number): string {
        return "s" + s + "v" + v;
    }
    pushCrossLineElems(s: number, v: number): void {
        const id: string = this.getCurrentVoiceId(s, v);
    ...
    popCrossLineElems(s: number, v: number): void {
        const id: string = this.getCurrentVoiceId(s, v);
        this.slurs = this.slursbyvoice[id] || {};
        this.ties = this.tiesbyvoice[id] || [];
        this.partstartelem = this.endingsbyvoice[id];
        this.voiceScale = this.scaleByVoice[id];
        if (this.voiceScale === undefined)
            this.voiceScale = 1;
        this.voiceColor = this.colorByVoice[id];
    }
    containsLyrics(staves: Array<{ [key: string]: any }>): void {
        for (let i: number = 0; i < staves.length; i++) {
            for (let j: number = 0; j < staves[i].voices.length; j++) {
                for (let k: number = 0; k < staves[i].voices[j].length; k++) {
    ...
    createABCLine(staffs: Array<{ [key: string]: any }>, tempo: Tempo, l: number): StaffGroupElement {
        this.minY = 2; // PER: This will be the lowest that any note reaches. It will be used to set the dynamics row.
        this.containsLyrics(staffs);
        const staffgroup: StaffGroupElement = new StaffGroupElement(this.getTextSize);
        this.tempoSet = false;
        for (let s: number = 0; s < staffs.length; s++) {
            if (hint)
                this.restoreState();
            hint = false;
    ...
    createABCStaff(staffgroup: StaffGroupElement, abcstaff: any, tempo: Tempo, s: number, l: number): void {
        staffgroup.getTextSize.updateFonts(abcstaff);
        for (let v: number = 0; v < abcstaff.voices.length; v++) {
            const voice: VoiceElement = new VoiceElement(v, abcstaff.voices.length);
            if (v === 0) {
            ...
            const clef: AbsoluteElement = (!this.initialClef || l === 0) && createClef(abcstaff.clef, this.tuneNumber);
            if (clef) {
            ...
            const keySig: AbsoluteElement = createKeySignature(abcstaff.key, this.tuneNumber);
            if (keySig) {
            ...
            if (abcstaff.meter) {
                ...
                const ts: AbsoluteElement = createTimeSignature(abcstaff.meter, this.tuneNumber);
                voice.addChild(ts);
                this.startlimitelem = ts;
            }
            if (voice.duplicate)
                voice.children = [];
            const staffLines: number = abcstaff.clef.stafflines || abcstaff.clef.stafflines === 0 ? abcstaff.clef.stafflines : 5;
            staffgroup.addVoice(voice, s, staffLines);
            const isSingleLineStaff: boolean = staffLines === 1;
    ...
    createABCVoice(abcline: Array<{ [key: string]: any }>, tempo: Tempo, s: number, v: number, isSingleLineStaff: boolean, voice: VoiceElement): void {
        ...
        const voiceNumber: number = voice.voicetotal < 2 ? -1 : voice.voicenumber;
        ...
        for (let i: number = 0; i < this.ties.length; i++) {
        ...
        for (let j: number = 0; j < this.abcline.length; j++) {
            setAveragePitch(this.abcline[j]);
            this.minY = Math.min(this.abcline[j].minpitch, this.minY || 999);
        }
        const isFirstStaff: boolean = (s === 0);
        let pos: number = 0;
        while (pos < this.abcline.length) {
            const ret = getBeamGroup(this.abcline, pos);
            const abselems: Array<AbsoluteElement> = this.createABCElement(isFirstStaff, isSingleLineStaff, voice, ret.elem);
            if (abselems) {
                for (let i: number = 0; i < abselems.length; i++) {
                    if (!this.tempoSet && tempo && !tempo.suppress) {
                        this.tempoSet = true;
                        const tempoElement: AbsoluteElement = new AbsoluteElement(tempo, 0, 0, "tempo", this.tuneNumber, {});
    ...
    createABCElement(isFirstStaff: boolean, isSingleLineStaff: boolean, voice: VoiceElement, elem: Array<{ [key: string]: any }>): any[] {
        const elemset: Array<any> = [];
        switch (elem.el_type) {
            case undefined:
            ...
            case "clef":
                ...
                break;
            case "key":
                const absKey: AbsoluteElement = createKeySignature(elem, this.tuneNumber);
                if (absKey) {
                    elemset[0] = absKey;
                    this.startlimitelem = elemset[0];
                }
            ...
            case "stem":
                ...
                break;
            case "part":
                const abselem: AbsoluteElement = new AbsoluteElement(elem, 0, 0, 'part', this.tuneNumber);
                ...
                break;
            case "tempo":
                const abselem3: AbsoluteElement = new AbsoluteElement(elem, 0, 0, 'tempo', this.tuneNumber);
                if (!elem.suppress) {
            ...
            case "color":
                ...
                break;
            default:
                const abselem2: AbsoluteElement = new AbsoluteElement(elem, 0, 0, 'unsupported', this.tuneNumber);
                abselem2.addFixed(new RelativeElement("element type " + elem.el_type, 0, 0, undefined, { type: "debug" }));
                elemset[0] = abselem2;
        }
        return elemset;
    }
    createBeam(isSingleLineStaff: boolean, voice: VoiceElement, elems: Array<{ [key: string]: any }>): any[] {
        const abselemset: Array<any> = [];
        const beamelem: BeamElem = new BeamElem(this.stemHeight * this.voiceScale, this.stemdir, this.flatBeams, elems[0]);
        if (hint)
            beamelem.setHint();
        ...
        const tempStemDir: string = this.stemdir;
        this.stemdir = beamelem.stemsUp ? 'up' : 'down';
        for (let i: number = 0; i < elems.length; i++) {
            const elem = elems[i];
            const abselem: AbsoluteElement = this.createNote(elem, true, isSingleLineStaff, voice);
    ...
    createNote(elem: any, nostem: boolean, isSingleLineStaff: boolean, voice: VoiceElement): AbsoluteElement {
        let notehead = null;
        let roomtaken: number = 0;
        let roomtakenright: number = 0;
        let symbolWidth: number = 0;
        let additionalLedgers: Array<any> = [];
        let dir;
        let duration: number = getDuration(elem);
        let zeroDuration: boolean = false;
        if (duration === 0) {
            zeroDuration = true;
            duration = 0.25;
            nostem = true;
        }
        const durlog: number = Math.floor(Math.log(duration) / Math.log(2));
        let dot: number = 0;
        for (let tot: number = Math.pow(2, durlog), inc: number = tot / 2; tot < duration; dot++, tot += inc, inc /= 2)
            ;
        if (elem.startTriplet) {
            this.tripletmultiplier = elem.tripletMultiplier;
        }
        const durationForSpacing: number = (elem.rest && elem.rest.type === 'multimeasure') ? 1 :
            (elem.rest && elem.rest.type === 'invisible-multimeasure' ? this.measureLength * elem.rest.text :
                duration * this.tripletmultiplier);
        const absType: string = elem.rest ? "rest" : "note";
        const abselem: AbsoluteElement = new AbsoluteElement(elem, durationForSpacing, 1, absType, this.tuneNumber, { durationClassOveride: elem.duration * this.tripletmultiplier });
        if (hint)
            abselem.setHint();
        if (elem.rest) {
        ...
        if (elem.decoration) {
            const bottom: number = nostem && dir !== 'up' ? Math.min(-3, abselem.bottom - 6) : abselem.bottom;
    ...
    addNoteToAbcElement(abselem: AbsoluteElement, elem: any, dot: number, stemdir: string, style: any, zeroDuration: boolean, durlog: number, nostem: boolean, voice: VoiceElement) {
        let dotshiftx: number = 0;
        let noteHead;
        let roomTaken: number = 0;
        let roomTakenRight: number = 0;
        let min;
        let additionalLedgers: Array<any> = [];
        const accidentalSlot: Array<any> = [];
        let symbolWidth: number = 0;
        const dir: string = (elem.averagepitch >= 6) ? "down" : "up";
        const finalDir: string = stemdir || dir;
        style = elem.style ? elem.style : style;
        if (!style || style === "normal")
            style = "note";
        const noteSymbol: string = zeroDuration ? chartable[style].nostem : chartable[style][-durlog];
        for (let p: number = (finalDir === "down") ? elem.pitches.length - 2 : 1; (finalDir === "down") ? p >= 0 : p < elem.pitches.length; p = (finalDir === "down") ? p - 1 : p + 1) {
            const prev = elem.pitches[(finalDir === "down") ? p + 1 : p - 1];
            const curr = elem.pitches[p];
            const delta: number = (finalDir === "down") ? prev.pitch - curr.pitch : curr.pitch - prev.pitch;
        ...
        const pp: number = elem.pitches.length;
        for (let p: number = 0; p < elem.pitches.length; p++) {
            let flag = null;
            if (!nostem) {
            ...
            const isTopWhenStemIsDown: boolean = (stemdir === "up" || finalDir === "up") && p === 0;
            const isBottomWhenStemIsUp: boolean = (stemdir === "down" || finalDir === "down") && p === pp - 1;
            if (isTopWhenStemIsDown || isBottomWhenStemIsUp) {
                ...
                if (elem.startSlur) {
                    ...
                    for (let i: number = 0; i < elem.startSlur.length; i++) {
                ...
                if (elem.endSlur) {
                    ...
                    for (let i: number = 0; i < elem.endSlur.length; i++) {
                        addIfNotExist(elem.pitches[p].endSlur, elem.endSlur[i]);
                    }
                }
            }
            const hasStem: boolean = !nostem && durlog <= -1;
            const ret = createNoteHead(abselem, c, elem.pitches[p], { dir: finalDir, extrax: -roomTaken, flag: flag, dot: dot, dotshiftx: dotshiftx, scale: this.voiceScale, accidentalSlot: accidentalSlot, shouldExtendStem: !stemdir, printAccidentals: !voice.isPercussion });
        ...
        const hasStemOnChord: boolean = !nostem && durlog <= -1;
        if (hasStemOnChord) {
            const stemHeight: number = Math.round(70 * this.voiceScale) / 10;
            let p1: number = (finalDir === "down") ? elem.minpitch - stemHeight : elem.minpitch + 1 / 3;
            if (p1 > 6 && !stemdir)
                p1 = 6;
            let p2: number = (finalDir === "down") ? elem.maxpitch - 1 / 3 : elem.maxpitch + stemHeight;
            if (p2 < 6 && !stemdir)
                p2 = 6;
            const dx: number = (finalDir === "down" || abselem.heads.length === 0) ? 0 : abselem.heads[0].w;
            const width: number = (finalDir === "down") ? 1 : -1;
    ...
    addLyric(abselem: AbsoluteElement, elem: any): void {
        let lyricStr: string = "";
        elem.lyric.forEach((ly: any) => {
            const div: string = ly.divider === ' ' ? "" : ly.divider;
            lyricStr += ly.syllable + div + "\n";
        });
        const lyricDim = this.getTextSize.calc(lyricStr, 'vocalfont', "lyric");
        const position: string = elem.positioning ? elem.positioning.vocalPosition : 'below';
    ...
    addGraceNotes(elem: any, voice: VoiceElement, abselem: AbsoluteElement, notehead: RelativeElement, stemHeight: number, isBagpipes: boolean, roomtaken: number): number {
        const gracescale: number = 3 / 5;
        const graceScaleStem: number = 3.5 / 5;
        ...
        const graceoffsets: Array<any> = [];
        for (let i: number = elem.gracenotes.length - 1; i >= 0; i--) {
            roomtaken += 10;
        ...
        for (let i: number = 0; i < elem.gracenotes.length; i++) {
            const gracepitch: number = elem.gracenotes[i].verticalPos;
            const flag: string = (gracebeam) ? null : chartable.uflags[(isBagpipes) ? 5 : 3];
            const accidentalSlot: Array<any> = [];
            const ret = createNoteHead(abselem, "noteheads.quarter", elem.gracenotes[i], { dir: "up", headx: -graceoffsets[i], extrax: -graceoffsets[i], flag: flag, scale: gracescale * this.voiceScale, accidentalSlot: accidentalSlot });
            ret.notehead.highestVert = ret.notehead.pitch + stemHeight;
            const grace: RelativeElement = ret.notehead;
            ...
            if (elem.gracenotes[i].acciaccatura) {
                ...
                const dAcciaccatura: number = gracebeam ? 5 : 6;
                abselem.addRight(new RelativeElement("flags.ugrace", -graceoffsets[i] + dAcciaccatura, 0, pos, { scalex: gracescale, scaley: gracescale }));
            }
            if (gracebeam) {
                let graceDuration: number = elem.gracenotes[i].duration / 2;
                if (isBagpipes)
                    graceDuration /= 2;
                const pseudoabselem: RelativeElement = {
                    heads: [grace],
                ...
                const p1: number = gracepitch + 1 / 3 * gracescale;
                const p2: number = gracepitch + 7 * gracescale;
                const dx: number = grace.dx + grace.w;
            ...
            const isInvisibleRest: boolean = elem.rest && (elem.rest.type === "spacer" || elem.rest.type === "invisible");
    ...
    addSlursAndTies(abselem: AbsoluteElement, pitchelem: any, notehead: RelativeElement, voice: VoiceElement, dir: string, isGrace: boolean): void {
        if (pitchelem.endTie) {
            if (this.ties.length > 0) {
                let found: boolean = false;
                for (let j: number = 0; j < this.ties.length; j++) {
        ...
        if (pitchelem.startTie) {
            const tie: TieElem = new TieElem({ anchor1: notehead, force: (this.stemdir === "down" || this.stemdir === "up"), stemDir: this.stemdir, isGrace: isGrace, voiceNumber: voiceNumber, style: pitchelem.startTie.style });
            if (hint)
                tie.setHint();
            this.ties.push(tie);
            voice.addOther(tie);
        ...
        if (pitchelem.endSlur) {
            for (let i: number = 0; i < pitchelem.endSlur.length; i++) {
        ...
        if (pitchelem.startSlur) {
            for (let i: number = 0; i < pitchelem.startSlur.length; i++) {
    ...
    addMeasureNumber(number: number, abselem: AbsoluteElement): void {
        const measureNumDim = this.getTextSize.calc(number, "measurefont", 'bar-number');
        let dx: number = 0;
        if (abselem.isClef)
            dx += measureNumDim.width / 2;
        const vert: number = measureNumDim.width > 10 && abselem.abcelem.type === "treble" ? 13.5 : 11;
    ...
    createBarLine(voice: VoiceElement, elem: any, isFirstStaff: boolean): AbsoluteElement {
        const abselem: AbsoluteElement = new AbsoluteElement(elem, 0, 10, 'bar', this.tuneNumber);
        let anchor = null;
        let dx: number = 0;
        if (elem.barNumber) {
        ...
        if (elem.startEnding && isFirstStaff) {
            if (voice.voicenumber === 0) {
                const textWidth: number = this.getTextSize.calc(elem.startEnding, "repeatfont", '').width;
...
const getBeamGroup = (abcline: Array<{ [key: string]: any }>, pos: number) => {
...
const sortPitch = (elem: any): void => {
    ...
    do {
        sorted = true;
        for (let p: number = 0; p < elem.pitches.length - 1; p++) {
...
const ledgerLines: AbsoluteElement = (abselem: AbsoluteElement, minPitch: number, maxPitch: number, isRest: boolean, symbolWidth: number, additionalLedgers: Array<any>, dir: string | boolean, dx: number, scale: number): void => {
    for (let i: number = maxPitch; i > 11; i--) {
        if (i % 2 === 0 && !isRest) {
            abselem.addFixed(new RelativeElement(null, dx, (symbolWidth + 4) * scale, i, { type: "ledger" }));
        }
    }
    for (let i: number = minPitch; i < 1; i++) {
        if (i % 2 === 0 && !isRest) {
            abselem.addFixed(new RelativeElement(null, dx, (symbolWidth + 4) * scale, i, { type: "ledger" }));
        }
    }
    for (let i: number = 0; i < additionalLedgers.length; i++) {
        let ofs: number = symbolWidth;
        if (dir === 'down')
            ofs = -ofs;
        abselem.addFixed(new RelativeElement(null, ofs + dx, (symbolWidth + 4) * scale, additionalLedgers[i], { type: "ledger" }));
    }
};
const addRestToAbsElement: AbsoluteElement = (abselem: AbsoluteElement, elem: any, duration: number, dot: number, isMultiVoice: boolean, stemdir: any, isSingleLineStaff: boolean, durlog: number, voiceScale: number) => {
    let c: string = "";
    let restpitch: number = 7;
    let noteHead;
    let roomTaken: number = 0;
    let roomTakenRight: number = 0;
    if (isMultiVoice) {
    ...
    switch (elem.rest.type) {
        ...
        case "multimeasure":
            ...
            const numMeasures: RelativeElement = new RelativeElement("" + elem.rest.text, mmWidth, mmWidth, 16, { type: "multimeasure-text" });
            abselem.addExtra(numMeasures);
    }
    if (elem.rest.type.indexOf("multimeasure") < 0 && elem.rest.type !== "invisible") {
        const ret = createNoteHead(abselem, c, { verticalPos: restpitch }, { dot: dot, scale: voiceScale });
        noteHead = ret.notehead;
        if (noteHead) {
            abselem.addHead(noteHead);
            roomTaken = ret.accidentalshiftx;
...
const addIfNotExist = (arr, item): void => {
