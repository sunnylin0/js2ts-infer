...
class Decoration {
    startGlissandoX: any;
    dynamicPositioning: any;
    minTop: number = 12;
    minBottom: number = 0;
    startDiminuendoX = undefined;
    startCrescendoX = undefined;

    constructor() {
    }
    endLine(voice: VoiceElement): void {
    ...
    dynamicDecoration(voice: VoiceElement, decoration: Array<string>, abselem: AbsoluteElement, positioning: string): void {
        let diminuendo;
        let crescendo;
        let glissando;
    ...
    createDecoration(voice: VoiceElement, decoration: Array<string>, pitch: number, width: number, abselem: AbsoluteElement, roomtaken: number, dir: string, minPitch: number, positioning: any, hasVocals: boolean, accentAbove: boolean): void {
        if (!positioning)
...
function closeDecoration(voice: VoiceElement, decoration: Array<string>, pitch: number, width: number, abselem: AbsoluteElement, roomtaken: number, dir: string, minPitch: number, accentAbove: boolean) {
    let yPos;
    for (let i: number = 0; i < decoration.length; i++) {
        if (decoration[i] === "staccato" || decoration[i] === "tenuto" || (decoration[i] === "accent" && !accentAbove)) {
            let symbol: string = "scripts." + decoration[i];
        ...
        if (decoration[i] === "slide" && abselem.heads[0]) {
            let yPos2: number = abselem.heads[0].pitch;
            yPos2 -= 2;
            const blank1: RelativeElement = new RelativeElement("", -roomtaken - 15, 0, yPos2 - 1);
            const blank2: RelativeElement = new RelativeElement("", -roomtaken - 5, 0, yPos2 + 1);
...
function volumeDecoration(voice: VoiceElement, decoration: Array<string>, abselem: AbsoluteElement, positioning: string): void {
    for (let i: number = 0; i < decoration.length; i++) {
        switch (decoration[i]) {
            case "p":
            case "mp":
            ...
            case "sfz":
            case "mf":
                const elem: DynamicDecoration = new DynamicDecoration(abselem, decoration[i], positioning);
                voice.addOther(elem);
        }
    }
}
function compoundDecoration(decoration: Array<string>, pitch: number, width: number, abselem: AbsoluteElement, dir: string): void {
    function highestPitch() {
        if (abselem.heads.length === 0)
            return 10;
        let p: number = abselem.heads[0].pitch;
        for (let i: number = 1; i < abselem.heads.length; i++)
    ...
    function addCompound(symbol: string, count: number) {
        let placement: number = (dir === 'down') ? lowestPitch() + 1 : highestPitch() + 9;
        if (dir !== 'down' && count === 1)
            placement--;
        let deltaX: number = width / 2;
        deltaX += (dir === 'down') ? -5 : 3;
    ...
    for (let i: number = 0; i < decoration.length; i++) {
        switch (decoration[i]) {
            case "/":
                addCompound("flags.ugrace", 1);
                break;
            case "//":
                addCompound("flags.ugrace", 2);
                break;
            case "///":
                addCompound("flags.ugrace", 3);
                break;
            case "////":
                addCompound("flags.ugrace", 4);
                break;
        }
    }
}
function stackedDecoration(decoration: Array<string>, width: number, abselem: AbsoluteElement, yPos: any, positioning: string, minTop: number, minBottom: number, accentAbove: boolean): boolean {
    function incrementPlacement(placement: string, height: number) {
        if (placement === 'above')
            yPos.above += height;
        else
            yPos.below -= height;
    }
    function getPlacement(placement: string) {
        let y;
        if (placement === 'above') {
            y = yPos.above;
            if (y < minTop)
    ...
    function textDec(text: string, placement: string, anchor: string) {
        const y = getPlacement(placement);
        const textFudge: number = 2;
        const textHeight: number = 5;
    ...
    function symbolDec(symbol: string, placement: string) {
        let deltaX: number = width / 2;
        if (glyphs.getSymbolAlign(symbol) !== "center") {
            deltaX -= (glyphs.getSymbolWidth(symbol) / 2);
        }
        const height: number = glyphs.symbolHeightInPitches(symbol) + 1;
        let y: number = getPlacement(placement);
...
function leftDecoration(decoration: Array<string>, abselem: AbsoluteElement, roomtaken: number): void {
    for (let i: number = 0; i < decoration.length; i++) {
        switch (decoration[i]) {
            case "arpeggio":
                for (let j: number = abselem.abcelem.minpitch - 1; j <= abselem.abcelem.maxpitch; j += 2) {
...
function firstNote(els: AbsoluteElement[]): AbsoluteElement {
...
function lastNote(els: AbsoluteElement[]): AbsoluteElement {
