...
export function numberOfTunes(abc) {
    var tunes = abc.split("\nX:");
...
export class TuneBook {
    ...
    getTuneById(id) {
        for (var i: number = 0; i < this.tunes.length; i++) {
            if (this.tunes[i].id === '' + id)
                return this.tunes[i];
        }
        return null;
    }
    getTuneByTitle(title) {
        for (var i: number = 0; i < this.tunes.length; i++) {
...
export function renderEngine(callback: any, output: HTMLDivElement, abc: string, params: any): any[] {
    var ret: Array<any> = [];
    var isArray = function (testObject: HTMLDivElement): boolean {
    ...
    if (params === undefined)
        params = {};
    var currentTune: number = params.startingTune ? parseInt(params.startingTune, 10) : 0;
    // parse the abc string
    var book: TuneBook = new TuneBook(abc);
    var abcParser: Parse = new Parse();
    window.abcParse = abcParser;
    // output each tune, if it exists. Otherwise clear the div.
    for (var i: number = 0; i < output.length; i++) {
        var div: HTMLDivElement = output[i];
        if (div === "*") {
        ...
        if (div) {
            if (currentTune >= 0 && currentTune < book.tunes.length) {
                ...
                var warnings: Array<string> = abcParser.getWarnings();
                if (warnings)
...
export function extractMeasures(abc): any[] {
    var tunes = [];
    var book: TuneBook = new TuneBook(abc);
    for (var i: number = 0; i < book.tunes.length; i++) {
        var tune = book.tunes[i];
        var arr = tune.abc.split("K:");
        var arr2 = arr[1].split("\n");
        var header: string = arr[0] + "K:" + arr2[0] + "\n";
        var lastChord = null;
        ...
        var hasPickup: boolean = tuneObj.getPickupLength() > 0;
        for (var j: number = 0; j < tuneObj.lines.length; j++) {
            var line = tuneObj.lines[j];
            if (line.staff) {
                for (var k: number = 0; k < 1; k++) {
                    var staff = line.staff[k];
                    for (var kk: number = 0; kk < 1; kk++) {
                        var voice = staff.voices[kk];
                        for (var kkk: number = 0; kkk < voice.length; kkk++) {
                            var elem = voice[kkk];
