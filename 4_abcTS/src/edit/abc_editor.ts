...
export default class Editor {
    editarea: EditArea;
    abcjsParams: any;
    indicate_changed: boolean = false;
    div: HTMLDivElement = null;
    selectionChangeCallback = null;
    clientClickListener = null;
    synth = null;
    generate_midi = null;
    downloadMidi = null;
    inlineMidi = null;
    warningsdiv: HTMLDivElement = null;
    onchangeCallback = null;
    redrawCallback = null;
    currentAbc: string = "";
    tunes: Array<any> | Array<{ [key: string]: any }> = [];
    bReentry: boolean = false;
    timerId: number = null;
    bIsPaused: boolean = false;
    midiPause: boolean = false;
    wasDirty: boolean = false;
    warnings: Array<any> | Array<string> = [];

    constructor(editarea: string, params: any) {
        ...
        let canvasId: string = params.canvas_id || params.paper_id;
        if (canvasId) {
            this.div = typeof canvasId === 'string' ? document.getElementById(canvasId) : canvasId;
        }
        else {
            this.div = document.createElement("DIV");
            const el: HTMLTextAreaElement = this.editarea.getElem();
            if (el.parentNode)
    ...
    addClassName(element: HTMLTextAreaElement, className: string): HTMLTextAreaElement {
        const hasClassName = (el, name): boolean => {
    ...
    removeClassName(element: HTMLTextAreaElement, className: string): HTMLTextAreaElement {
        element.className = parseCommon.strip(element.className.replace(new RegExp("(^|\\s+)" + className + "(\\s+|$)"), ' '));
        return element;
    }
    setReadOnly(readOnly): void {
        const readonlyClass: string = 'abc_textarea_readonly';
        const el: HTMLTextAreaElement = this.editarea.getElem();
        if (readOnly) {
    ...
    redrawMidi(): void {
        if (this.generate_midi && !this.midiPause) {
            const event: CustomEvent = new CustomEvent("generateMidi", {
                detail: {
                    tunes: this.tunes,
        ...
        if (this.synth) {
            const userAction: boolean = !!this.synth.synthControl;
    ...
    paramChanged(engraverParams): void {
        if (engraverParams) {
    ...
    synthParamChanged(options): void {
        if (!this.synth)
            return;
    ...
    setDirtyStyle(isDirty: boolean): void {
        if (!this.indicate_changed)
            return;
        const dirtyClass: string = 'abc_textarea_dirty';
        const el: HTMLTextAreaElement = this.editarea.getElem();
        if (isDirty) {
    ...
    fireChanged(): void {
        ...
        if (this.parseABC()) {
            ...
            const isDirty: boolean = this.isDirty();
    ...
    highlight(abcelem, tuneNumber, classes, analysis, drag, mouseEvent): void {
