...
export default class Tune {
    ...
    media: string = "screen";
    metaText: MetaText = {} as MetaText;
    metaTextInfo: MetaTextInfo = {} as MetaTextInfo;
    formatting: Formatting = {} as Formatting;
    lines: Lines[] = [];
    staffNum: number = 0;
    voiceNum: number = 0;
    lineNum: number = 0;
    runningFonts = {};
    tempoLocations = {};
    engraver: EngraverController = {} as EngraverController;
    ...
    reset(): void {
        this.version = "1.1.0";
        this.media = "screen";
        this.metaText = {} as MetaText;
        this.metaTextInfo = {} as MetaTextInfo;
        this.formatting = {} as Formatting;
        this.lines = [];
        this.staffNum = 0;
        this.voiceNum = 0;
        this.lineNum = 0;
    ...
    ;
    copy(dest: Tune, src, prop: string, attrs: string[]): void {
        for (var i: number = 0; i < attrs.length; i++)
            dest[prop][attrs[i]] = src[prop][attrs[i]];
    }
    copyTopInfo(src): void {
        var attrs: string[] = ['tempo', 'title', 'header', 'rhythm', 'origin', 'composer', 'author', 'partOrder'];
        this.copy(this, src, "metaText", attrs);
        this.copy(this, src, "metaTextInfo", attrs);
    }
    ;
    copyBottomInfo(src): void {
        var attrs: string[] = ['unalignedWords',
            'book',
            'source',
            'discography',
            'notes',
    ...
    getBeatLength(): number {
        ...
        var meter: MeterValue = this.getMeterFraction();
    ...
    computePickupLength(lines: Lines[], barLength: number): number {
        var pickupLength: number = 0;
        for (var i: number = 0; i < lines.length; i++) {
            if (lines[i].staff) {
                for (var j: number = 0; j < lines[i].staff.length; j++) {
                    for (var v: number = 0; v < lines[i].staff[j].voices.length; v++) {
                        var voice: Voice[] = lines[i].staff[j].voices[v];
                        var tripletMultiplier: number = 1;
                        for (var el: number = 0; el < voice.length; el++) {
                            var isSpacer: boolean = voice[el].rest && voice[el].rest.type === "spacer";
    ...
    getPickupLength(): number {
        var barLength: number = this.getBarLength();
        var pickupLength: number = this.computePickupLength(this.lines, barLength);
    ...
    millisecondsPerMeasure(bpmOverride: number): number {
        ...
        if (bpmOverride) {
            ...
            var tempo: Tempo = this.metaText ? this.metaText.tempo : null;
            bpm = this.getBpm(tempo);
        }
        if (bpm <= 0)
            bpm = 1; // I don't think this can happen, but we don't want a possibility of dividing by zero.
        var beatsPerMeasure: number = this.getBeatsPerMeasure();
        var minutesPerMeasure: number = beatsPerMeasure / bpm;
        return minutesPerMeasure * 60000;
    }
    ;
    getBeatsPerMeasure(): number {
        var beatLen: number = this.getBeatLength();
        var barLen: number = this.getBarLength();
        return barLen / beatLen;
    }
    ;
    getMeter(): Meter {
        for (var i: number = 0; i < this.lines.length; i++) {
            var line: Lines = this.lines[i];
            if (line.staff) {
                for (var j: number = 0; j < line.staff.length; j++) {
                    var meter: Meter = line.staff[j].meter;
                    if (meter) {
                        return meter;
                    }
                }
    ...
    ;
    getElementFromChar(char): Voice {
        for (var i: number = 0; i < this.lines.length; i++) {
            var line: Lines = this.lines[i];
            if (line.staff) {
                for (var j: number = 0; j < line.staff.length; j++) {
                    var staff: Staff = line.staff[j];
                    for (var k: number = 0; k < staff.voices.length; k++) {
                        var voice: Voice[] = staff.voices[k];
                        for (var ii: number = 0; ii < voice.length; ii++) {
                            var elem: Voice = voice[ii];
    ...
    addVerticalInfo(timingEvents: any[]): void {
        ...
        var lastEventTop;
        var lastEventBottom;
        for (var e: number = timingEvents.length - 1; e >= 0; e--) {
    ...
    makeSortedArray(hash): any[] {
        ...
        arr = arr.sort(function (a, b) {
            var diff: number = a.milliseconds - b.milliseconds;
    ...
    addElementToEvents(eventHash, element, voiceTimeMilliseconds: number, top, height, line, measureNumber, timeDivider: number, isTiedState, nextIsBar: boolean) {
        if (element.hint)
        ...
        if (realDuration > 0) {
            ...
            for (var i: number = 0; i < element.elemset.length; i++) {
            ...
            if (isTiedState !== undefined) {
                ...
                if (!eventHash["event" + voiceTimeMilliseconds]) {
                    ...
                    if (element.abcelem.midiPitches && element.abcelem.midiPitches.length) {
                        ...
                        for (var i: number = 0; i < element.abcelem.midiPitches.length; i++)
                    ...
                    if (element.abcelem.midiGraceNotePitches && element.abcelem.midiGraceNotePitches.length) {
                        ...
                        for (var j: number = 0; j < element.abcelem.midiGraceNotePitches.length; j++)
    ...
    makeVoicesArray(): any[] {
        ...
        var measureNumber = [];
        var tempos: {} = {};
        for (var line: number = 0; line < this.engraver.staffgroups.length; line++) {
            var group: StaffGroupElement = this.engraver.staffgroups[line];
            if (group && group.staffs && group.staffs.length > 0) {
                var firstStaff: Staffs = group.staffs[0];
                var middleC: number = firstStaff.absoluteY;
                var top: number = middleC - firstStaff.top * spacing.STEP;
                var lastStaff: Staffs = group.staffs[group.staffs.length - 1];
                middleC = lastStaff.absoluteY;
                var bottom: number = middleC - lastStaff.bottom * spacing.STEP;
                var height: number = bottom - top;
                var voices: VoiceElement[] = group.voices;
                for (var v: number = 0; v < voices.length; v++) {
                    if (voices[v].staff && voices[v].staff.isTabStaff)
                        continue;
                    var noteFound: boolean = false;
                    if (!voicesArr[v])
                        voicesArr[v] = [];
                    if (measureNumber[v] === undefined)
                        measureNumber[v] = 0;
                    var elements: AbsoluteElement[] = voices[v].children;
                    for (var elem: number = 0; elem < elements.length; elem++) {
    ...
    ;
    setupEvents(startingDelay: number, timeDivider: number, bpm, warp: number): any[] {
        if (!warp)
            warp = 1;
        var timingEvents = [];
        var eventHash: {} = {};
        ...
        var time: number = startingDelay;
        var isTiedState;
        var nextIsBar: boolean = true;
        var voices: any[] = this.makeVoicesArray();
        var maxVoiceTimeMilliseconds: number = 0;
        for (var v: number = 0; v < voices.length; v++) {
            var voiceTime: number = time;
            var voiceTimeMilliseconds: number = Math.round(voiceTime * 1000);
            var startingRepeatElem: number = 0;
            var endingRepeatElem: number = -1;
            var elements = voices[v];
            bpm = bpm; // This is a bit redundant but fixes the 'startingBpm' reference if bpm was rename
            timeDivider = this.getBeatLength() * bpm / 60;
            var tempoDone: number = -1;
            for (var elem: number = 0; elem < elements.length; elem++) {
                ...
                if (element.type === 'bar') {
                    ...
                    var endRepeat: boolean = (barType === "bar_right_repeat" || barType === "bar_dbl_repeat");
                    var startEnding: boolean = (element.abcelem.startEnding === '1');
                    var startRepeat: boolean = (barType === "bar_left_repeat" || barType === "bar_dbl_repeat" || barType === "bar_right_repeat");
                    if (endRepeat) {
                        ...
                        var lastVoiceTimeMilliseconds: number = 0;
                        tempoDone = -1;
                        for (var el2: number = startingRepeatElem; el2 < endingRepeatElem; el2++) {
        ...
        timingEvents = this.makeSortedArray(eventHash);
        this.addVerticalInfo(timingEvents);
        this.addEndPoints(this.lines, timingEvents);
        timingEvents.push({ type: "end", milliseconds: maxVoiceTimeMilliseconds });
        this.addUsefulCallbackInfo(timingEvents, bpm * warp);
        return timingEvents;
    }
    ;
    addUsefulCallbackInfo(timingEvents: any[], bpm: number): void {
        var millisecondsPerMeasure: number = this.millisecondsPerMeasure(bpm);
        for (var i: number = 0; i < timingEvents.length; i++) {
            var ev = timingEvents[i];
            ev.millisecondsPerMeasure = millisecondsPerMeasure;
        }
    }
    ;
    skipTies(elements: any[], index: number) {
        while (index < elements.length && elements[index].left === null)
            index++;
        return elements[index];
    }
    addEndPoints(lines: Lines[], elements: any[]): void {
        if (elements.length < 1)
            return;
        for (var i: number = 0; i < elements.length - 1; i++) {
            var el = elements[i];
            var next = this.skipTies(elements, i + 1);
            if (el.left !== null) {
    ...
    getBpm(tempo: Tempo | Abcelem) {
        var bpm;
        if (!tempo)
            tempo = this.metaText ? this.metaText.tempo : null;
        if (tempo) {
            bpm = tempo.bpm;
            var beatLength: number = this.getBeatLength();
    ...
    ;
    setTiming(bpm, measuresOfDelay): any[] {
        ...
        var beatLength: number = this.getBeatLength();
        var beatsPerSecond: number = bpm / 60;
        var measureLength: number = this.getBarLength();
        var startingDelay: number = measureLength / beatLength * measuresOfDelay / beatsPerSecond;
        if (startingDelay)
            startingDelay -= this.getPickupLength() / beatLength / beatsPerSecond;
        var timeDivider: number = beatLength * beatsPerSecond;
    ...
    setUpAudio(options) {
        if (!options)
            options = {};
        var seq: any[] = sequence(this, options);
