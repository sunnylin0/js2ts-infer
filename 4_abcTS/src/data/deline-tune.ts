function delineTune(inputLines: Lines[], options): any[] {
    if (!options)
        options = {};
    var lineBreaks: boolean = !!options.lineBreaks;
    var outputLines = [];
    var inMusicLine: boolean = false;
    var currentMeter = [];
    var currentKey = [];
    var currentClef = [];
    ...
    for (var i: number = 0; i < inputLines.length; i++) {
        var inputLine: Lines = cloneLine(inputLines[i]);
        if (inputLine.staff) {
            if (inMusicLine && !inputLine.vskip) {
                ...
                for (var s: number = 0; s < outputLine.staff.length; s++) {
                    ...
                    if (inputStaff) {
                        for (var v: number = 0; v < outputStaff.voices.length; v++) {
                ...
                for (var ii: number = 0; ii < inputLine.staff.length; ii++) {
...
// 	}
// }
function replacer(key, value): string {
...
function addMeterToVoices(meter, voices): void {
    meter.el_type = "meter";
    meter.startChar = -1;
    meter.endChar = -1;
...
function addKeyToVoices(key, voices): void {
    key.el_type = "key";
    key.startChar = -1;
    key.endChar = -1;
    for (var i: number = 0; i < voices.length; i++) {
...
function addClefToVoices(clef, voices): void {
    clef.el_type = "clef";
    clef.startChar = -1;
    clef.endChar = -1;
    for (var i: number = 0; i < voices.length; i++) {
...
function addFontToVoices(font, voices, type: string): void {
    font.el_type = "font";
    font.type = type;
    font.startChar = -1;
    font.endChar = -1;
...
function objEqual(input, output): boolean {
    if (!input)
        return true; // the default is whatever the old output is.
    var inputValue: string = JSON.stringify(input, replacer);
    var outputValue: string = JSON.stringify(output, replacer);
    return inputValue === outputValue;
}
function cloneLine(line: Lines): Lines {
    var output:Lines = {} as Lines;
    var keys: string[] = Object.keys(line);
    for (var i: number = 0; i < keys.length; i++) {
        if (keys[i] !== "staff")
            ...
            for (var j: number = 0; j < line.staff.length; j++) {
                var staff: Staff = {} as Staff;
                var keys2: string[] = Object.keys(line.staff[j]);
                for (var k: number = 0; k < keys2.length; k++) {
                    if (keys2[k] !== "voices")
                        staff[keys2[k]] = line.staff[j][keys2[k]];
                    else {
                        staff.voices = [];
                        for (var v: number = 0; v < line.staff[j].voices.length; v++) {
