...
const letters: string = "CDEFGAB";
// const octaves = [",,,,", ",,,", ",,", ",", "", "'", "''", "'''", "''''"];
export default function strTranspose(abc, abcTune, steps): keyAccidentals {
    if (abcTune === "TEST") {
    ...
    let changes = [];
    const tunes: any[] = Array.isArray(abcTune) ? abcTune : [abcTune];
    for (let i: number = 0; i < tunes.length; i++) {
...
function changeAllKeySigs(abc, steps): any[] {
    const changes = [];
...
function transposeVoices(abc, voices, key, steps): any[] {
    let changes = [];
...
function setLetterDistance(destinationKey, keyRoot, steps): number {
    let letterDistance: number = letters.indexOf(destinationKey.root) - letters.indexOf(keyRoot);
    if (keyRoot === "none")
...
function transposeVoice(abc, voice, keyRoot, keyAccidentalsMap, destinationKey, steps): any[] {
    let changes = [];
    let letterDistance: number = setLetterDistance(destinationKey, keyRoot, steps);
    let measureAccidentals: {} = {};
    let transposedMeasureAccidentals: {} = {};
    for (let i: number = 0; i < voice.length; i++) {
        const el = voice[i];
        if (el.chord) {
            for (let c: number = 0; c < el.chord.length; c++) {
                const ch = el.chord[c];
                if (ch.position === 'default') {
                    const prefersFlats: boolean = destinationKey.accidentals.length && destinationKey.accidentals[0].acc === 'flat';
                    let newChord: string = transposeChordName(ch.name, steps, prefersFlats, true);
        ...
        if (el.el_type === 'note' && el.pitches) {
            const pitchArray: any[] = findNotes(abc, el.startChar, el.endChar);
            for (let j: number = 0; j < pitchArray.length; j++) {
...
function getNewKey(key, steps) {
    if (key.root === "none") {
        return { root: transposeKey("C", steps), mode: "", acc: "", accidentals: [] };
    }
    const major: string = relativeMajor(key.root + key.acc + key.mode);
    const newMajor: string = transposeKey(major, steps);
...
function transposePitch(note, key, letterDistance: number, measureAccidentals) {
    const pitch = note.pitch;
    const origDistFromC: number = letters.indexOf(note.name);
    const root: number = letters.indexOf(key.root);
    const index: number = (root + pitch) % 7;
    let newDistFromC: number = origDistFromC + letterDistance;
    let oct = note.oct;
...
const regPitch: RegExp = /([_^=]*)([A-Ga-g])([,']*)/;
const regOptionalNote: RegExp = /([_^=]*[A-Ga-g][,']*)?(\d*\/*\d*)?([\>\<\-\)]*)?/;
function parseNote(note, keyRoot, keyAccidentalsMap, measureAccidentals) {
    const root: number = keyRoot === "none" ? 0 : letters.indexOf(keyRoot);
    const match = note.match(regPitch);
    if (!match)
        return null;
    const name = match[2].toUpperCase();
    let pos: number = letters.indexOf(name) - root;
    if (pos < 0)
        pos += 7;
    const octavesArr: string[] = [",,,,", ",,,", ",,", ",", "", "'", "''", "'''", "''''"];
    let oct: number = octavesArr.indexOf(match[3]);
    if (name === match[2])
        oct--;
...
function replaceGrace(abc, start, end, newGrace: string, index: number) {
    const noteStr = abc.substring(start, end);
    const regOpenBrace: RegExp = /\{/;
    const regCloseBrace: RegExp = /\}/;
    const regPreBrace: RegExp = /([^\{]*)/;
    const regPreNote: RegExp = /(\/*)/;
    const regex: RegExp = new RegExp(regPreBrace.source + regOpenBrace.source + regPreNote.source + regOptionalNote.source +
...
function replaceChord(abc, start, end, newChord: string) {
    const match = abc.substring(start, end).match(/([^"]+)?(".+")+/);
    if (match) {
        let finalStart = start;
        if (match[1])
            finalStart += match[1].length;
...
function calcAdjustment(thisAccidental, keyAccidental, measureAccidental): number {
