...
const baseDuration: number = 480 * 4; // nice and divisible, equals 1 whole note
export default function create(abcTune, options = {}) {
    const commands = abcTune.setUpAudio(options);
    const midi: Midi = rendererFactory();
    ...
    let beatsPerSecond: number = tempo / 60;
    ...
    for (let i: number = 0; i < commands.tracks.length; i++) {
        midi.startTrack();
        const notePlacement: {} = {};
        for (let j: number = 0; j < commands.tracks[i].length; j++) {
            ...
            switch (event.cmd) {
                ...
                case 'program':
                    ...
                    break;
                case 'note':
                    const gapLengthInBeats: number = event.gap * beatsPerSecond;
                    const start = event.start;
                    const end: number = start + event.duration - gapLengthInBeats;
...
function addNotes(midi: Midi, notePlacement, baseDuration: number): void {
    const times: number[] = Object.keys(notePlacement).map(t => parseFloat(t));
    times.sort((a, b) => a - b);
    let lastTime: number = 0;
    for (let i: number = 0; i < times.length; i++) {
        const events = notePlacement[times[i]];
        if (times[i] > lastTime) {
            const distance: number = (times[i] - lastTime) * baseDuration;
