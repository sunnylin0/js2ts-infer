...
var MusicParser = function (_tokenizer, _warn, _multilineVars, _tune, _tuneBuilder, _header): void {
...
var isInTie = function (multilineVars, overlayLevel, el): boolean {
                            ...
                            if (i < line.length && line[i] === ']') {
                                ...
                                while (i < line.length && !postChordDone) {
                                    switch (line[i]) {
                                        ...
                                        case '>':
                                        case '<':
                                            var br2: number[] = getBrokenRhythm(line, i);
...
var setIsInTie = function (multilineVars, overlayLevel, value): void {
    // If this is single voice music then the voice index isn't set, so we use the first voice.
    var voiceIndex = multilineVars.currentVoice ? multilineVars.currentVoice.staffNum * 100 + multilineVars.currentVoice.index : 0;
    if (multilineVars.inTie[overlayLevel] === undefined)
...
var letter_to_chord = function (line, i): (string | number)[] {
    if (line[i] === '"') {
...
var letter_to_grace = function (line, i): any[] {
...
function letter_to_overlay(line, i: number): any[] {
    if (line[i] === '&') {
        var start: number = i;
...
function durationOfMeasure(multilineVars): number {
...
var letter_to_accent = function (line, i): any[] | (string | number)[] | boolean | number[] {
...
var letter_to_spacer = function (line, i): number[] {
    var start = i;
...
var letter_to_bar = function (line, curr_pos): any[] {
...
MusicParser.prototype.startNewLine = function () {
    ...
    if (multilineVars.key.explicitAccidentals) {
        for (var i: number = 0; i < multilineVars.key.explicitAccidentals.length; i++) {
    ...
    if (multilineVars.currentVoice) {
        ...
        var voices: string[] = Object.keys(multilineVars.voices);
...
var getCoreNote = function (line, index, el, canHaveBrokenRhythm): boolean {
    //var el = { startChar: index };
    var isComplete = function (state): boolean {
            ...
            case '<':
                if (isComplete(state)) {
                    if (canHaveBrokenRhythm) {
                        var br2: number[] = getBrokenRhythm(line, index);
...
var getBrokenRhythm = function (line, index): number[] {
    switch (line[index]) {
        case '>':
