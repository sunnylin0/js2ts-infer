...
export default function chordGrid(visualObj: Tune): any[] {
    const meter: MeterValue = visualObj.getMeterFraction();
    const isCommonTime: boolean = meter.num === 4 && meter.den === 4;
    const isCutTime: boolean = meter.num === 2 && meter.den === 2;
    if (!isCutTime && !isCommonTime)
        throw new Error("notCommonTime");
    const deline = visualObj.deline();
    let chartLines = [];
    let nonSubtitle: boolean = false;
    deline.forEach((section) => {
        if (section.subtitle) {
            ...
            const parts: any[] = flattenVoices(staves);
...
const breakSynonyms: Array<string> = ['break', '(break)', 'no chord', 'n.c.', 'tacet'];
function flattenVoices(staves): any[] {
    const parts = [];
    let partName: string = "";
    let measures = [];
    let currentBar = { chord: ['', '', '', ''] };
    let lastChord: string = "";
    let nextBarEnding: string = "";
    staves.forEach((staff, staffNum) => {
        if (staff.voices) {
            staff.voices.forEach((voice, voiceNum) => {
                let beatNum: number = 0;
                let measureNum: number = 0;
                voice.forEach(element => {
                    if (element.el_type === 'part') {
                        ...
                        const intBeat: number = Math.floor(beatNum);
                        ...
                        if (!element.rest || element.rest.type !== 'spacer') {
                            ...
                            const thisDuration: number = Math.floor(dur * 4);
                            if (thisDuration > 4) {
                                measureNum += Math.floor(thisDuration / 4);
                                beatNum = 0;
                            }
                            else {
                                let thisBeat: number = dur * 4;
...
function findLastChord(measures: any[]) {
    for (let m: number = measures.length - 1; m >= 0; m--) {
        for (let c: number = measures[m].chord.length - 1; c >= 0; c--) {
        ...
        if (line.type === "part") {
            ...
            if (ending1 >= 0 && ending2 >= 0) {
                if (ending2 - ending1 === partLine.length - ending2) {
                    ...
                    for (let i: number = 0; i < ending2 - ending1 && matches; i++) {
                        ...
                        if (measureLhs.annotations && measureRhs.annotations) {
                            if (measureLhs.annotations.length !== measureRhs.annotations.length)
                                ...
                                for (let j: number = 0; j < measureLhs.annotations.length; j++) {
...
function addLineBreaks(chartLines: any[]): void {
    chartLines.forEach(line => {
        if (line.type === "part") {
            ...
            if (length === 12)
                is12bar = true;
            const barsPerLine: number = is12bar ? 4 : 8;
            for (let i: number = 0; i < oldLines.length; i += barsPerLine) {
            ...
            for (let i: number = 0; i < newLines.length; i++) {
                if (newLines[i][0].ending) {
                    const prevLine: number = Math.max(0, i - 1);
                    const toAdd: number = newLines[prevLine].length - newLines[i].length;
                    const thisLine = [];
                    for (let j: number = 0; j < toAdd; j++)
...
function addPercents(chartLines: any[]): void {
    chartLines.forEach(part => {
        if (part.lines) {
            let lastMeasureSingle: boolean = false;
            let lastChord: string = "";
...
function addDecoration(element, currentBar): void {
    if (element.decoration) {
        for (let i: number = 0; i < element.decoration.length; i++) {
