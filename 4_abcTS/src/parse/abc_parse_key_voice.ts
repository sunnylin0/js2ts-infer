...
function calcMiddle(clef: string, oct: number): number {
    ...
    const mid: number = value ? value.mid : 0;
    return mid + oct;
}
...
function normalizeAccidentals(accs): void {
...
const parseKeyVoice = {
    ...
    addPosToKey: function (clef: any, key: any) {
        ...
        key.accidentals.forEach(function (acc) {
            let pitch: number = pitches[acc.note];
            pitch = pitch - mid;
    ...
    parseKey: function (str: string, isInline: boolean) {
        ...
        switch (tokens[0].token) {
            ...
            default:
                ...
                if (retPitch.len > 0) {
                    ...
                    const keyCompensate: number = !isInline && multilineVars.globalTranspose ? -multilineVars.globalTranspose : 0;
                    let savedOrigKey;
                    if (isInline)
                    ...
                    if (oldKey && multilineVars.keywarn !== false) {
                        let kk;
                        for (let k: number = 0; k < multilineVars.key.accidentals.length; k++) {
        ...
        if (accs.accs) {
            ...
            for (let i: number = 0; i < accs.accs.length; i++) {
                let found: boolean = false;
                for (let j: number = 0; j < multilineVars.key.accidentals.length && !found; j++) {
                ...
                if (!found) {
                    ...
                    if (multilineVars.key.impliedNaturals) {
                        for (let kkk: number = 0; kkk < multilineVars.key.impliedNaturals.length; kkk++) {
        ...
        while (tokens.length > 0) {
            switch (tokens[0].token) {
                ...
                case "middle":
                    ...
                    const pitch: number = tokenizer.getPitchFromTokens(tokens);
                    if (pitch.warn)
    ...
    parseVoice: function (line, i, e) {
        ...
        const addNextTokenToStaffInfo = function (name): void {
        ...
        const addNextTokenToVoiceInfo = function (vid, name, type): void {
            const attr = tokenizer.getVoiceToken(line, start, end);
            if (attr.warn !== undefined)
                warn("Expected value for " + name + " in voice: " + attr.warn, line, start);
            else if (attr.err !== undefined)
        ...
        const addNextNoteTokenToVoiceInfo = function (vid, name): void {
        ...
        if (multilineVars.staves) {
            for (let i: number = 0; i < multilineVars.staves.length; i++) {
        ...
        if (currentVoice) {
            if (currentVoice.index === undefined) {
                ...
                const staffNum: number = multilineVars.staves.length - 1;
                const voiceNum: number = multilineVars.staves[staffNum].voices.length - 1;
