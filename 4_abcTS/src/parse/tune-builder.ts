...
export default class TuneBuilder {
    ...
    setVisualTranspose(visualTranspose): void {
        if (visualTranspose !== undefined)
            this.tune.visualTranspose = visualTranspose;
    }
    cleanUp(barsperstaff: number, staffnonote: any, currSlur: Array<any>): any[] {
        closeLine(this.tune);
        ...
        let hadOverlays: boolean = false;
        while (resolveOverlays(this.tune)) {
            hadOverlays = true;
        }
        if (hadOverlays) {
            let voiceNum: number = 0;
            let isUseful: boolean | string = voiceUseful(this.tune.lines, voiceNum);
        ...
        for (let i: number = 0; i < this.tune.lines.length; i++) {
            const staff: Array<{ [key: string]: any }> = this.tune.lines[i].staff;
            if (staff) {
                for (this.tune.staffNum = 0; this.tune.staffNum < staff.length; this.tune.staffNum++) {
                    ...
                    for (this.tune.voiceNum = 0; this.tune.voiceNum < staff[this.tune.staffNum].voices.length; this.tune.voiceNum++) {
                        const voice: Array<{ [key: string]: any }> = staff[this.tune.staffNum].voices[this.tune.voiceNum];
                        cleanUpSlursInLine(voice, this.tune.staffNum, this.tune.voiceNum, currSlur);
                        for (let j: number = 0; j < voice.length; j++) {
    ...
    addTieToLastNote(dottedTie: any): boolean {
        const el: Voice = getLastNote(this.tune);
    ...
    appendElement(type: string, startChar: number, endChar: number, hashParams: any): boolean {
        ...
        if (type === 'note') {
            const dur: number = getDuration(hashParams);
            if (dur >= 0.25) {
    ...
    appendStartingElement(type: string, startChar: number, endChar: number, hashParams2: any): void {
        closeLine(this.tune);
        let impliedNaturals;
        if (type === 'key') {
        ...
        if (!this.tune.lines[this.tune.lineNum])
            return;
        const staff: Array<{ [key: string]: any }> = this.tune.lines[this.tune.lineNum].staff;
        if (!staff)
            return;
        ...
        const voice: Array<any> | Array<{ [key: string]: any }> = staff[this.tune.staffNum].voices[this.tune.voiceNum];
        if (voice) {
            for (let i: number = 0; i < voice.length; i++) {
    ...
    addSubtitle(str: string, info: any): void {
    ...
    addSeparator(spaceAbove, spaceBelow, lineLength, info): void {
    ...
    setRunningFont(type: string, font: any): void {
        this.tune.runningFonts[type] = font;
    }
    setBarNumberImmediate(barNumber): number {
    ...
    getCurrentVoice(): Array<{ [key: string]: any }> {
    ...
    setCurrentVoice(staffNum: number, voiceNum: number, name: string): boolean {
    ...
    addMetaText(key: string, value: string, info: any): void {
    ...
    addMetaTextArray(key: string, value: string, info: any): void {
        if (this.tune.metaText[key] === undefined) {
            this.tune.metaText[key] = [value];
            this.tune.metaTextInfo[key] = info;
        }
    ...
    addMetaTextObj(key, value, info): void {
        this.tune.metaText[key] = value;
        this.tune.metaTextInfo[key] = info;
    }
}
function isArrayOfStrings(arr: Array<string>): boolean {
    if (!arr)
        return false;
...
function simplifyMetaText(tune: Tune): void {
...
function resolveOverlays(tune: Tune): boolean {
    let madeChanges: boolean = false;
    for (let i: number = 0; i < tune.lines.length; i++) {
        const line: Lines = tune.lines[i];
        if (line.staff) {
            for (let j: number = 0; j < line.staff.length; j++) {
                const staff: Staff = line.staff[j];
                const overlayVoice: Array<any> = [];
                for (let k: number = 0; k < staff.voices.length; k++) {
                    const voice: Array<{ [key: string]: any }> = staff.voices[k];
                    overlayVoice.push({ hasOverlay: false, voice: [], snip: [] });
                    let durationThisBar: number = 0;
                    let inOverlay: boolean = false;
                    let snipStart: number = -1;
                    for (let kk: number = 0; kk < voice.length; kk++) {
                        const event: Voice = voice[kk];
...
function findLastBar(voice: Voice[], start): number {
    let i: number = start - 1;
    for (; i > 0 && voice[i].el_type !== "bar"; i--) {
    }
    return i;
}
function fixTitles(lines: Array<{ [key: string]: any }> | Array<any>): void {
    let firstMusicLine: boolean = true;
    for (let i: number = 0; i < lines.length; i++) {
        ...
        if (line.staff) {
            for (let j: number = 0; j < line.staff.length; j++) {
                ...
                if (staff.title) {
                    let hasATitle: boolean = false;
                    for (let k: number = 0; k < staff.title.length; k++) {
                        if (staff.title[k]) {
...
function cleanUpSlursInLine(line: Array<{ [key: string]: any }>, staffNum: number, voiceNum: number, currSlur: Array<any> | Array<Array<Array<any>>> | Array<Array<Array<any>> | Array<Array<>>> | Array<Array<Array<Array<number>>>> | Array<Array<Array<>>>): void {
    if (!currSlur[staffNum])
        currSlur[staffNum] = [];
    if (!currSlur[staffNum][voiceNum])
        currSlur[staffNum][voiceNum] = [];
    let x;
    const addEndSlur = function (obj: any, num: number, chordPos: number): number {
        if (currSlur[staffNum][voiceNum][chordPos] === undefined) {
            ...
            if (currSlur[staffNum][voiceNum][chordPos] === undefined) {
                let offNum: number = chordPos * 100 + 1;
    ...
    const addStartSlur = function (obj: any, num: number, chordPos: number, usedNums: Array<any>): void {
        obj.startSlur = [];
        if (currSlur[staffNum][voiceNum][chordPos] === undefined) {
            currSlur[staffNum][voiceNum][chordPos] = [];
        }
        let nextNum: number = chordPos * 100 + 1;
        for (let i: number = 0; i < num; i++) {
            if (usedNums) {
    ...
    for (let i: number = 0; i < line.length; i++) {
        ...
        if (el.el_type === 'note') {
            if (el.gracenotes) {
                for (let g: number = 0; g < el.gracenotes.length; g++) {
                    if (el.gracenotes[g].endSlur) {
                        const gg = el.gracenotes[g].endSlur;
                        el.gracenotes[g].endSlur = [];
                        for (let ggg: number = 0; ggg < gg; ggg++)
            ...
            if (el.pitches) {
                const usedNums: Array<any> = [];
                for (let p: number = 0; p < el.pitches.length; p++) {
                    if (el.pitches[p].endSlur) {
                        const k: number = el.pitches[p].endSlur;
                        el.pitches[p].endSlur = [];
                        for (let j: number = 0; j < k; j++) {
                            const slurNum: number = addEndSlur(el.pitches[p], 1, p + 1);
...
function wrapMusicLines(lines: Array<{ [key: string]: any }>, barsperstaff: number): boolean {
    for (let i: number = 0; i < lines.length; i++) {
        if (lines[i].staff !== undefined) {
            for (let s: number = 0; s < lines[i].staff.length; s++) {
                const permanentItems: Array<any> = [];
                for (let v: number = 0; v < lines[i].staff[s].voices.length; v++) {
                    const voice: Array<{ [key: string]: any }> = lines[i].staff[s].voices[v];
                    let barNumThisLine: number = 0;
                    for (let n: number = 0; n < voice.length; n++) {
                            ...
                            if (barNumThisLine >= barsperstaff) {
                                if (n < voice.length - 1) {
                                    ...
                                    if (!nextLine) {
                                        ...
                                        for (let ss: number = 0; ss < nextLine.staff.length; ss++) {
                                            for (let vv: number = 0; vv < nextLine.staff[ss].voices.length; vv++)
                                                nextLine.staff[ss].voices[vv] = [];
                                        }
                                    }
                                    const startElement: number = n + 1;
                                    const section: Array<{ [key: string]: any }> = lines[i].staff[s].voices[v].slice(startElement);
...
function getPrevMusicLine(lines: Array<{ [key: string]: any }>, currentLine: number) {
...
function getNextMusicLine(lines: Array<{ [key: string]: any }>, currentLine: number) {
    currentLine++;
...
function getLastNote(tune: Tune): Voice {
    ...
    if (!tune.lines[tune.lineNum].staff[tune.staffNum])
        return null;
    const voice: Array<{ [key: string]: any }> = tune.lines[tune.lineNum].staff[tune.staffNum].voices[tune.voiceNum];
    if (!voice)
        return null;
    for (let i: number = voice.length - 1; i >= 0; i--) {
        const el: Voice = voice[i];
        if (el.el_type === 'note') {
            return el;
        }
    }
...
function getDuration(el: any): number {
    if (el.duration)
        return el.duration;
    return 0;
}
function closeLine(tune: Tune): void {
...
function containsNotes(voice: Array<{ [key: string]: any }> | Array<any>): boolean {
    for (let i: number = 0; i < voice.length; i++) {
        if (voice[i].el_type === 'note' || voice[i].el_type === 'bar')
            return true;
    }
...
function containsNotesStrict(voice: Voice[]): boolean {
    for (let i: number = 0; i < voice.length; i++) {
...
function pushLine(tune: Tune, hash: any): void {
...
function pushNote(tune: Tune, hp: any, voiceDefs: any, currentVoiceName: string): void {
    const currStaff: Staff = tune.lines[tune.lineNum].staff[tune.staffNum];
    if (hp.pitches !== undefined) {
        const mid: number = currStaff.workingClef.verticalPos;
        hp.pitches.forEach(function (p: any) { p.verticalPos = p.pitch - mid; });
    }
    if (hp.gracenotes !== undefined) {
        const mid2: number = currStaff.workingClef.verticalPos;
        hp.gracenotes.forEach(function (p: any) { p.verticalPos = p.pitch - mid2; });
    }
...
function endBeamHere(hashParams: any, tune: Tune): void {
    tune.potentialStartBeam.startBeam = true;
    hashParams.endBeam = true;
    delete tune.potentialStartBeam;
    delete tune.potentialEndBeam;
}
function endBeamLast(tune: Tune): void {
...
function setLineFont(tune: Tune, type: string, font: any): void {
    if (tune.runningFonts[type]) {
        let isDifferent: boolean = false;
        const keys: Array<string> = Object.keys(font);
...
function createVoice(tune: Tune, params: any): void {
    const thisStaff: Staff = tune.lines[tune.lineNum].staff[tune.staffNum];
    ...
    if (params.stem)
        ...
        if (thisStaff.voices[0] !== undefined) {
            ...
            for (let i: number = 0; i < thisStaff.voices[0].length; i++) {
...
function createStaff(tune: Tune, params: any): void {
    ...
    const staff: Staff = tune.lines[tune.lineNum].staff[tune.staffNum];
...
function createLine(tune: Tune, params: any): void {
    tune.lines[tune.lineNum] = { staff: [] };
    createStaff(tune, params);
}
function voiceUseful(lines: Lines[], voiceNum: number): boolean | string {
    let isUseful: boolean = false;
    let voiceExists: boolean = false;
    for (let line: number = 0; line < lines.length; line++) {
        const staves: Staff[] = lines[line].staff;
        if (staves) {
            for (let s: number = 0; s < staves.length; s++) {
                const staff: Staff = staves[s];
                if (voiceNum < staff.voices.length) {
                    voiceExists = true;
                    const voice: Voice[] = staff.voices[voiceNum];
                    for (let e: number = 0; e < voice.length; e++) {
                        const el: Voice = voice[e];
...
function deleteVoice(lines: Lines[], voiceNum: number): void {
    for (let line: number = 0; line < lines.length; line++) {
        const staves: Staff[] = lines[line].staff;
        if (staves) {
            for (let s: number = 0; s < staves.length; s++) {
...
function appendElementHelper(tune: Tune, type: string, startChar: number, endChar: number, hashParams: any): void {
    hashParams.el_type = type;
    if (startChar !== null)
        hashParams.startChar = startChar;
    if (endChar !== null)
