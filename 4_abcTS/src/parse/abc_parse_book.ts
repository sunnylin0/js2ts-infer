...
const bookParser = function (book: string) {
    "use strict";
    let directives: string = "";
    const initialWhiteSpace: Array<string> = book.match(/(\s*)/);
    book = parseCommon.strip(book);
    const tuneStrings: Array<string> = book.split("\nX:");
    // Put back the X: that we lost when splitting the tunes.
    for (let i: number = 1; i < tuneStrings.length; i++)
    ...
    let pos: number = initialWhiteSpace && initialWhiteSpace[0] ? initialWhiteSpace[0].length : 0;
    const tunes: Array<any> = [];
    ...
    const header: string = directives;
    // Now, the tune ends at a blank line, so truncate it if needed. There may be "intertune" stuff.
    tunes.forEach(function (tune) {
        const end: number = tune.abc.indexOf('\n\n');
        if (end > 0)
        ...
        tune.title = "";
        let titleArr: Array<string> = tune.pure.split("T:");
        if (titleArr.length > 1) {
            let titleLines: Array<string> = titleArr[1].split("\n");
            tune.title = parseCommon.strip(titleLines[0]);
        }
        // for the user's convenience, parse and store the id separately. The id is between the first X: and the next \n
        const id: string = tune.pure.substring(2, tune.pure.indexOf("\n"));
