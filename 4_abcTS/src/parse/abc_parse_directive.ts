...
const fontTypeCanHaveBox = { gchordfont: true, measurefont: true, partsfont: true, annotationfont: true, composerfont: true, historyfont: true, infofont: true, subtitlefont: true, textfont: true, titlefont: true, voicefont: true };
const fontTranslation = function (fontFace: string) {
    switch (fontFace) {
...
const getFontParameter = function (tokens: Array<{ [key: string]: any }>, currentSetting: any, str: string, position: number, cmd: string): {} {
    function processNumberOnly() {
        const size: number = parseInt(tokens[0].token, 10);
        tokens.shift();
        if (!currentSetting) {
    ...
    let state: string = 'face';
    let hyphenLast: boolean = false;
    while (tokens.length) {
    ...
    let faceStr: string = faceArr.join(' ');
    if (faceStr === '') {
        if (!currentSetting) {
...
const getChangingFont = function (cmd: string, tokens: Array<{ [key: string]: any }>, str: string): string {
    if (tokens.length === 0)
...
const getGlobalFont = function (cmd: string, tokens: Array<{ [key: string]: any }>, str: string): string {
    if (tokens.length === 0)
        return "Directive \"" + cmd + "\" requires a font as a parameter.";
    tune.formatting[cmd] = getFontParameter(tokens, tune.formatting[cmd], str, 0, cmd);
    return null;
};
const setScale = function (cmd: string, tokens: Array<{ [key: string]: any }>): string {
    let scratch: string = "";
    tokens.forEach(function (tok) {
        scratch += tok.token;
    });
    const num: number = parseFloat(scratch);
    if (isNaN(num) || num === 0)
        return "Directive \"" + cmd + "\" requires a number as a parameter.";
    tune.formatting.scale = num;
};
const drumNames: Array<string> = [
    "acoustic-bass-drum",
    "bass-drum-1",
    "side-stick",
    "acoustic-snare",
...
const interpretPercMap = function (restOfString) {
    ...
    const value = { sound: pitch };
    if (tokens.length === 3)
        value.noteHead = tokens[2];
    return { key: key, value: value };
};
const getRequiredMeasurement = function (cmd, tokens) {
    const points = tokenizer.getMeasurement(tokens);
    if (points.used === 0 || tokens.length !== 0)
        return { error: "Directive \"" + cmd + "\" requires a measurement as a parameter." };
    return points.value;
};
const oneParameterMeasurement = function (cmd: string, tokens: Array<{ [key: string]: any }>): string {
    const points = tokenizer.getMeasurement(tokens);
    if (points.used === 0 || tokens.length !== 0)
...
const addMultilineVar = function (key: string, cmd: string, tokens: Array<{ [key: string]: any }>, min, max): string {
...
const addMultilineVarBool = function (key, cmd, tokens): string {
    if (tokens.length === 1 && (tokens[0].token === 'true' || tokens[0].token === 'false')) {
        multilineVars[key] = tokens[0].token === 'true';
        return null;
    }
    const str: string = addMultilineVar(key, cmd, tokens, 0, 1);
    if (str !== null)
        return str;
    multilineVars[key] = (multilineVars[key] === 1);
    return null;
};
const addMultilineVarOneParamChoice = function (key, cmd, tokens, choices): string {
    if (tokens.length !== 1)
        return "Directive \"" + cmd + "\" requires one of [ " + choices.join(", ") + " ] as a parameter.";
    const choice = tokens[0].token;
    let found: boolean = false;
    for (let i: number = 0; !found && i < choices.length; i++) {
        if (choices[i] === choice)
            found = true;
    }
    if (!found)
...
const midiCmdParam0: Array<string> = ["nobarlines", "barlines", "beataccents", "nobeataccents", "droneon", "droneoff", "drumon", "drumoff", "fermatafixed", "fermataproportional", "gchordon", "gchordoff", "controlcombo", "temperamentnormal", "noportamento"];
const midiCmdParam1String: Array<string> = ["gchord", "ptstress", "beatstring"];
const midiCmdParam1Integer: Array<string> = ["bassvol", "chordvol", "c", "channel", "beatmod", "deltaloudness", "drumbars", "gracedivider", "makechordchannels", "randomchordattack", "chordattack", "stressmodel", "transpose", "rtranspose", "vol", "volinc", "gchordbars"];
const midiCmdParam1Integer1OptionalInteger: Array<string> = ["program"];
const midiCmdParam2Integer: Array<string> = ["ratio", "snt", "bendvelocity", "pitchbend", "control", "temperamentlinear"];
const midiCmdParam4Integer: Array<string> = ["beat"];
const midiCmdParam5Integer: Array<string> = ["drone"];
const midiCmdParam1String1Integer: Array<string> = ["portamento"];
const midiCmdParamFraction: Array<string> = ["expand", "grace", "trim"];
const midiCmdParam1StringVariableIntegers: Array<string> = ["drum", "chordname"];
const midiCmdParam1Integer1OptionalString: Array<string> = ["bassprog", "chordprog"];
const parseMidiCommand = function (midi: Array<{ [key: string]: any }>, tune: Tune, restOfString: string): void {
...
const parseDirective = {
    initialize: function (tokenizer_: Tokenizer, warn_: any, multilineVars_: any, tune_: Tune, tuneBuilder_: TuneBuilder) {
    ...
    parseFontChangeLine: function (textstr: string) {
        textstr = textstr.replace(/\$\$/g, "\x03");
        const textParts: string[] = textstr.split('$');
        if (textParts.length > 1 && multilineVars.setfont) {
            const textarr = [];
            if (textParts[0] !== '')
                textarr.push({ text: textParts[0] });
            for (let i: number = 1; i < textParts.length; i++) {
                if (textParts[i][0] === '0')
                    textarr.push({ text: textParts[i].substring(1).replace(/\x03/g, "$$") });
                else {
                    const whichFont: number = parseInt(textParts[i][0], 10);
    ...
    addDirective: function (str) {
        ...
        let scratch: string = "";
        let line;
        const positionChoices: string[] = ['auto', 'above', 'below', 'hidden'];
        switch (cmd) {
            case "bagpipes":
            ...
            case "graceslurs":
                ...
                break;
            case "lineThickness":
                const lt = parseStretchLast(tokens);
                ...
                break;
            case "stretchlast":
                const sl = parseStretchLast(tokens);
            ...
            case "voicecolor":
                ...
                return null;
            case "vskip":
                const vskip: number = Math.round(getRequiredMeasurement(cmd, tokens));
                if (vskip.error)
                    return vskip.error;
            ...
            case "setfont":
                ...
                if (sfTokens.length >= 4) {
                    if (sfTokens[0].token === '-' && sfTokens[1].type === 'number') {
                        const sfNum: number = parseInt(sfTokens[1].token, 10);
                        if (sfNum >= 1 && sfNum <= 9) {
            ...
            case "score":
                ...
                const addVoice = function (id, newStaff, bracket, brace, continueBar): void {
                    if (newStaff || multilineVars.staves.length === 0) {
                        multilineVars.staves.push({ index: multilineVars.staves.length, numVoices: 0 });
                    }
                    const staff: string = parseCommon.last(multilineVars.staves);
                ...
                let lastVoice;
                const addContinueBar = function (): void {
                    continueBar = true;
                    if (lastVoice) {
                        let ty: string = 'start';
            ...
            case "header":
            case "footer":
                let footerStr = tokenizer.getMeat(restOfString, 0, restOfString.length);
