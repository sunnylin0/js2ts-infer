...
export function wrapLines(tune: Tune, lineBreaks: any, barNumbers: number): void {
    ...
    const linesBreakElements: any[] = findLineBreaks(lines, lineBreaks);
    tune.lines = addLineBreaks(lines, linesBreakElements, barNumbers);
    tune.lineBreaks = linesBreakElements;
}
function addLineBreaks(lines, linesBreakElements: any[], barNumbers: number): any[] {
    const outputLines = [];
    const lastKeySig = [];
    const lastStem = [];
    let currentBarNumber: number = 1;
    for (let i: number = 0; i < linesBreakElements.length; i++) {
        ...
        if (lines[action.ogLine].staff) {
            ...
            if (!outputLines[action.line].staff[action.staff]) {
                ...
                const keys: string[] = Object.keys(inputStaff);
                for (let k: number = 0; k < keys.length; k++) {
                    let skip: boolean = keys[k] === "voices";
            ...
            for (let kk: number = currVoice.length - 1; kk >= 0; kk--) {
            ...
            if (barNumbers !== undefined && action.staff === 0 && action.voice === 0) {
                for (let kk: number = 0; kk < currVoice.length; kk++) {
    ...
    for (let ii: number = 0; ii < outputLines.length; ii++) {
...
function findLineBreaks(lines, lineBreakArray): any[] {
    const lineBreakIndexes = [];
    let lbai: number = 0;
    let lineCounter: number = 0;
    let outputLine: number = 0;
    for (let i: number = 0; i < lines.length; i++) {
        const line = lines[i];
        if (line.staff) {
            const lineStart: number = lineCounter;
...
function freeFormLineBreaks(widths, lineBreakPoint: number) {
    const lineBreaks = [];
    const totals = [];
    let totalThisLine: number = 0;
    for (let i: number = 0; i < widths.length; i++) {
        ...
        if (attemptedWidth < lineBreakPoint)
            ...
            const oldDistance: number = lineBreakPoint - totalThisLine;
            const newDistance: number = attemptedWidth - lineBreakPoint;
...
function oneTry(measureWidths, idealWidths: any[], accumulator, lineAccumulator, lineWidths, lastVariance, highestVariance, currLine, lineBreaks, startIndex, otherTries: any[]): void {
    for (let i = startIndex; i < measureWidths.length; i++) {
        ...
        const thisVariance: number = Math.abs(accumulator - idealWidths[currLine]);
        const varianceIsClose: boolean = Math.abs(thisVariance - lastVariance) < idealWidths[0] / 10;
        if (varianceIsClose) {
            if (thisVariance < lastVariance) {
                const newWidths: any[] = clone(lineWidths);
                const newBreaks: any[] = clone(lineBreaks);
                newBreaks.push(i - 1);
...
function optimizeLineWidths(widths, lineBreakPoint: number, lineBreaks, explanation) {
    const numLines: number = Math.ceil(widths.total / lineBreakPoint);
    const idealWidth: number = Math.floor(widths.total / numLines);
    const idealWidths = [];
    ...
    for (let i: number = 0; i < otherTries.length; i++) {
        const otherTry = otherTries[i];
        otherTry.variances = [];
        otherTry.aveVariance = 0;
        for (let j: number = 0; j < otherTry.lineWidths.length; j++) {
...
function fixedMeasureLineBreaks(widths, lineBreakPoint: number, preferredMeasuresPerLine: number) {
    const lineBreaks = [];
    const totals = [];
    let thisWidth: number = 0;
...
function getRevisedTuneParams(lineBreaks: any[], staffWidth, params) {
    const revisedParams = {
...
export function calcLineWraps(tune, widths, params) {
    ...
    const scale: number = params.scale ? Math.max(params.scale, 0.1) : 1;
    const minSpacing: number = params.wrap.minSpacing ? Math.max(parseFloat(params.wrap.minSpacing), 1) : 1;
    const minSpacingLimit: number = params.wrap.minSpacingLimit ? Math.max(parseFloat(params.wrap.minSpacingLimit), 1) : minSpacing - 0.1;
    let maxSpacing: number = params.wrap.maxSpacing ? Math.max(parseFloat(params.wrap.maxSpacing), 1) : undefined;
    if (params.wrap.lastLineLimit && !maxSpacing)
        maxSpacing = Math.max(parseFloat(params.wrap.lastLineLimit), 1);
    const preferredMeasuresPerLine: number = params.wrap.preferredMeasuresPerLine ? Math.max(parseInt(params.wrap.preferredMeasuresPerLine, 10), 0) : undefined;
    const accumulatedLineBreaks = [];
    const explanations = [];
    for (let s: number = 0; s < widths.length; s++) {
        const section = widths[s];
        const usableWidth: number = params.staffwidth - section.left;
        const lineBreakPoint: number = usableWidth / minSpacing / scale;
        const minLineSize: number = maxSpacing ? usableWidth / maxSpacing / scale : 0;
        const allowableVariance: number = usableWidth / minSpacingLimit / scale;
        const explanation = {
            widths: section,
