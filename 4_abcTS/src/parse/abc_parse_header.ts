...
export default class ParseHeader {
    ...
    constructor(_tokenizer: Tokenizer, _warn: any, _multilineVars: any, _tune: Tune, _tuneBuilder: TuneBuilder) {

        this.reset(_tokenizer, _warn, _multilineVars, _tune, _tuneBuilder);
    }
    reset(tokenizer: Tokenizer, warn: any, multilineVars: any, tune: Tune, tuneBuilder: TuneBuilder): void {
    ...
    setTitle(title: string, origSize: number): void {
    ...
    setMeter(line: string) {
        ...
        if (line === 'C') {
            ...
            const tokens: Array<{ [key: string]: any }> = this.tokenizer.tokenize(line, 0, line.length);
            try {
                const parseNum = () => {
    ...
    calcTempo(relTempo) {
        ...
        for (let i: number = 0; i < relTempo.duration.length; i++)
    ...
    addUserDefinition(line: string, start: number, end: number): void {
        const equals: number = line.indexOf('=', start);
        if (equals === -1) {
            this.warn("Need an = in a macro definition", line, start);
            return;
        }
        const before: string = parseCommon.strip(line.substring(start, equals));
        const after: string = parseCommon.strip(line.substring(equals + 1));
        if (before.length !== 1) {
            this.warn("Macro definitions can only be one character", line, start);
            return;
        }
        const legalChars: string = "HIJKLMNOPQRSTUVWXYhijklmnopqrstuvw~";
    ...
    setDefaultLength(line: string, start: number, end: number): void {
        const len: string = line.substring(start, end).replace(/ /g, "");
        const len_arr: Array<string> = len.split('/');
        if (len_arr.length === 2) {
            const n: number = parseInt(len_arr[0], 10);
            const d: number = parseInt(len_arr[1], 10);
            if (d > 0) {
    ...
    setTempo(line: string, start: number, end: number, iChar: number) {
        try {
            const tokens: Array<{ [key: string]: any }> = this.tokenizer.tokenize(line, start, end);
            if (tokens.length === 0)
                throw "Missing parameter in Q: field";
            const tempo = { startChar: iChar + start - 2, endChar: iChar + end };
            ...
            if (token.type === 'alpha' && token.token === 'C') {
                ...
                let num: number = parseInt(token.token, 10);
                if (tokens.length === 0 || tokens[0].type === 'quote') {
                    ...
                    if (token.type !== 'number')
                        throw "Expected fraction in Q: field";
                    let den: number = parseInt(token.token, 10);
    ...
    letter_to_inline_header(line: string, i: number, startLine: boolean): (string | number | boolean)[] {
        let needsNewLine: boolean = false;
        const ws: number = this.tokenizer.eatWhiteSpace(line, i);
        i += ws;
        if (line.length >= i + 5 && line[i] === '[' && line[i + 2] === ':') {
            let e: number = line.indexOf(']', i);
            const startChar: number = this.multilineVars.iChar + i;
            const endChar: number = this.multilineVars.iChar + e + 1;
            switch (line.substring(i, i + 3)) {
                ...
                case "[K:":
                    ...
                    return [e - i + 1 + ws];
                case "[P:":
                    const part: string = parseDirective.parseFontChangeLine(line.substring(i + 3, e));
    ...
    letter_to_body_header(line: string, i: number): (string | number | boolean)[] {
        let needsNewLine: boolean = false;
        if (line.length >= i + 3) {
            switch (line.substring(i, i + 2)) {
                ...
                case "L:":
                    ...
                    return [line.length];
                case "Q:":
                    let e: number = line.indexOf('\x12', i + 2);
                    if (e === -1)
                        e = line.length;
                    const tempo = this.setTempo(line, i + 2, e, this.multilineVars.iChar);
                    if (tempo.type === 'delaySet')
                        this.tuneBuilder.appendElement('tempo', this.multilineVars.iChar + i, this.multilineVars.iChar + line.length, this.calcTempo(tempo.tempo));
                    else if (tempo.type === 'immediate')
                        this.tuneBuilder.appendElement('tempo', this.multilineVars.iChar + i, this.multilineVars.iChar + line.length, tempo.tempo);
    ...
    parseHeader(line: string): {} {
        const field: string = this.metaTextHeaders[line[0]];
        const origSize: number = line.length - 2;
        let restOfLine: string = this.tokenizer.translateString(this.tokenizer.stripComment(line.substring(2)));
        if (field === 'unalignedWords' || field === 'notes') {
            ...
            const startChar: number = this.multilineVars.iChar;
            const endChar: number = startChar + line.length;
            switch (line[0]) {
                case 'H':
