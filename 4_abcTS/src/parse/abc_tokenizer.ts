...
export default class Tokenizer {
    ...
    constructor(lines: Array<string>, multilineVars: any) {
        this.lines = lines;
    ...
    getKeyPitch(str: string) {
        const i: number = this.skipWhiteSpace(str);
    ...
    getMode(str: string): number {
        const skipAlpha = function (str: string, start: number): number {
        ...
        if (this.finished(str, i))
            return { len: 0 };
        let firstThree: string = str.substring(i, i + 3).toLowerCase();
    ...
    getClef(str, bExplicitOnly) {
        const strOrig = str;
        let i: number = this.skipWhiteSpace(str);
        if (this.finished(str, i))
            return { len: 0 };
        let needsClef: boolean = false;
    ...
    getBarLine(line: string, i: number) {
        switch (line[i]) {
            case ']':
                ++i;
                switch (line[i]) {
    ...
    getTokenOf(str: string, legalChars: string) {
        for (let i: number = 0; i < str.length; i++) {
    ...
    isMatch(str, match: string): number {
        const i: number = this.skipWhiteSpace(str);
        if (this.finished(str, i))
            return 0;
    ...
    getKeyAccidentals2(tokens: Array<{ [key: string]: any }>) {
        let accs;
    ...
    getMeat(line: string, start: number, end: number) {
        const comment: number = line.indexOf('%', start);
    ...
    tokenize(line: string, start: number, end: number, alphaUntilWhiteSpace: boolean): any[] {
        ...
        while (start < end) {
            if (line[start] === '"') {
                ...
                if (alphaUntilWhiteSpace)
                    while (i < end && !this.isWhiteSpace(line[i]))
                        i++;
                else
                    while (i < end && this.isLetter(line[i]))
                        i++;
                ...
                i = start + 1;
                let float2 = null;
                while (i < end && this.isNumber(line[i]))
                    i++;
    ...
    getVoiceToken(line: string, start: number, end: number) {
        let i: number = start;
        while (i < end && (this.isWhiteSpace(line[i]) || line[i] === '='))
            i++;
        if (line[i] === '"') {
            let close: number = line.indexOf('"', i + 1);
    ...
    getNumber(line: string, index: number) {
        let num: number = 0;
    ...
    getFraction(line: string, index: number) {
        let num: number = 1;
        let den: number = 1;
        if (line[index] !== '/') {
    ...
    theReverser(str: string): string {
        const thePatterns: Array<{ [key: string]: any }> = [
        ...
        const getTitleNumber = function (s: string): string {
            const regex: RegExp = /^(\d+)\./;
            const match: RegExpExecArray = regex.exec(s);
            if (match) {
                return match[1];
            }
            else {
        ...
        for (let i: number = 0; i < thePatterns.length; i++) {
            const thisPattern = thePatterns[i];
            const match: Array<string> = str.match(thisPattern.match);
            if (match) {
                const theTitleNumber: string = getTitleNumber(str);
                if (theTitleNumber) {
                    str = str.replace(theTitleNumber + ".", "");
                    str = str.trim();
                }
                const len: number = match[0].length;
                let result: string = thisPattern.replace + str.substring(0, str.length - len);
                if (theTitleNumber) {
    ...
    getMeasurement(tokens: Array<{ [key: string]: any }>) {
        if (tokens.length === 0)
            return { used: 0 };
        let used: number = 1;
        let num: string = '';
    ...
    getBrackettedSubstring(line: string, i: number, maxErrorChars: number, _matchChar: string): (string | number | boolean)[] {
        const substInChord = function (str: string): string {
            str = str.replace(/\\n/g, "\n");
            str = str.replace(/\\"/g, '"');
            return str;
        };
        const matchChar: string = _matchChar || line[i];
        let pos: number = i + 1;
    ...
    nextLine(): string {
        ...
        if (this.lineIndex < this.lines.length) {
            const result: string = this.lines[this.lineIndex];
            this.lineIndex++;
            return result;
        }
        return null;
