...
export default class Parse {
    ...
    wordsContinuation: string = '';
    symbolContinuation: string = '';
    tune: Tune = new Tune();
    ...
    constructor() {
        ...
        this.multilineVars = {
            ...
            differentFont: (type, defaultFonts) => {
                if (this.multilineVars[type].decoration !== defaultFonts[type].decoration)
                    return true;
                if (this.multilineVars[type].face !== defaultFonts[type].face)
                    return true;
                if (this.multilineVars[type].size !== defaultFonts[type].size)
                    return true;
                if (this.multilineVars[type].style !== defaultFonts[type].style)
                    return true;
                if (this.multilineVars[type].weight !== defaultFonts[type].weight)
                    return true;
                return false;
            },
            addFormattingOptions: (el, defaultFonts, elType) => {
                if (elType === 'note') {
                    if (this.multilineVars.vocalPosition !== 'auto')
                        this.addPositioning(el, 'vocalPosition', this.multilineVars.vocalPosition);
                    if (this.multilineVars.dynamicPosition !== 'auto')
                        this.addPositioning(el, 'dynamicPosition', this.multilineVars.dynamicPosition);
                    if (this.multilineVars.chordPosition !== 'auto')
                        this.addPositioning(el, 'chordPosition', this.multilineVars.chordPosition);
                    if (this.multilineVars.ornamentPosition !== 'auto')
                        this.addPositioning(el, 'ornamentPosition', this.multilineVars.ornamentPosition);
                    if (this.multilineVars.volumePosition !== 'auto')
                        this.addPositioning(el, 'volumePosition', this.multilineVars.volumePosition);
                    if (this.multilineVars.differentFont("annotationfont", defaultFonts))
                        this.addFont(el, 'annotationfont', this.multilineVars.annotationfont);
                    if (this.multilineVars.differentFont("gchordfont", defaultFonts))
                        this.addFont(el, 'gchordfont', this.multilineVars.gchordfont);
                    if (this.multilineVars.differentFont("vocalfont", defaultFonts))
                        this.addFont(el, 'vocalfont', this.multilineVars.vocalfont);
                    if (this.multilineVars.differentFont("tripletfont", defaultFonts))
                        this.addFont(el, 'tripletfont', this.multilineVars.tripletfont);
                }
                else if (elType === 'bar') {
                    if (this.multilineVars.dynamicPosition !== 'auto')
                        this.addPositioning(el, 'dynamicPosition', this.multilineVars.dynamicPosition);
                    ...
                    if (this.multilineVars.volumePosition !== 'auto')
                        this.addPositioning(el, 'volumePosition', this.multilineVars.volumePosition);
                    if (this.multilineVars.differentFont("measurefont", defaultFonts))
                        this.addFont(el, 'measurefont', this.multilineVars.measurefont);
                    if (this.multilineVars.differentFont("repeatfont", defaultFonts))
                        this.addFont(el, 'repeatfont', this.multilineVars.repeatfont);
                }
            },
            duplicateStartEndingHoldOvers: () => {
                ...
                for (let i: number = 0; i < this.multilineVars.inTie.length; i++) {
                    this.multilineVars.endingHoldOver.inTie.push([]);
                    if (this.multilineVars.inTie[i]) {
                        for (let j: number = 0; j < this.multilineVars.inTie[i].length; j++) {
            ...
            restoreStartEndingHoldOvers: () => {
                ...
                this.multilineVars.inTie = [];
                this.multilineVars.inTieChord = {};
                for (let i: number = 0; i < this.multilineVars.endingHoldOver.inTie.length; i++) {
                    this.multilineVars.inTie.push([]);
                    for (let j: number = 0; j < this.multilineVars.endingHoldOver.inTie[i].length; j++) {
    ...
    warn = (str, line, col_num) => {
        ...
        if (bad_char === ' ' || !bad_char)
            bad_char = "SPACE";
        const clean_line: string = this.encode(line.substring(col_num - 64, col_num)) + '<span style="text-decoration:underline;font-size:1.3em;font-weight:bold;">' + bad_char + '</span>' + this.encode(line.substring(col_num + 1).substring(0, 64));
    ...
    addWords = (line, words) => {
        ...
        const addWord = (i): boolean => {
            let word: string = parseCommon.strip(words.substring(last_divider, i));
    ...
    appendLastMeasure = (voice, nextVoice) => {
        ...
        for (let i: number = 0; i < nextVoice.length; i++) {
    ...
    addHintMeasure = (staff, nextStaff) => {
        for (let i: number = 0; i < staff.length; i++) {
            ...
            if (nextStave) {
                for (let j: number = 0; j < nextStave.voices.length; j++) {
    ...
    addPositioning(el, type: string, value: AbcPositionChoice): void {
        if (!el.positioning)
            el.positioning = {};
        el.positioning[type] = value;
    }
    addFont(el, type: string, value): void {
        if (!el.fonts)
            el.fonts = {};
        el.fonts[type] = value;
    }
    ...
    parse(strTune: string, switches: any, startPos: number): void {
        if (!switches)
            switches = {};
        if (!startPos)
            startPos = 0;
        ...
        const arr: Array<string> = strTune.split("\n\\");
        if (arr.length > 1) {
            for (let i2: number = 1; i2 < arr.length; i2++) {
        ...
        strTune = strTune.replace(/\\([ \t]*)(%.*)*\n/g, function (all, backslash, comment) {
            const padding: string = comment ? Array(comment.length + 1).join(' ') : "";
            return backslash + "\x12" + padding + '\n';
        });
        const lines: Array<string> = strTune.split('\n');
        ...
        try {
            ...
            let line: string = this.tokenizer.nextLine();
            while (line) {
