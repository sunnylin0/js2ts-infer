...
export default class TimingCallbacks {
	public qpm: any;
	public extraMeasuresAtBeginning: any;
	public beatCallback: any;
	public eventCallback: any;
	public lineEndCallback: any;
	public lineEndAnticipation: any;
	public beatSubdivisions: any;
	public joggerTimer: any;
	public noteTimings: any;
	public lineEndTimings: any;
	public startTime: any;
	public currentBeat: any;
	public currentEvent: any;
	public currentLine: any;
	public currentTime: any;
	public isPaused: any;
	public isRunning: any;
	public pausedPercent: any;
	public justUnpaused: any;
	public newSeekPercent: any;
	public lastTimestamp: any;
	public millisecondsPerBeat: any;
	public lastMoment: any;
	public beatStarts: any;
	public totalBeats: any;
	public reportNext: any;

	constructor(target: any, params: any) {
		if (!params) params = {};
	...
	public replaceTarget(newTarget: any) {
		...
		if (irregularMeter) {
			...
			var numMeasures = this.lastMoment / measureLength
			var parts: any = irregularMeter.split("+")
			...
			for (var measureNumber = 0; measureNumber < numMeasures; measureNumber++) {
				...
				for (var kk = 0; kk < parts.length; kk++) {
					...
					if (this.beatSubdivisions === 1) {
						...
						var numDivisions = beatLength * this.beatSubdivisions
						for (var k = 0; k < Math.floor(numDivisions); k++) {
							var subBeat = k / numDivisions
							var ts = Math.round(measureStartTs + subBeatCounter * this.millisecondsPerBeat)
		...
		if (!this.isPaused && this.isRunning) {
			...
			if (this.currentTime >= this.lastMoment) {
				if (this.eventCallback) {
					...
					this.shouldStop(promise).then((shouldStop: boolean) => {
						if (shouldStop)
							this.stop();
					})
				} else
	...
	public shouldStop(promise: any): Promise<boolean> {
	...
	public doBeatCallback(timestamp: number) {
		if (this.beatCallback) {
			...
			if (ev) {
				...
				var offMs = Math.max(0, timestamp-this.startTime-ev.milliseconds); 
			...
			var thisStartTime = this.startTime; 
			this.beatCallback(
				this.beatStarts[this.currentBeat].b,
	...
	public start(offsetPercent?: number, units?: string) {
		this.isRunning = true;
		if (this.isPaused) {
			this.isPaused = false;
	...
	public setProgress(position: any, units?: string) {
		var percent;
		switch (units) {
			case "seconds":
...
function getLineEndTimings(timings: any, anticipation: number) {
	var callbackTimes: any = [];
	var lastTop: any = null;
