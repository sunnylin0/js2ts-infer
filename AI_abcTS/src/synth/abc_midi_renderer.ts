...
class Midi {
	private trackstrings: string = "";
	private trackcount: number = 0;
	private noteOnAndChannel: string = "%90";
	private noteOffAndChannel: string = "%80";
	private track: string = "";
	private trackName: string = "";
	private trackInstrument: string = "";
	private silencelength: number = 0;
	private instrument: number | undefined;
	private channel: number = 0;
	private noteWarped: Record<number, boolean> = {};

	public setTempo(qpm: number): void {
	...
	public setGlobalInfo(qpm: number, name?: string, key?: any, time?: any): void {
		if (this.trackcount === 0) {
			this.startTrack();
			const divisions = Math.round(60000000 / qpm);
			this.track += "%00%FF%51%03" + this.toHex(divisions, 6);
			if (key) this.track += this.keySignature(key);
			if (time) this.track += this.timeSignature(time);
			if (name) this.track += this.encodeString(name, "%01");
			this.endTrack();
		}
	}
	...
	public setInstrument(number: number): void {
		this.trackInstrument = "%00%C0" + this.toHex(number, 2);
		this.instrument = number;
	}

	public setChannel(number: number, pan?: number): void {
		this.channel = number;
	...
	private HALF_STEP = 4096;
	public startNote(pitch: number, loudness: number, cents?: number): void {
	...
	public endNote(pitch: number): void {
		this.track += this.toDurationHex(this.silencelength);
		this.silencelength = 0;
	...
	public addRest(length: number): void {
	...
	public embed(parent: HTMLElement, noplayer?: boolean): void {
	...
	private setAttributes(elm: HTMLElement, attrs: Record<string, string>): HTMLElement {
		for (const attr in attrs) elm.setAttribute(attr, attrs[attr]);
		return elm;
	}

	private encodeString(str: string, cmdType: string): string {
		let nameArray = "";
		for (let i = 0; i < str.length; i++) nameArray += this.toHex(str.charCodeAt(i), 2);
		return "%00%FF" + cmdType + this.toHex(nameArray.length / 3, 2) + nameArray;
	}

	private keySignature(key: any): string {
	...
	private timeSignature(time: any): string {
		let hex = "%00%FF%58%04" + this.toHex(time.num, 2);
		const dens: Record<number, number> = { 1: 0, 2: 1, 4: 2, 8: 3, 16: 4, 32: 5 };
		const den = dens[time.den];
		if (den === undefined) return "";
		hex += this.toHex(den, 2);

		let clocks: number | undefined;
		switch (time.num + "/" + time.den) {
			case "2/4": case "3/4": case "4/4": case "5/4": clocks = 24; break;
			case "6/4": clocks = 72; break;
			case "2/2": case "3/2": case "4/2": clocks = 48; break;
			case "3/8": case "6/8": case "9/8": case "12/8": clocks = 36; break;
		}
		if (!clocks) return "";
	...
	private to7BitHex(n: number): string {
	...
	private toDurationHex(n: number): string {
		const a: number[] = [];
