...
export class MidiFlattener {
	private barAccidentals: number[] = [];
	private accidentals: number[] = [0, 0, 0, 0, 0, 0, 0];
	private midiTranspose: number = 0;
	private bagpipes: boolean = false;
	private tracks: any[] = [];
	private startingTempo: number;
	private startingMeter: any;
	private tempoChangeFactor: number = 1;
	private instrument: any;
	private currentInstrument: any;
	private currentTrack: any[] = [];
	private lastNoteDurationPosition: number = -1;
	private currentTrackName: any;
	private lastEventTime: number = 0;
	private chordTrack: ChordTrack;

	private meter: any = { num: 4, den: 4 };
	private drumInstrument: number = 128;
	private lastBarTime: number = 0;
	private doBeatAccents: boolean = true;
	private stressBeat1: number = 105;
	private stressBeatDown: number = 95;
	private stressBeatUp: number = 85;
	private volumesPerNotePitch: number[][] = [];
	private beatFraction: number = 0.25;
	private nextVolume: number | undefined;
	private nextVolumeDelta: number | undefined;
	private slurCount: number = 0;

	private drumTrack: any[] = [];
	private drumTrackFinished: boolean = false;
	private drumDefinition: any = {};
	private drumBars: number = 1;

	private pickupLength: number = 0;
	private percmap: any;

	private normalBreakBetweenNotes: number = 0;
	private slurredBreakBetweenNotes: number = -0.001;
	private staccatoBreakBetweenNotes: number = 0.4;

	public flatten(voices: any[], options: any, percmap_: any, midiOptions: any) {
		if (!options) options = {};
	...
	private timeToRealTime(time: number) {
		return time / 1000000;
	}

	private durationRounded(duration: number) {
		return Math.round(duration * this.tempoChangeFactor * 1000000) / 1000000;
	}

	private preProcess(voices: any[], options: any) {
		for (var i = 0; i < voices.length; i++) {
			var voice = voices[i];
			var ties: any = {};
	...
	private getBeatFraction(meter: any) {
	...
	private calcBeat(measureStart: number, beatLength: number, currTime: number) {
		var distanceFromStart = currTime - measureStart;
		return distanceFromStart / beatLength;
	}

	private processVolume(beat: number, voiceOff: boolean, pitchIndexOfNote?: number) {
		if (voiceOff)
			return 0;
		...
		if (this.nextVolume !== undefined) {
			...
			else if (parseInt(barBeat as any, 10) === barBeat)
				volume = pitchStressBeatDown;
			else
				volume = pitchStressBeatUp;
		}
	...
	private findNoteModifications(elem: any, velocity: number) {
		var ret: any = {};
		if (elem.decoration) {
	...
	private doModifiedNotes(noteModification: string, p: any) {
		var start = p.start;
	...
	private writeNote(elem: any, voiceOff: boolean) {
		...
		if (elem.pitches) {
			...
			for (var i = 0; i < ePitches.length; i++) {
				...
				var p: any = { cmd: 'note', pitch: actualPitch, volume: pitchVelocity, start: this.timeToRealTime(elem.time), duration: this.durationRounded(note.duration), instrument: this.currentInstrument, startChar: elem.elem.startChar, endChar: elem.elem.endChar };
	...
	private getRealDuration(elem: any) {
		...
		if (note.accidental) {
			switch (note.accidental) {
				...
				case "dblflat":
					this.barAccidentals[pitch] = -2; break;
				case "quartersharp":
					this.barAccidentals[pitch] = 0.25; break;
				case "quarterflat":
					this.barAccidentals[pitch] = -0.25; break;
			}
		}
	...
	private setKeySignature(elem: any) {
	...
	private processGraceNotes(graces: any[], companionDuration: number) {
		var graceDuration = 0;
		var ret = [];
		var grace;
		...
		for (g = 0; g < graces.length; g++) {
			...
			var pitch: any = { pitch: actualPitch, duration: grace.duration * multiplier };
	...
	private writeGraceNotes(graces: any[], start: number, velocity: number, currentInstrument: any) {
		var midiGrace = [];
	...
	private adjustForMicroTone(description: any) {
	...
	private extractOctave(pitch: number) {
		return Math.floor(pitch / 7);
	}

	private extractNote(pitch: number) {
		pitch = pitch % 7;
		if (pitch < 0) pitch += 7;
		return pitch;
	}
	...
	private normalizeDrumDefinition(params: any) {
		...
		var ret: any = { on: true, bars: params.bars, pattern: [] };
		...
		for (var j = 0; j < events.length; j++) {
			...
			var div = false;
			var num = 0;
			for (var k = 1; k < (event as string).length; k++) {
				switch ((event as string)[k]) {
					case "/":
						if (num !== 0)
							len *= num;
						num = 0;
					...
					case "1": case "2": case "3": case "4": case "5": case "6": case "7": case "8": case "9": case "0":
						num = num * 10 + parseInt((event as string)[k], 10);
						break;
					default:
						return { on: false };
				}
	...
	private writeDrum(channel: number) {
	...
	private findOctaves(tracks: any[][], detuneCents: number) {
		var timing: any = {};
...
export default function flatten(voices: any[], options: any, percmap_: any, midiOptions: any) {
