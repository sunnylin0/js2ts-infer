...
export default function createNoteMap(sequence: any): NoteMapEvent[][] {
	const map: NoteMapEvent[][] = [];
	for (let i = 0; i < sequence.tracks.length; i++)
		map.push([]);

	let currentInstrument = instrumentIndexToName[0];
	sequence.tracks.forEach((track: any[], i: number) => {
