...
export default function placeNote(
	...
	return noteBufferPromise
		.then((response: any) => {
			...
			const volumesMultiplier = (sound.volume / 96) * volumeMultiplier;
			const gainNode = offlineCtx.createGain();

			let panNode: StereoPannerNode | undefined;
			if (sound.pan && (offlineCtx as any).createStereoPanner) {
			...
			if ((source as any).noteOff) {
				(source as any).noteOff(len + fadeTimeSec);
			} else {
			...
			return new Promise<void>((resolve) => {
				offlineCtx.oncomplete = (e) => {
					if (e.renderedBuffer && (e.renderedBuffer as any).getChannelData) {
						for (let i = 0; i < startArray.length; i++) {
							let start = startArray[i] * sound.tempoMultiplier;
							if (ofsMs)
								start -= ofsMs / 1000;
							if (start < 0)
								start = 0;
...
function copyToChannel(toBuffer: AudioBuffer, fromBuffer: AudioBuffer, start: number): void {
	for (let ch = 0; ch < 2; ch++) {
		...
		for (let n = 0; n < fromData.length; n++) {
