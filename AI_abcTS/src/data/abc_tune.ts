...
import { Formatting, Media, MetaText, MetaTextInfo, Meter, NoteTimingEvent, TuneLine } from 'abcjs';
...
export default class Tune {
	...
	public media: Media = "screen";
	public metaText: MetaText = {};
	public metaTextInfo: MetaTextInfo = {} as MetaTextInfo;
	public formatting: Formatting = {} as Formatting;
	public lines: TuneLine[] = [];
	public staffNum: number = 0;
	public voiceNum: number = 0;
	public lineNum: number = 0;
	public runningFonts: any = {};
	public visualTranspose?: number;
	public engraver: any;
	public meter: Meter | any;
	public totalTime!: number;
	public totalBeats!: number;
	public tempoLocations: { [key: number]: number } = {};
	public noteTimings!: NoteTimingEvent[];
	public potentialStartBeam: any;
	public potentialEndBeam: any;
	public vskipPending: any;
	public chordGrid?: any[];
	...
	public reset() {
		...
		this.media = "screen";
		this.metaText = {};
		this.metaTextInfo = {} as any;
		this.formatting = {} as any;
		this.lines = [];
		this.staffNum = 0;
		this.voiceNum = 0;
		this.lineNum = 0;
	...
	private copy(dest, src, prop, attrs) {
		for (var i = 0; i < attrs.length; i++)
			dest[prop][attrs[i]] = src[prop][attrs[i]];
	}

	public copyTopInfo(src: Tune) {
		var attrs = ['tempo', 'title', 'header', 'rhythm', 'origin', 'composer', 'author', 'partOrder'] as const;
		this.copy(this, src, "metaText", attrs as any);
		this.copy(this, src, "metaTextInfo", attrs as any);
	};

	public copyBottomInfo(src: Tune) {
	...
	private computePickupLength(lines, barLength) {
		var pickupLength = 0;
	...
	public getPickupLength() {
		var barLength = this.getBarLength();
		var pickupLength = this.computePickupLength(this.lines, barLength);
	...
	public getBarLength(): number {
		var meter = this.getMeterFraction();
		return (meter.num as number) / (meter.den as number);
	};
	...
	public millisecondsPerMeasure(bpmOverride?: number): number {
		var bpm;
		if (bpmOverride) {
			bpm = bpmOverride;
		} else {
			var tempo = this.metaText ? this.metaText.tempo : null;
			bpm = this.getBpm(tempo as any);
		}
		if (bpm <= 0)
	...
	public getMeterFraction(): { num: number, den: number } {
		var meter = this.getMeter();
		var num = 4;
		var den = 4;
		if (meter) {
			if (meter.type === 'specified') {
				if (meter.value && meter.value.length > 0 && (meter.value[0].num as any).indexOf('+') > 0) {
					var parts = (meter.value[0].num as any).split('+')
					num = 0;
					for (var i = 0; i < parts.length; i++)
						num += parseInt(parts[i], 10)
				} else
					num = parseInt(meter.value[0].num as any, 10);
				den = parseInt(meter.value[0].den as any, 10);
	...
	public getKeySignature(): any {
		for (var i = 0; i < this.lines.length; i++) {
			var line = this.lines[i] as any;
			if (line.staff) {
				for (var j = 0; j < line.staff.length; j++) {
					if (line.staff[j].key)
	...
	public getElementFromChar(char) {
		for (var i = 0; i < this.lines.length; i++) {
			var line = this.lines[i];
			if (line.staff) {
				for (var j = 0; j < line.staff.length; j++) {
	...
	private addVerticalInfo(timingEvents) {
	...
	public addElementToEvents(eventHash: any, element: any, voiceTimeMilliseconds: number, top: number, height: number, line: number, measureNumber: number, timeDivider: number, isTiedState: any, nextIsBar: boolean) {
		if (element.hint)
	...
	public setupEvents(startingDelay: number, timeDivider: number, bpm: number, warp?: number): any[] {
		if (!warp) warp = 1;
		var timingEvents = [];
	...
	public addUsefulCallbackInfo(timingEvents: any[], bpm: number) {
	...
	private skipTies(elements, index) {
		while (index < elements.length && elements[index].left === null)
			index++;
		return elements[index];
	}
	private addEndPoints(lines, elements) {
	...
	public getBpm(tempo: any): number {
		var bpm;
		if (!tempo)
	...
	public setTiming(bpm?: number, measuresOfDelay?: number) {
