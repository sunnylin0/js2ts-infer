...
const clefLines: { [key: string]: { clef: string; pitch?: number; mid: number } } = {
...
function calcMiddle(clef: string, oct: number): number {
	const value = clefLines[clef];
	const mid = value ? value.mid : 0;
	return mid + oct;
}

function parseMiddle(str: string): { mid: number; str: string } {
	let i = 0;
	let p = str[i++];
	if (p === '^' || p === '_')
		p = str[i++];
...
function normalizeAccidentals(accs: any[]): void {
...
const parseKeyVoice = {
	...
	standardKey: function (keyName: string, root: string, acc: string, localTranspose: number) {
		return transpose.keySignature(multilineVars, keyName, root, acc, localTranspose);
	},

	fixClef: function (clef: any) {
	...
	deepCopyKey: function (key: any) {
		const ret: any = { accidentals: [], root: key.root, acc: key.acc, mode: key.mode };
		if (key.accidentals) {
			key.accidentals.forEach(function (k: any) {
	...
	addPosToKey: function (clef: any, key: any) {
		const mid = clef.verticalPos;
		key.accidentals.forEach(function (acc: any) {
			let pitch = pitches[acc.note];
			pitch = pitch - mid;
			acc.verticalPos = pitch;
		});
		if (key.impliedNaturals)
			key.impliedNaturals.forEach(function (acc: any) {
				let pitch = pitches[acc.note];
				pitch = pitch - mid;
				acc.verticalPos = pitch;
			});
	...
	fixKey: function (clef: any, key: any) {
		const fixedKey = Object.assign({}, key);
		parseKeyVoice.addPosToKey(clef, fixedKey);
		return fixedKey;
	},

	parseKey: function (str: string, isInline: boolean) {
		if (str.length === 0) {
			str = 'none';
		}
	...
	parseVoice: function (line: string, i: number, e: number) {
		...
		const staffInfo: any = { startStaff: isNew };
		const addNextTokenToStaffInfo = function (name: string) {
		...
		const addNextTokenToVoiceInfo = function (vid: string, name: string, type: string) {
			const attr = tokenizer.getVoiceToken(line, start, end);
			if (attr.warn !== undefined)
				warn("Expected value for " + name + " in voice: " + attr.warn, line, start);
			else if (attr.err !== undefined)
		...
		const getNextToken = function (name: string, type: string) {
			const attr = tokenizer.getVoiceToken(line, start, end);
			if (attr.warn !== undefined)
				warn("Expected value for " + name + " in voice: " + attr.warn, line, start);
			else if (attr.err !== undefined)
		...
		const addNextNoteTokenToVoiceInfo = function (vid: string, name: string) {
			const noteToTransposition: any = {
				"_B": 2,
				"_E": 9,
				"_b": -10,
				"_e": -3
		...
		if (staffInfo.name) multilineVars.voices[id].name = staffInfo.name;
		if (staffInfo.subname) multilineVars.voices[id].subname = staffInfo.subname;
		...
		if (currentVoice) {
			if (currentVoice.index === undefined) {
				...
				if (staffInfo.verticalPos) clef2.verticalPos = staffInfo.verticalPos;
				if (staffInfo.bracket) multilineVars.staves[staffNum].bracket = staffInfo.bracket;
				if (staffInfo.brace) multilineVars.staves[staffNum].brace = staffInfo.brace;
