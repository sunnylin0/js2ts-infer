...
export default class Tokenizer {
	...
	public lines: string[];
	public multilineVars: any;

	constructor(lines: string[], multilineVars: any) {
		this.lineIndex = 0;
		this.lines = lines;
		this.multilineVars = multilineVars;
	}

	public skipWhiteSpace(str: string): number {
	...
	private finished(str: string, i: number): boolean {
		return i >= str.length;
	}

	public eatWhiteSpace(line: string, index: number): number {
		let i = index;
		for (; i < line.length; i++) {
	...
	public getKeyPitch(str: string): { len: number; token?: string } {
		const i = this.skipWhiteSpace(str);
	...
	public getSharpFlat(str: string): { len: number; token?: string } {
		if (str === 'bass')
			return { len: 0 };
		switch (str[0]) {
	...
	public getMode(str: string): { len: number; token?: string } {
		...
		if (firstThree.length > 1 && (firstThree[1] === ' ' || firstThree[1] === '^' || firstThree[1] === '_' || firstThree[1] === '=')) {
			firstThree = firstThree[0];
		}
		switch (firstThree) {
	...
	public getClef(str: string, bExplicitOnly?: boolean): { len: number; token?: string; explicit?: boolean; warn?: string } {
		const strOrig = str;
	...
	public getBarLine(line: string, i: number): { len: number; token?: string; warn?: string } {
		switch (line[i]) {
			case ']':
				++i;
				switch (line[i]) {
	...
	public getTokenOf(str: string, legalChars: string): { len: number; token: string } {
		for (let i = 0; i < str.length; i++) {
			if (legalChars.indexOf(str[i]) < 0)
				return { len: i, token: str.substring(0, i) };
		}
		return { len: str.length, token: str };
	}

	public getToken(str: string, start: number, end: number): string {
		let i = start;
	...
	public isMatch(str: string, match: string): number {
		const i = this.skipWhiteSpace(str);
		if (this.finished(str, i))
			return 0;
	...
	public getPitchFromTokens(tokens: any[]): any {
		const ret: any = {};
		const pitches: { [key: string]: number } = { A: 5, B: 6, C: 0, D: 1, E: 2, F: 3, G: 4, a: 12, b: 13, c: 7, d: 8, e: 9, f: 10, g: 11 };
	...
	public getKeyAccidentals2(tokens: any[]): any {
		let accs: any[] | undefined;
	...
	public getKeyAccidental(str: string): any {
	...
	public getMeat(line: string, start: number, end: number): { start: number; end: number } {
	...
	public tokenize(line: string, start: number, end: number, alphaUntilWhiteSpace: boolean): any[] {
		const ret = this.getMeat(line, start, end);
		start = ret.start;
		end = ret.end;
		const tokens: any[] = [];
		let i;
		while (start < end) {
			if (line[start] === '"') {
				...
				i++;
			} else if (this.isLetter(line[start])) {
				i = start + 1;
				if (alphaUntilWhiteSpace)
					while (i < end && !this.isWhiteSpace(line[i])) i++;
				else
					while (i < end && this.isLetter(line[i])) i++;
				tokens.push({ type: 'alpha', token: line.substring(start, i), continueId: this.isNumber(line[i]), start: start, end: i });
				start = i + 1;
			} else if (line[start] === '.' && this.isNumber(line[start + 1])) {
				i = start + 1;
				let float2: number | null = null;
				while (i < end && this.isNumber(line[i])) i++;

				float2 = parseFloat(line.substring(start, i));
				tokens.push({ type: 'number', token: line.substring(start, i), intt: null, floatt: float2, continueId: this.isLetter(line[i]), start: start, end: i });
				start = i + 1;
			} else if (this.isNumber(line[start]) || (line[start] === '-' && this.isNumber(line[start + 1]))) {
				i = start + 1;
				let intt: number | null = null;
				let floatt: number | null = null;
				while (i < end && this.isNumber(line[i])) i++;
				if (line[i] === '.' && this.isNumber(line[i + 1])) {
					i++;
					while (i < end && this.isNumber(line[i])) i++;
				} else {
				...
				tokens.push({ type: 'number', token: line.substring(start, i), intt: intt, floatt: floatt, continueId: this.isLetter(line[i]), start: start, end: i });
				start = i + 1;
	...
	public getVoiceToken(line: string, start: number, end: number): any {
		let i = start;
	...
	public translateString(str: string): string {
		...
		const charMap1: { [key: string]: string } = {
			"#": "♯",
			"b": "♭",
			"=": "♮"
		};
		const charMap2: { [key: string]: string } = {
			"201": "♯", "202": "♭", "203": "♮", "241": "¡",
	...
	public getNumber(line: string, index: number): { num: number; index: number } {
		let num = 0;
	...
	public getFraction(line: string, index: number): { value: number; index: number } {
		let num = 1;
		let den = 1;
		if (line[index] !== '/') {
			const ret = this.getNumber(line, index);
			num = ret.num;
			index = ret.index;
		}
		if (line[index] === '/') {
			...
			if (line[index] === '/') {
				...
				const iSave = index;
				const ret2 = this.getNumber(line, index);
	...
	public theReverser(str: string): string {
		const thePatterns = [
		...
		const getTitleNumber = function (s: string) {
			const regex = /^(\d+)\./;
		...
		for (let i = 0; i < thePatterns.length; i++) {
			const thisPattern = thePatterns[i];
			const match = str.match(thisPattern.match);
			if (match) {
				const theTitleNumber = getTitleNumber(str);
				if (theTitleNumber) {
					str = str.replace(theTitleNumber + ".", "");
					str = str.trim();
				}
				const len = match[0].length;
				let result = thisPattern.replace + str.substring(0, str.length - len);
	...
	public stripComment(str: string): string {
		const i = str.indexOf('%');
		if (i >= 0)
	...
	public getInt(str: string): { value?: number; digits: number } {
	...
	public getFloat(str: string): { value?: number; digits: number } {
	...
	public getMeasurement(tokens: any[]): { used: number; value?: number } {
	...
	public getBrackettedSubstring(line: string, i: number, maxErrorChars: number, _matchChar?: string): [number, string, boolean] {
		const substInChord = function (str: string) {
			str = str.replace(/\\n/g, "\n");
			str = str.replace(/\\"/g, '"');
			return str;
		};
	...
	public nextLine(): string | null {
		...
		if (this.lineIndex < this.lines.length) {
			const result = this.lines[this.lineIndex];
			this.lineIndex++;
			return result;
		}
		return null;
