...
export function wrapLines(tune: any, lineBreaks: number[][] | undefined, barNumbers: number | undefined): void {
...
function addLineBreaks(lines: any[], linesBreakElements: any[], barNumbers: number | undefined): any[] {
	const outputLines: any[] = [];
	const lastKeySig: any[] = [];
	const lastStem: any[] = [];
			...
			for (let kk = currVoice.length - 1; kk >= 0; kk--) {
				if (currVoice[kk].el_type === "key") {
					lastKeySig[action.staff] = {
						...
						accidentals: currVoice[kk].accidentals.filter(function (acc: any) { return acc.acc !== 'natural'; })
					};
					break;
				}
			}
...
function findLineBreaks(lines: any[], lineBreakArray: number[][]): any[] {
	const lineBreakIndexes: any[] = [];
	let lbai = 0;
	let lineCounter = 0;
	let outputLine = 0;
...
function freeFormLineBreaks(widths: number[], lineBreakPoint: number): { lineBreaks: number[], totals: number[] } {
...
function oneTry(measureWidths: number[], idealWidths: number[], accumulator: number, lineAccumulator: number, lineWidths: number[], lastVariance: number, highestVariance: number, currLine: number, lineBreaks: number[], startIndex: number, otherTries: any[]): void {
...
function optimizeLineWidths(widths: any, lineBreakPoint: number, lineBreaks: number[], explanation: any): { failed: boolean, lineBreaks: number[], variance: number } {
...
function fixedMeasureLineBreaks(widths: number[], lineBreakPoint: number, preferredMeasuresPerLine: number): { failed: boolean, totals: number[], lineBreaks: number[] } {
	const lineBreaks: number[] = [];
	const totals: number[] = [];
	let thisWidth = 0;
	let failed = false;
...
function getRevisedTuneParams(lineBreaks: any[], staffWidth: number, params: any): { revisedParams: any } {
	const revisedParams: any = {
...
export function calcLineWraps(tune: any, widths: any[], params: any): any {
	...
	const accumulatedLineBreaks: any[] = [];
	...
	for (let s = 0; s < widths.length; s++) {
		...
		if (!lineBreaks) {
			let ff = freeFormLineBreaks(section.measureWidths, lineBreakPoint) as any;
	...
	const ret: any = getRevisedTuneParams(accumulatedLineBreaks, staffWidth, params);
