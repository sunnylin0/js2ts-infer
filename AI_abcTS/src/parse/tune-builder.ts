...
export default class TuneBuilder {
	private voiceDefs: { [key: string]: any } = {};
	...
	public setVisualTranspose(visualTranspose: number): void {
		if (visualTranspose !== undefined)
			this.tune.visualTranspose = visualTranspose;
	}

	public cleanUp(barsperstaff: number, staffnonote: boolean, currSlur: any): any {
		closeLine(this.tune);
				...
				for (s = 0; s < this.tune.lines[i].staff.length; s++) {
					if (this.tune.lines[i].staff[s] === undefined) {
						...
						for (v = 0; v < this.tune.lines[i].staff[s].voices.length; v++) {
							...
							else if (containsNotes(this.tune.lines[i].staff[s].voices[v])) hasAny = true;
						}
					}
				}
				if (!hasAny) {
		...
		if (anyDeleted) {
			this.tune.lines = this.tune.lines.filter(function (line: any) { return !!line; });
			this.tune.lines.forEach(function (line: any) {
				if (line.staff)
					line.staff = line.staff.filter(function (l: any) { return !!l; });
			});
		}
		...
		if (staffnonote) {
			...
			if (anyDeleted) {
				this.tune.lines.forEach(function (line: any) {
					if (line.staff)
						line.staff = line.staff.filter(function (staff: any) { return !!staff; });
				});
			}
		}
	...
	public addTieToLastNote(dottedTie: boolean): boolean {
	...
	public appendElement(type: string, startChar: number | null, endChar: number | null, hashParams: any): boolean {
	...
	public appendStartingElement(type: string, startChar: number | null, endChar: number | null, hashParams2: any): void {
		closeLine(this.tune);

		let impliedNaturals: any;
		if (type === 'key') {
		...
		if (!this.tune.lines[this.tune.lineNum]) return;
	...
	public addSubtitle(str: string, info: any): void {
		pushLine(this.tune, { subtitle: { text: str, startChar: info.startChar, endChar: info.endChar } });
	}

	public addSpacing(num: number): void {
		this.tune.vskipPending = num;
	}

	public addNewPage(num: number): void {
		pushLine(this.tune, { newpage: num });
	}

	public addSeparator(spaceAbove: number, spaceBelow: number, lineLength: number, info: any): void {
	...
	public startNewLine(params: any): void {
		...
		if (this.tune.lines[this.tune.lineNum] === undefined) createLine(this.tune, params);
		else if (this.tune.lines[this.tune.lineNum].staff === undefined) {
			this.tune.lineNum++;
			this.startNewLine(params);
		} else if (this.tune.lines[this.tune.lineNum].staff[this.tune.staffNum] === undefined) createStaff(this.tune, params);
		else if (this.tune.lines[this.tune.lineNum].staff[this.tune.staffNum].voices[this.tune.voiceNum] === undefined) createVoice(this.tune, params);
	...
	public setRunningFont(type: string, font: any): void {
		this.tune.runningFonts[type] = font;
	}

	public setBarNumberImmediate(barNumber: number): number {
	...
	public isFirstLine(index: number): boolean {
		for (let i = index - 1; i >= 0; i--) {
			if (this.tune.lines[i].staff !== undefined) return false;
		}
		return true;
	}

	public getCurrentVoice(): any {
	...
	public setCurrentVoice(staffNum: number, voiceNum: number, name: string): boolean {
		...
		for (; i < this.tune.lines.length; i++) {
			if (this.tune.lines[i].staff) {
				if (this.tune.lines[i].staff[staffNum] === undefined || this.tune.lines[i].staff[staffNum].voices[voiceNum] === undefined ||
					...
					this.tune.lineNum = i;
					if (!this.tune.lines[i].staff[staffNum] || !!this.tune.lines[i].staff[staffNum].voices[voiceNum]) return true;
					return false;
				}
			}
		}
	...
	public addMetaText(key: string, value: any, info: any): void {
	...
	public addMetaTextArray(key: string, value: any, info: any): void {
		if (this.tune.metaText[key] === undefined) {
			this.tune.metaText[key] = [value];
			this.tune.metaTextInfo[key] = info;
		} else {
	...
	public addMetaTextObj(key: string, value: any, info: any): void {
		this.tune.metaText[key] = value;
		this.tune.metaTextInfo[key] = info;
	}
}
...
function simplifyMetaText(tune: any): void {
...
function resolveOverlays(tune: any): boolean {
						...
						if (event.el_type === "overlay" && !inOverlay) {
							...
							for (let ii = 0; ii < i; ii++) {
								if (tune.lines[ii].staff) {
									tune.lines[ii].staff.forEach((s: any) => {
										if (staff.voices.length >= s.voices.length) {
											s.voices.forEach((v: any) => {
												let nv: any[] = [];
												v.forEach((ev: any) => {
													if (ev.el_type === "bar") {
														nv.push(ev);
...
function findLastBar(voice: any[], start: number): number {
	let i = start - 1;
...
function fixTitles(lines: any[]): void {
...
function cleanUpSlursInLine(line: any[], staffNum: number, voiceNum: number, currSlur: any): void {
	...
	const addEndSlur = function (obj: any, num: number, chordPos: number) {
		if (currSlur[staffNum][voiceNum][chordPos] === undefined) {
			...
			if (currSlur[staffNum][voiceNum][chordPos] === undefined) {
				...
				obj.endSlur.forEach(function (xs: any) { if (offNum === xs) --offNum; });
	...
	const addStartSlur = function (obj: any, num: number, chordPos: number, usedNums?: any[]) {
		obj.startSlur = [];
		if (currSlur[staffNum][voiceNum][chordPos] === undefined) {
		...
		for (let i = 0; i < num; i++) {
			if (usedNums) {
				usedNums.forEach(function (xs) { if (nextNum === xs) ++nextNum; });
				usedNums.forEach(function (xs) { if (nextNum === xs) ++nextNum; });
				usedNums.forEach(function (xs) { if (nextNum === xs) ++nextNum; });
			}
			currSlur[staffNum][voiceNum][chordPos].forEach(function (xs: any) { if (nextNum === xs) ++nextNum; });
			currSlur[staffNum][voiceNum][chordPos].forEach(function (xs: any) { if (nextNum === xs) ++nextNum; });
...
function wrapMusicLines(lines: any[], barsperstaff: number): boolean {
	for (let i = 0; i < lines.length; i++) {
		if (lines[i].staff !== undefined) {
			for (let s = 0; s < lines[i].staff.length; s++) {
				const permanentItems: any[] = [];
...
function getPrevMusicLine(lines: any[], currentLine: number): any {
...
function getNextMusicLine(lines: any[], currentLine: number): any {
	currentLine++;
...
function getLastNote(tune: any): any {
...
function closeLine(tune: any): void {
...
function containsNotes(voice: any[]): boolean {
	for (let i = 0; i < voice.length; i++) {
		if (voice[i].el_type === 'note' || voice[i].el_type === 'bar')
			return true;
	}
...
function containsNotesStrict(voice: any[]): boolean {
	for (let i = 0; i < voice.length; i++) {
...
function pushLine(tune: any, hash: any): void {
...
function pushNote(tune: any, hp: any, voiceDefs: any, currentVoiceName: string): void {
...
function endBeamHere(hashParams: any, tune: any): void {
	tune.potentialStartBeam.startBeam = true;
...
function endBeamLast(tune: any): void {
...
function setLineFont(tune: any, type: string, font: any): void {
...
function createVoice(tune: any, params: any): void {
...
function createStaff(tune: any, params: any): void {
...
function createLine(tune: any, params: any): void {
	tune.lines[tune.lineNum] = { staff: [] };
	createStaff(tune, params);
}

function voiceUseful(lines: any[], voiceNum: number): boolean | 'not-found' {
	let isUseful = false;
...
function deleteVoice(lines: any[], voiceNum: number): void {
	for (let line = 0; line < lines.length; line++) {
		const staves = lines[line].staff;
		if (staves) {
			for (let s = 0; s < staves.length; s++) {
...
function appendElementHelper(tune: any, type: string, startChar: number | null, endChar: number | null, hashParams: any): void {
	hashParams.el_type = type;
