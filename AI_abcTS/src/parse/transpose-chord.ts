const sharpChords: string[] = ['C', 'C♯', 'D', "D♯", 'E', 'F', "F♯", 'G', 'G♯', 'A', 'A♯', 'B'];
const flatChords: string[] = ['C', 'D♭', 'D', 'E♭', 'E', 'F', 'G♭', 'G', 'A♭', 'A', 'B♭', 'B'];
const sharpChordsFree: string[] = ['C', 'C#', 'D', "D#", 'E', 'F', "F#", 'G', 'G#', 'A', 'A#', 'B'];
const flatChordsFree: string[] = ['C', 'Db', 'D', 'Eb', 'E', 'F', 'Gb', 'G', 'Ab', 'A', 'Bb', 'B'];
...
function transposeChordName(chord: string, steps: number, preferFlats: boolean, freeGCchord: boolean): string {
	...
	let index = sharpChords.indexOf(name);
	...
	const isDim = extra1 && (extra1.indexOf('dim') >= 0 || extra1.indexOf('°') >= 0);
	...
	if (bass) {
		let bassIndex = sharpChords.indexOf(bass);
		if (bassIndex < 0) bassIndex = flatChords.indexOf(bass);
		if (bassIndex < 0) bassIndex = sharpChordsFree.indexOf(bass);
		if (bassIndex < 0) bassIndex = flatChordsFree.indexOf(bass);
		...
		if (bassIndex >= 0) {
			...
			if (preferFlats) {
				if (freeGCchord) chord += flatChordsFree[bassIndex];
				else chord += flatChords[bassIndex];
			} else {
				if (freeGCchord) chord += sharpChordsFree[bassIndex];
				else chord += sharpChords[bassIndex];
			}
		} else {
