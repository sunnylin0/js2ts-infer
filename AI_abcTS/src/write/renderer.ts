...
class Renderer {
	...
	public space: number;
	public padding: Record<string, number> = {}; // renderer's padding is managed by the controller
	public firefox112: boolean;
	public y: number = 0;
	public abctune: any = null;
	public path: any = null;
	public isPrint: boolean = false;
	public lineThickness: number = 0;
	public spacing: Record<string, number> = {};
	public paddingOverride?: Record<string, number>;
	public showDebug?: any;
	public foregroundColor?: string;
	public ariaLabel?: string;
	public minPadding: number = 0;

	constructor(paper: HTMLElement) {
		this.paper = new Svg(paper);
		this.space = 3 * spacing.SPACE;
		this.reset();
		this.firefox112 = typeof navigator !== "undefined" && navigator.userAgent.indexOf("Firefox/112.0") >= 0;
	}
	...
	public newTune(abcTune: any): void {
	...
	public setPaddingOverride(params: any): void {
	...
	public setPadding(abctune: any): void {
		...
		const setPaddingVariable = (paddingKey: string, formattingKey: string, printDefault: number, screenDefault: number) => {
			if (abctune.formatting[formattingKey] !== undefined) this.padding[paddingKey] = abctune.formatting[formattingKey];
			else if (this.paddingOverride && this.paddingOverride[paddingKey] !== undefined) this.padding[paddingKey] = this.paddingOverride[paddingKey];
			else if (this.isPrint) this.padding[paddingKey] = printDefault;
			else this.padding[paddingKey] = screenDefault;
		};
	...
	public adjustNonScaledItems(scale: number): void {
		...
		if (this.abctune && this.abctune.formatting) {
			if (this.abctune.formatting.headerfont) this.abctune.formatting.headerfont.size /= scale;
			if (this.abctune.formatting.footerfont) this.abctune.formatting.footerfont.size /= scale;
		}
	}
	...
	public setVerticalSpace(formatting: any): void {
		// conversion from pts to px 4/3
		const f = (val: number | undefined) => (val !== undefined ? (val * 4) / 3 : undefined);
		if (formatting.staffsep !== undefined) this.spacing.staffSeparation = f(formatting.staffsep)!;
		if (formatting.composerspace !== undefined) this.spacing.composer = f(formatting.composerspace)!;
		if (formatting.partsspace !== undefined) this.spacing.parts = f(formatting.partsspace)!;
		if (formatting.textspace !== undefined) this.spacing.text = f(formatting.textspace)!;
		if (formatting.musicspace !== undefined) this.spacing.music = f(formatting.musicspace)!;
		if (formatting.titlespace !== undefined) this.spacing.title = f(formatting.titlespace)!;
		if (formatting.sysstaffsep !== undefined) this.spacing.systemStaffSeparation = f(formatting.sysstaffsep)!;
		if (formatting.stafftopmargin !== undefined) this.spacing.staffTopMargin = f(formatting.stafftopmargin)!;
		if (formatting.subtitlespace !== undefined) this.spacing.subtitle = f(formatting.subtitlespace)!;
		if (formatting.topspace !== undefined) this.spacing.top = f(formatting.topspace)!;
		if (formatting.vocalspace !== undefined) this.spacing.vocal = f(formatting.vocalspace)!;
		if (formatting.wordsspace !== undefined) this.spacing.words = f(formatting.wordsspace)!;
	}
	 ...
	 */
	public calcY(ofs: number): number {
		return this.y - ofs * spacing.STEP;
	}

	public yToPitch(ofs: number): number {
		return ofs / spacing.STEP;
	}

	public moveY(em: number, numLines: number = 1): void {
		this.y += em * numLines;
	}

	public absolutemoveY(y: number): void {
		this.y = y;
	}
}
