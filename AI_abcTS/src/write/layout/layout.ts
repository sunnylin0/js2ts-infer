 ...
 */
const layout = function (renderer: any, abctune: any, width: number, space: number, expandToWidest: boolean, timeBasedLayout?: any): number {
	let i: number;
	let abcLine: any;
	let maxWidth = width;
 ...
 */
const setXSpacing = function (renderer: any, width: number, space: number, staffGroup: any, formatting: any, isLastLine: boolean, debug: boolean): number {
...
function calcHorizontalSpacing(isLastLine: boolean, stretchLast: number | undefined, targetWidth: number, lineWidth: number, spacing: number, spacingUnits: number, minSpace: number, padding: number): number | null {
	if (isLastLine) {
...
function centerWholeRests(voices: any[]): void {
