...
function layoutInGrid(renderer: any, staffGroup: any, timeBasedLayout: any): number {
...
function getTotalDuration(staffGroup: any, minPadding: number): { totalDuration: number, minSpacing: number } {
	let maxSpacing = 0;
	let maxCount = 0;
	for (let i = 0; i < staffGroup.voices.length; i++) {
		let count = 0;
