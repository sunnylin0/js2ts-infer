class VoiceElement {
	public static beginLayout(startx: number, voice: any): void {
		voice.i = 0;
	...
	public static layoutEnded(voice: any): boolean {
		return (voice.i >= voice.children.length);
	}

	public static getNextX(voice: any): number {
		return Math.max(voice.minx, voice.nextx);
	}

	public static getSpacingUnits(voice: any): number {
		return Math.sqrt(voice.spacingduration * 8);
	}

	public static layoutOneItem(x: number, spacing: number, voice: any, minPadding: number, firstVoice?: any): number {
	...
	public static shiftRight(dx: number, voice: any): void {
		const child = voice.children[voice.i];
		if (!child) return;
	...
	public static updateNextX(x: number, spacing: number, voice: any): void {
		voice.nextx = x + (spacing * this.getSpacingUnits(voice));
	}

	public static updateIndices(voice: any): void {
...
function getExtraWidth(child: any, minPadding: number): number {
	let padding = 0;
...
function getMinWidth(child: any): number {
	return child.w;
}
