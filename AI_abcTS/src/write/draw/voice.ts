...
import { Renderer, StaffPos } from "./type-definitions";

function drawVoice(renderer: Renderer, params: any, bartop: number, selectables: any, staffPos: StaffPos): void {
	const width = params.w - 1;
	(renderer as any).staffbottom = params.staff.bottom;
	const saveColor = renderer.foregroundColor;
	if (params.color)
		renderer.foregroundColor = params.color;

	if (params.header) { // print voice name
		const textEl = renderText(renderer, { x: renderer.padding.left, y: renderer.calcY(params.headerPosition), text: params.header, type: 'voicefont', klass: 'staff-extra voice-name', anchor: 'start', centerVertically: true, name: "voice-name" }, true);
	...
	for (i = 0; i < params.children.length; i++) {
		...
		if (child.type === 'note' || child.type === 'rest')
			foundNote = true;
		let justInitializedMeasureNumber = false;
		if (child.type !== 'staff-extra' && !(renderer.controller.classes as any).isInMeasure()) {
			(renderer.controller.classes as any).startMeasure();
		...
		if (child.type === 'note' || isNonSpacerRest(child))
			(renderer.controller.classes as any).incrNote();
		if (child.type === 'bar' && !justInitializedMeasureNumber && foundNote) {
			(renderer.controller.classes as any).incrMeasure();
		}
	}
	...
	for (i = 0; i < params.otherchildren.length; i++) {
		...
		if (child === 'bar') {
			...
			switch (child.type) {
				...
				default:
					console.log(child);
					drawAbsolute(renderer, child, params.startx + 10, selectables, staffPos);
			}
		}
	}
...
function isNonSpacerRest(elem: any): boolean {
	if (elem.type !== 'rest')
		return false;
