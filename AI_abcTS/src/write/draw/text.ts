...
function renderText(renderer: Renderer, params: any, alreadyInGroup?: boolean): any {
	let y = params.y;
	...
	if (params.phrases) {
		...
		const elem = (renderer.paper as any).richTextLine(params.phrases, params.x, params.y, params.klass, params.anchor);
		return elem;
	}

	if (params.lane) {
		const laneMargin = params.dim.font.size * 0.25;
	...
	if (params.dim) {
		...
		hash = (renderer as any).controller.getFontAndAttr.calc(params.type, params.klass);
	if (params.anchor)
	...
	if (hash.font.box) {
		if (!alreadyInGroup)
			renderer.paper.openGroup({ klass: hash.attr['class'], fill: (renderer as any).foregroundColor, "data-name": params.name });
	...
	let elem: any = renderer.paper.text(text, hash.attr);
	if (hash.font.box) {
		const size = (elem as any).getBBox();
