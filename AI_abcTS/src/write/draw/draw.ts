...
import { TuneObject, TuneLine } from "abcjs";
function draw(renderer: Renderer, classes: any, abcTune: TuneObject, width: number, maxWidth: number, responsive: string | boolean | undefined, scale: number, selectTypes: string[] | undefined, tuneNumber: number, lineOffset: number, chordGrid: boolean | "noMusic" | undefined): any {
	const selectables = new Selectables(renderer.paper, selectTypes, tuneNumber);
	...
	if (!suppressMusic) {
		for (let line = 0; line < abcTune.lines.length; line++) {
			classes.incrLine();
			const abcLine: TuneLine = abcTune.lines[line];
			if (abcLine.staff) {
				...
				const staffgroup = engraveStaffLine(renderer, abcLine.staffGroup, selectables, line);
...
function engraveStaffLine(renderer: Renderer, staffGroup: any, selectables: any, lineNumber: number): any {
	drawStaffGroup(renderer, staffGroup, selectables, lineNumber);
	const height = staffGroup.height * spacing.STEP;
	renderer.y += height;
	return staffGroup;
}

function addStaffPadding(renderer: Renderer, staffSeparation: number, lastStaffGroup: any, thisStaffGroup: any): void {
	const lastStaff = lastStaffGroup.staffs[lastStaffGroup.staffs.length - 1];
	const lastBottomLine = -(lastStaff.bottom - 2); // The 2 is because the scale goes to 2 below the last line.
	const nextTopLine = thisStaffGroup.staffs[0].top - 10; // Because 10 represents the top line.
	const naturalSeparation = nextTopLine + lastBottomLine; // This is how far apart they'd be without extra spacing
	const separationInPixels = naturalSeparation * spacing.STEP;
