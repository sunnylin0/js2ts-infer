...
function drawTie(renderer: Renderer, params: any, linestartx: number, lineendx: number, selectables: any): SVGElement[] {
...
const layout = function (params: any, lineStartX: number, lineEndX: number): void {
...
const drawArc = function (renderer: Renderer, x1: number, x2: number, pitch1: number, pitch2: number, above: boolean, klass: string, isTie: boolean, dotted: boolean): SVGPathElement {
	...
	let ret: SVGPathElement;
	if (dotted) {
		klass += ' dotted';
		const pathString2 = sprintf("M %f %f C %f %f %f %f %f %f", x1, y1,
			controlx1, controly1, controlx2, controly2, x2, y2);
		ret = renderer.paper.path({ path: pathString2, stroke: (renderer as any).foregroundColor, fill: "none", 'stroke-dasharray': "5 5", 'class': (renderer as any).controller.classes.generate(klass), "data-name": isTie ? "tie" : "slur" });
	} else {
		const pathString = sprintf("M %f %f C %f %f %f %f %f %f C %f %f %f %f %f %f z", x1, y1,
			controlx1, controly1, controlx2, controly2, x2, y2,
		...
		ret = renderer.paper.path({ path: pathString, stroke: "none", fill: (renderer as any).foregroundColor, 'class': (renderer as any).controller.classes.generate(klass), "data-name": isTie ? "tie" : "slur" });
	}
