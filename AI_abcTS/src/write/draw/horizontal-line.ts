// For debugging, it is sometimes useful to know where you are vertically.
import sprintf from "./sprintf";
import { Renderer } from "./type-definitions";

function printHorizontalLine(renderer: Renderer, width: number, vertical?: number, comment?: string): void {
	const dy = 0.35;
	const fill = "rgba(0,0,255,.4)";
	let y = renderer.y;
	if (vertical) y = vertical;
	y = Math.round(y);
	renderer.paper.text("" + Math.round(y), { x: 10, y: y, "text-anchor": "start", "font-size": "18px", fill: fill, stroke: fill });
	const x1 = 50;
	const x2 = width;
	let pathString = sprintf("M %f %f L %f %f L %f %f L %f %f z", x1, y - dy, x1 + x2, y - dy,
		x2, y + dy, x1, y + dy);
	(renderer.paper as any).pathToBack({ path: pathString, stroke: "none", fill: fill, 'class': renderer.controller.classes.generate('staff') });
	for (let i = 1; i < width / 100; i++) {
		pathString = sprintf("M %f %f L %f %f L %f %f L %f %f z", i * 100 - dy, y - 5, i * 100 - dy, y + 5,
			i * 100 + dy, y - 5, i * 100 + dy, y + 5);
		(renderer.paper as any).pathToBack({ path: pathString, stroke: "none", fill: fill, 'class': renderer.controller.classes.generate('staff') });
	}
	if (comment)
		renderer.paper.text(comment, { x: width + 70, y: y, "text-anchor": "start", "font-size": "18px", fill: fill, stroke: fill });
}

export default printHorizontalLine;
