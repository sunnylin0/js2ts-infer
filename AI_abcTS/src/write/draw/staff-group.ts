...
function drawStaffGroup(renderer: Renderer, params: any, selectables: any, lineNumber: number): void {
	...
	const startY = renderer.y; // So that it can be restored after we're done.
	// Set the absolute Y position for each staff here, so the voice drawing below can just use if.
	for (let j = 0; j < params.staffs.length; j++) {
		const staff1 = params.staffs[j];
		//renderer.printHorizontalLine(50, renderer.y, "start");
		renderer.moveY(spacing.STEP, staff1.top);
		staff1.absoluteY = renderer.y;
		if ((renderer as any).showDebug) {
			if ((renderer as any).showDebug.indexOf("box") >= 0 && staff1.voices) {
				boxAllElements(renderer, params.voices, staff1.voices);
			}
			if ((renderer as any).showDebug.indexOf("grid") >= 0) {
				(renderer.paper as any).dottedLine({ x1: renderer.padding.left, x2: renderer.padding.left + (renderer as any).controller.width, y1: startY, y2: startY, stroke: "#0000ff" });
				printDebugBox(renderer,
					{
						x: renderer.padding.left,
						y: renderer.calcY(staff1.originalTop),
						width: (renderer as any).controller.width,
				...
				colorIndex = 0;
				const debugPrintGridItem = (staff: any, key: string) => {
					const colors = ["rgb(207,27,36)", "rgb(168,214,80)", "rgb(110,161,224)", "rgb(191,119,218)", "rgb(195,30,151)",
						"rgb(31,170,177)", "rgb(220,166,142)"];
					if (staff.positionY && staff.positionY[key]) {
						let height = staff.specialY[key] * spacing.STEP;
		...
		if ((renderer as any).showDebug) {
			if ((renderer as any).showDebug.indexOf("grid") >= 0) {
				(renderer.paper as any).dottedLine({
					x1: renderer.padding.left,
					x2: renderer.padding.left + (renderer as any).controller.width,
					y1: renderer.y,
					y2: renderer.y,
					stroke: "#0000aa"
				});
	...
	let topLine: number | undefined; // these are to connect multiple staves. We need to remember where they are.
	let bottomLine: number = 0;

	let linePitch = 2;
	let bartop = 0;
	for (let i = 0; i < params.voices.length; i++) {
		const staff = params.voices[i].staff;
		const tabName = params.voices[i].tabNameInfos;
		renderer.y = staff.absoluteY;
		(renderer.controller.classes as any).incrVoice();
		//renderer.y = staff.y;
		...
		if (!params.voices[i].duplicate) {
			...
			if (staff.lines !== 0) {
				...
				(renderer.controller.classes as any).newMeasure();
				const lines = printStaff(renderer, params.startx, params.w, staff.lines, staff.linePitch, 0.35);
				bottomLine = lines[1];
		...
		if (tabName) {
			...
			nonMusic(renderer, r, selectables);
	...
	const staffSize = params.staffs.length;
	if (staffSize > 1) {
		topLine = params.staffs[0].topLine;
		bottomLine = params.staffs[staffSize - 1].bottomLine;
		printStem(renderer, params.startx, 0.6, topLine!, bottomLine, undefined, "bar");
	}
	renderer.y = startY;
}

function printBrace(renderer: Renderer, absoluteY: number, brace: any, index: number, selectables: any): void {
	if (brace) {
...
function boxAllElements(renderer: Renderer, voices: any, which: any): void {
	for (let i = 0; i < which.length; i++) {
		const children = voices[which[i]].children;
		for (let j = 0; j < children.length; j++) {
			const elem = children[j];
			const coords = elem.getFixedCoords();
			if (elem.invisible || coords.t === undefined || coords.b === undefined)
				continue;
			const height = (coords.t - coords.b) * spacing.STEP;
			printDebugBox(renderer,
				{
					x: coords.x,
			...
			for (let k = 0; k < elem.children.length; k++) {
				const relElem = elem.children[k];
				const chord = relElem.getChordDim();
				if (chord) {
					let y = renderer.calcY(relElem.pitch);
