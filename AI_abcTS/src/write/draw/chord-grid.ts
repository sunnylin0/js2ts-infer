...
function drawChordGrid(renderer: Renderer, parts: any[], leftMargin: number, pageWidth: number, fonts: any): void {
	const partingFont = fonts.partsfont;
		...
		switch (part.type) {
			...
			case "part":
				if (part.lines.length > 0) {
					...
					part.lines.forEach((line: any[], lineNum: number) => {
						let hasEnding = false;
						...
						line.forEach((measure, barNum) => {
							...
							if (!measure.noBorder) {
								...
								if (measure.ending) {
									...
									endingWidth = (endingEl as any).getBBox().width + 4;
								}
...
function drawPercent(renderer: Renderer, x: number, y: number, offset: number): void {
	...
	const lineY2 = y - 10;
	const leftDotX = x - 10;
	const leftDotY = -renderer.yToPitch(offset) + 2;
	const rightDotX = x + 6.5;
	const rightDotY = -renderer.yToPitch(offset) - 2.3;
...
const symbolsMapping: Record<string, string> = {
	'segno': "scripts.segno",
	'coda': "scripts.coda",
	"fermata": "scripts.ufermata",
};

function drawAnnotations(renderer: Renderer, offset: number, left: number, annotations: string[], annotationFont: any): void {
	left += 3;
	let el: any;
...
function drawMeasure(renderer: Renderer, offset: number, leftMargin: number, colWidth: number, lineNum: number, barNum: number, chords: string[], chordFont: any, margin: number, height: number, extraTop: number): void {
	const left = leftMargin + colWidth * barNum;
...
function renderChord(renderer: Renderer, x: number, y: number, size: number, chord: string, font: any, maxWidth: number): void {
	const el = text(renderer, chord, x, y, size, font, null, "abcjs-chord", true);
	let bb = (el as any).getBBox();
	let fontSize = size;
...
const TOP_MARGIN_VAL = -3;

function drawSingleChord(renderer: Renderer, left: number, top: number, width: number, height: number, chord: string, font: any, extraTop: number): void {
	if (chord === '%')
...
function drawTwoChords(renderer: Renderer, left: number, top: number, width: number, height: number, chord1: string, chord2: string, font: any, extraTop: number): void {
...
function drawFourChords(renderer: Renderer, left: number, top: number, width: number, height: number, chords: string[], font: any, extraTop: number): void {
	const LOCAL_MARGIN = 3;
...
function shortenChord(chord: string): string {
	if (chord === "No Chord")
		return "N.C.";
	return chord;
}

function text(renderer: Renderer, str: string, x: number, y: number, size: number, font: any, dataName: string | null, klass: string | null, alignCenter: boolean): SVGElement {
	const attr: any = {
		x: x,
		y: y,
		stroke: "none",
