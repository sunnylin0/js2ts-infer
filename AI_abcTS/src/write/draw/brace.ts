...
function drawBrace(renderer: Renderer, params: any, selectables: any): SVGElement | null {
	...
	const startY = params.startVoice.staff.absoluteY - spacing.STEP * 10;
...
function straightPath(renderer: Renderer, xLeft: number, yTop: number, yBottom: number, type: string): SVGPathElement {
	xLeft += spacing.STEP;
	const xLineWidth = spacing.STEP * 0.75;
	const yOverlap = spacing.STEP * 0.75;
	const height = yBottom - yTop;
	// Straight line
	let pathString = sprintf("M %f %f l %f %f l %f %f l %f %f z",
	...
	// Top arm
	const wCurve = spacing.STEP * 2;
	const hCurve = spacing.STEP;
	...
	return renderer.paper.path({ path: pathString, stroke: (renderer as any).foregroundColor, fill: (renderer as any).foregroundColor, 'class': renderer.controller.classes.generate(type), "data-name": type });
}

function curvyPath(renderer: Renderer, xLeft: number, yTop: number, yBottom: number, type: string): SVGPathElement {
	const yHeight = yBottom - yTop;

	let pathString = curve(xLeft,
		yTop,
...
function curve(xLeft: number, yTop: number, xCurve: number[], yCurve: number[]): string {
...
const draw = (renderer: Renderer, xLeft: number, yTop: number, yBottom: number, type: string, header: string | undefined, selectables: any): SVGElement | null => {
	let ret: any;
	if (header) {
		renderer.paper.openGroup({ klass: renderer.controller.classes.generate("staff-extra voice-name"), "data-name": type });
		let position = yTop + (yBottom - yTop) / 2;
		position = position - (renderer as any).controller.getTextSize.baselineToCenter(header, "voicefont", 'staff-extra voice-name', 0, 1);
	...
	if (ret) selectables.wrapSvgEl({ el_type: type, startChar: -1, endChar: -1 }, ret);
