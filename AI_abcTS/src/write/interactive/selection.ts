...
function setupSelection(this: any, engraver: any, svgs: SVGElement[]): void {
	...
	for (let i = 0; i < svgs.length; i++) {
		svgs[i].addEventListener('touchstart', mouseDown.bind(engraver), { passive: true } as AddEventListenerOptions);
		svgs[i].addEventListener('touchmove', mouseMove.bind(engraver), { passive: true } as AddEventListenerOptions);
		svgs[i].addEventListener('touchend', mouseUp.bind(engraver), { passive: true } as AddEventListenerOptions);
...
function getCoord(ev: any): [number, number] {
	let scaleX = 1;
	let scaleY = 1;
...
function elementFocused(this: any, ev: any): void {
...
function keyboardDown(ev: any): void {
	switch (ev.keyCode) {
		case 38:
		case 40:
			ev.preventDefault();
...
function keyboardSelection(this: any, ev: any): void {
	let handled = false;
...
function findElementInHistory(selectables: any[], el: any): number {
	if (!el)
		return -1;
...
function findElementByCoord(self: any, x: number, y: number): number {
...
function getBestMatchCoordinates(dim: any, ev: any, scale: number): [number, number] {
...
function getMousePosition(self: any, ev: any): { x: number, y: number, clickedOn: number } {
	let x: number;
	let y: number;
...
function attachMissingTouchEventAttributes(touchEv: any): void {
...
function mouseDown(this: any, ev: any): void {
	let _ev = ev;
...
function mouseMove(this: any, ev: any): void {
	let _ev = ev;
...
function mouseUp(this: any, ev: any): void {
	let _ev = ev;
...
function setSelection(this: any, dragIndex: number): void {
...
function notifySelect(this: any, target: any, dragStep: number, dragMax: number, dragIndex: number, ev: any): void {
...
function clearSelection(this: any): void {
...
function rangeHighlight(this: any, start: number, end: number): void {
	clearSelection.call(this);
...
function getClassSet(el: SVGElement): Record<string, boolean> {
...
function setClassSet(el: SVGElement, klassSet: Record<string, boolean>): void {
...
function addGlobalClass(svg: any, klass: string): void {
	if (svg && svg.svg) {
...
function removeGlobalClass(svg: any, klass: string): void {
	if (svg && svg.svg) {
		const obj = getClassSet(svg.svg);
		delete obj[klass];
		setClassSet(svg.svg, obj);
