...
class BottomText {
	...
	constructor(metaText: any, width: number, isPrint: boolean, paddingLeft: number, spacing: any, shouldAddClasses: boolean, getTextSize: any) {
		this.rows = [];
	...
	public unalignedWords(unalignedWords: any[], marginLeft: number, spacing: any, shouldAddClasses: boolean, getTextSize: any): void {
	...
	public extraText(metaText: any, marginLeft: number, spacing: any, shouldAddClasses: boolean, getTextSize: any): void {
	...
	public footer(footer: any, width: number, paddingLeft: number, getTextSize: any): void {
...
function addSingleLine(rows: any[], preface: string, text: string | any[], marginLeft: number, klass: string, shouldAddClasses: boolean, getTextSize: any): void {
	if (text) {
		if (preface) {
			if (typeof text === 'string')
				text = preface + text;
			else
				text = ([{ text: preface }] as any[]).concat(text);
		}
...
function addMultiLine(rows: any[], preface: string, content: string | any[], marginLeft: number, defFont: string, absElemType: string, groupName: string, klass: string, name: string, spacing: any, shouldAddClasses: boolean, getTextSize: any): void {
	if (content) {
		const fullKlass = shouldAddClasses ? 'abcjs-extra-text ' + klass : '';
