...
const createTimeSignature = function (elem: any, tuneNumber: number): AbsoluteElement {
	...
	const abselem = new AbsoluteElement(elem, 0, 10, 'staff-extra time-signature', tuneNumber);
	if (elem.type === "specified") {
		...
		for (let i = 0; i < elem.value.length; i++) {
			...
			if (elem.value[i].den) {
				let numWidth = 0;
				for (let i2 = 0; i2 < elem.value[i].num.length; i2++)
					numWidth += glyphs.getSymbolWidth(elem.value[i].num[i2]);
				let denWidth = 0;
				for (let i2 = 0; i2 < elem.value[i].den.length; i2++)
				...
				let thisWidth = 0;
				for (let i3 = 0; i3 < elem.value[i].num.length; i3++)
