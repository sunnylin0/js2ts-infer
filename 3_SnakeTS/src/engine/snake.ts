...
export class Snake {
  ...
  constructor(gridW: number, gridH: number) {
    const midX: number = Math.floor(gridW / 2);
    const midY: number = Math.floor(gridH / 2);
  ...
  handleKeydown(e: KeyboardEvent): void {
    ...
    let log_keyname_str: string = log_keyname(dir);
   ...
   */
  pushDirection(dirName: string): void {
    ...
    // 如果是反向，直接過濾掉以防自殺
    const isOpposite: boolean = (targetDirVector.x + referenceDir.x === 0) &&
  ...
  move() {
    ...
    if (this.inputBuffer.length > 0) {
      ...
      const isOpposite: boolean = (nextDir.x + actualCurrentDir.x === 0) &&
