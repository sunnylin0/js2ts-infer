...
var el = {};
class MusicParser {
    constructor(_tokenizer, _warn, _multilineVars, _tune, _tuneBuilder, _header) {
    ...
    parseMusic(line) {
        ...
        var i = 0;
        var startOfLine = this.multilineVars.iChar;
        ...
        var delayStartNewLine = this.multilineVars.start_new_line;
        ...
        var retHeader = this.header.letter_to_body_header(line, i);
        if (retHeader[0] > 0) {
            i += retHeader[0];
        ...
        while (i < line.length) {
            ...
            if (line[i] === '%')
                break;
            var retInlineHeader = this.header.letter_to_inline_header(line, i, delayStartNewLine);
            if (retInlineHeader[0] > 0) {
                ...
                while (1) {
                    ...
                    if (ret[0] > 0) {
                        ...
                        if (!el.chord)
                            el.chord = [];
                        var chordName = this.tokenizer.translateString(ret[1]);
                        ...
                        i += ret[0];
                        var ii = this.tokenizer.skipWhiteSpace(line.substring(i));
                        if (ii > 0)
                ...
                if (ret[0] > 0) {
                    ...
                    if (line[i] === '[') {
                        ...
                        var done = false;
                        while (!done) {
                            var accent = this.#letter_to_accent(line, i);
                            if (accent[0] > 0) {
                                i += accent[0];
                            }
                                ...
                                if (i < line.length && line[i] === ']') {
                                    ...
                                    while (i < line.length && !postChordDone) {
                                        switch (line[i]) {
                                            ...
                                            case '>':
                                            case '<':
                                                var br2 = this.#getBrokenRhythm(line, i);
                        ...
                        if (core !== null) {
                            ...
                            this.multilineVars.addFormattingOptions(el, this.tune.formatting, 'note');
                            var succeeded = this.tuneBuilder.appendElement('note', startOfLine + startI, startOfLine + i, el);
                            if (!succeeded) {
                                this.startNewLine();
                                this.tuneBuilder.appendElement('note', startOfLine + startI, startOfLine + i, el);
                            }
    ...
    startNewLine() {
        ...
        var scoreTranspose = this.multilineVars.currentVoice ? this.multilineVars.currentVoice.scoreTranspose : 0;
        ...
        if (this.multilineVars.key.explicitAccidentals) {
            for (var i = 0; i < this.multilineVars.key.explicitAccidentals.length; i++) {
                var found = false;
        ...
        if (this.multilineVars.meter !== null) {
            if (this.multilineVars.currentVoice) {
                this.multilineVars.staves.forEach(function (st) {
        ...
        if (this.multilineVars.currentVoice) {
            ...
            var voices = Object.keys(this.multilineVars.voices);
    ...
    #isInTie(multilineVars, overlayLevel, el) {
    ...
    #setIsInTie(multilineVars, overlayLevel, value) {
        // If this is single voice music then the voice index isn't set, so we use the first voice.
        var voiceIndex = multilineVars.currentVoice ? multilineVars.currentVoice.staffNum * 100 + multilineVars.currentVoice.index : 0;
        if (multilineVars.inTie[overlayLevel] === undefined)
    ...
    #durationOfMeasure(multilineVars) {
    ...
    #letter_to_open_slurs_and_triplets(line, i) {
    ...
    #getCoreNote(line, index, el, canHaveBrokenRhythm) {
                ...
                case '<':
                    if (isComplete(state)) {
                        if (canHaveBrokenRhythm) {
                            var br2 = this.#getBrokenRhythm(line, index);
