...
const fontTranslation = function (fontFace) {
    switch (fontFace) {
...
const getFontParameter = function (tokens, currentSetting, str, position, cmd) {
...
const getChangingFont = function (cmd, tokens, str) {
    if (tokens.length === 0)
...
const getGlobalFont = function (cmd, tokens, str) {
    if (tokens.length === 0)
        return "Directive \"" + cmd + "\" requires a font as a parameter.";
    tune.formatting[cmd] = getFontParameter(tokens, tune.formatting[cmd], str, 0, cmd);
    return null;
};
const setScale = function (cmd, tokens) {
    let scratch = "";
...
const oneParameterMeasurement = function (cmd, tokens) {
    const points = tokenizer.getMeasurement(tokens);
    if (points.used === 0 || tokens.length !== 0)
...
const addMultilineVar = function (key, cmd, tokens, min, max) {
...
const addMultilineVarBool = function (key, cmd, tokens) {
...
const addMultilineVarOneParamChoice = function (key, cmd, tokens, choices) {
    if (tokens.length !== 1)
...
const parseMidiCommand = function (midi, tune, restOfString) {
    ...
    addDirective: function (str) {
        ...
        switch (cmd) {
            ...
            case "score":
                ...
                const addVoice = function (id, newStaff, bracket, brace, continueBar) {
                    if (newStaff || multilineVars.staves.length === 0) {
                        multilineVars.staves.push({ index: multilineVars.staves.length, numVoices: 0 });
                    }
                    const staff = parseCommon.last(multilineVars.staves);
