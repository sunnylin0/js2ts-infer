export default class ChordTrack {
    constructor(numVoices, chordsOff, midiOptions, meter) {
        ...
        this.boomVolume = 64;
        this.chickVolume = 48;
        this.breakSynonyms = ['break', '(break)', 'no chord', 'n.c.', 'tacet'];
        this.basses = {
    ...
    setMeter(meter) { this.meter = meter; }
    setTempoChangeFactor(factor) { this.tempoChangeFactor = factor; }
    setLastBarTime(time) { this.lastBarTime = time; }
    setTranspose(transpose) { this.transpose = transpose; }
    setRhythmHead(isRhythmHead, elem) {
    ...
    writeNoteCustom(pitch, beatLength, volume, beat, noteLength, instrument) {
        if (pitch !== undefined) {
    ...
    resolveChords(startTime, endTime) {
    ...
    resolvePitch(currentChord, type, firstBoom, newBass) {
        const ret = [];
        if (!currentChord)
            return ret;
        ...
        if (type.indexOf('chick') >= 0) {
            for (let i = 0; i < currentChord.chick.length; i++)
    ...
    expandCurrentChords(currentChords, num8thNotes, beatLength) {
        const chords = [];
    ...
    calcBeat(measureStart, currTime) {
        return (currTime - measureStart) * 8;
    }
    timeToRealTime(time) { return time / 1000000; }
    durationRounded(duration) { return Math.round(duration * this.tempoChangeFactor * 1000000) / 1000000; }
}
