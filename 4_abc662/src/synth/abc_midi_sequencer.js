...
export class MidiSequencer {
    constructor() {
        ...
        this.PERCUSSION_PROGRAM = 128;
    }
    sequence(abctune, options) {
        options = options || {};
        let qpm;
    ...
    setDynamics(elem, crescendoSize, inCrescendo, inDiminuendo, voiceNumber, voice, v, currentVolume, voices) {
        const volumes = {
            'pppp': [15, 10, 5, 1],
            'ppp': [30, 20, 10, 1],
            'pp': [45, 35, 20, 1],
        ...
        if (elem.decoration) {
            ...
            if (dynamicType) {
                ...
                for (let i = 0; i < currentVolume.length; i++)
    ...
    numNotesToDecoration(voice, start, decoration) {
        let counter = 0;
    ...
    endingVolume(voice, start, volumeDecorations) {
...
export default function sequence(abctune, options) {
