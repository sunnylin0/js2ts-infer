...
export default class Tune {
    ...
    ;
    computePickupLength(lines, barLength) {
        var pickupLength = 0;
    ...
    ;
    millisecondsPerMeasure(bpmOverride) {
        var bpm;
        if (bpmOverride) {
            bpm = bpmOverride;
        }
    ...
    ;
    setupEvents(startingDelay, timeDivider, bpm, warp) {
        if (!warp)
            warp = 1;
        var timingEvents = [];
        var eventHash = {};
