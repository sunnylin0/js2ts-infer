...
function straightPath(renderer, xLeft, yTop, yBottom, type) {
    ...
    // Straight line
    let pathString = sprintf("M %f %f l %f %f l %f %f l %f %f z", xLeft, yTop - yOverlap, // top left line
...
function curvyPath(renderer, xLeft, yTop, yBottom, type) {
    ...
    let pathString = curve(xLeft, yTop, [7.5, -8, 21, 0, 18.5, -10.5, 7.5], [0, yHeight / 5.5, yHeight / 3.14, yHeight / 2, yHeight / 2.93, yHeight / 4.88, 0]);
...
const draw = (renderer, xLeft, yTop, yBottom, type, header, selectables) => {
    let ret;
    if (header) {
