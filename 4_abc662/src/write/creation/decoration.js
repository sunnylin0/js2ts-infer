...
class Decoration {
    ...
    dynamicDecoration(voice, decoration, abselem, positioning) {
        let diminuendo;
        let crescendo;
        let glissando;
    ...
    createDecoration(voice, decoration, pitch, width, abselem, roomtaken, dir, minPitch, positioning, hasVocals, accentAbove) {
        if (!positioning)
...
function closeDecoration(voice, decoration, pitch, width, abselem, roomtaken, dir, minPitch, accentAbove) {
    let yPos;
    for (let i = 0; i < decoration.length; i++) {
...
function volumeDecoration(voice, decoration, abselem, positioning) {
    for (let i = 0; i < decoration.length; i++) {
        switch (decoration[i]) {
            case "p":
            case "mp":
...
function compoundDecoration(decoration, pitch, width, abselem, dir) {
    ...
    function addCompound(symbol, count) {
...
function stackedDecoration(decoration, width, abselem, yPos, positioning, minTop, minBottom, accentAbove) {
    ...
    function textDec(text, placement, anchor) {
    ...
    function symbolDec(symbol, placement) {
        let deltaX = width / 2;
        if (glyphs.getSymbolAlign(symbol) !== "center") {
            deltaX -= (glyphs.getSymbolWidth(symbol) / 2);
        }
