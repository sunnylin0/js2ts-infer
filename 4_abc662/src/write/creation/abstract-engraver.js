...
class AbstractEngraver {
    constructor(getTextSize, tuneNumber, options) {
        this.slurs = {};
        this.ties = [];
        this.voiceScale = 1;
        this.slursbyvoice = {};
    ...
    setStemHeight(heightInPixels) {
        this.stemHeight = Math.round(heightInPixels * 10 / spacing.STEP) / 10;
    }
    getCurrentVoiceId(s, v) {
        return "s" + s + "v" + v;
    }
    pushCrossLineElems(s, v) {
    ...
    createABCStaff(staffgroup, abcstaff, tempo, s, l) {
        staffgroup.getTextSize.updateFonts(abcstaff);
        for (let v = 0; v < abcstaff.voices.length; v++) {
            const voice = new VoiceElement(v, abcstaff.voices.length);
            if (v === 0) {
    ...
    createABCVoice(abcline, tempo, s, v, isSingleLineStaff, voice) {
    ...
    createABCElement(isFirstStaff, isSingleLineStaff, voice, elem) {
        const elemset = [];
        switch (elem.el_type) {
            case undefined:
    ...
    createBeam(isSingleLineStaff, voice, elems) {
        const abselemset = [];
    ...
    createNote(elem, nostem, isSingleLineStaff, voice) {
        let notehead = null;
        let roomtaken = 0;
        let roomtakenright = 0;
        let symbolWidth = 0;
        let additionalLedgers = [];
        let dir;
    ...
    addNoteToAbcElement(abselem, elem, dot, stemdir, style, zeroDuration, durlog, nostem, voice) {
        let dotshiftx = 0;
        let noteHead;
        let roomTaken = 0;
        let roomTakenRight = 0;
    ...
    addGraceNotes(elem, voice, abselem, notehead, stemHeight, isBagpipes, roomtaken) {
    ...
    addSlursAndTies(abselem, pitchelem, notehead, voice, dir, isGrace) {
        if (pitchelem.endTie) {
    ...
    addMeasureNumber(number, abselem) {
    ...
    createBarLine(voice, elem, isFirstStaff) {
...
const getBeamGroup = (abcline, pos) => {
...
const ledgerLines = (abselem, minPitch, maxPitch, isRest, symbolWidth, additionalLedgers, dir, dx, scale) => {
...
const addRestToAbsElement = (abselem, elem, duration, dot, isMultiVoice, stemdir, isSingleLineStaff, durlog, voiceScale) => {
    let c = "";
    let restpitch = 7;
    let noteHead;
    let roomTaken = 0;
