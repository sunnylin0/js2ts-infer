...
## 2. 專案架構 (Architecture)
```text
c:\Users\ESAO_NB27\Desktop\abc-js2ts
├── dist/                          # 編譯後的 CommonJS 與 ESM 產物 (*.js, *.d.ts)
├── history/                       # 歷史變更日誌與進度追蹤
│   ├── Releases.md                # 各版本大綱與變更摘要
│   ├── task_log.md                # 任務清單與接受條件
│   ├── implementation_plan_log.md # 實作計畫存檔
│   └── walkthrough_log.md         # 驗證與說明存檔
...
│   ├── refactor/                  # AST 重構子模組（由 ast-refactorer.ts 拆分而來）
│   │   ├── tsquery-ext.ts         # Node.prototype.query patch（Side-Effect，必須最先 import）
│   │   ├── type-utils.ts          # 型別推導工具群（merge/widen/resolve/unwrap 等 12 個函式）
│   │   ├── annotate.ts            # annotateFunction、resolveAndSetReturnType、refactorCjsToEsm
│   │   ├── process-file.ts        # processFileRefactoring 主管線（單一檔案 JS→TS 重構）
│   │   └── propagation.ts         # 全域型別傳播（Reverse / Forward / ReturnType / writeInterfaces）
│   ├── ast-refactorer.ts          # ⚠ Re-export 入口，實際邏輯已移至 src/refactor/
│   ├── babel-plugin-js2ts.ts      # Babel AST 插樁插件
│   ├── cli.ts                     # CLI 入口定義
│   ├── code-generator.ts          # ts-morph 型別注入與重構生成控制器
│   ├── feedback-loop.ts           # TSC 編譯錯誤反饋與 AI 自我修正（Gemini API，預設旁路關閉）
